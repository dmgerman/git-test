...
op_assign
l_string|&quot;git-pack-objects [-q] [--no-reuse-delta] [--non-empty] [--local] [--incremental] [--window=N] [--depth=N] {--stdout | base-name} [--revs [--unpacked | --all]* &lt;ref-list | &lt;object-list]&quot;
suffix:semicolon
...
multiline_comment|/*&n; * we are going to reuse the existing pack entry data.  make&n; * sure it is not corrupt.&n; */
DECL|function|revalidate_pack_entry
r_static
r_int
id|revalidate_pack_entry
c_func
(paren
r_struct
id|object_entry
...
l_int|0
suffix:semicolon
multiline_comment|/* the caller has already called use_packed_git() for us,&n;&t; * so it is safe to access the pack data from mmapped location.&n;&t; * make sure the entry inflates correctly.&n;&t; */
id|used
op_assign
...
l_int|20
suffix:semicolon
multiline_comment|/* skip base object name */
id|data
op_add_assign
id|used
suffix:semicolon
...
c_func
(paren
l_string|&quot;corrupt delta in pack %s&quot;
comma
id|sha1_to_hex
c_func
(paren
...
multiline_comment|/* copied from rev-list but needs to do things slightly differently */
...
suffix:semicolon
multiline_comment|/* make sure we did not get pathspecs */
r_if
c_cond
(paren
id|revs.prune_data
...
suffix:semicolon
multiline_comment|/* --unpacked and --all makes sense only with --revs */
r_if
c_cond
(paren
op_logical_neg
