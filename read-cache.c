...
mdefine_line|#define CE_NAMEMASK  (0x0fff)
...
DECL|macro|CACHE_EXT_UNTRACKED
...
multiline_comment|/*****************************************************************&n; * Index File I/O&n; *****************************************************************/
DECL|macro|INDEX_FORMAT_DEFAULT
mdefine_line|#define INDEX_FORMAT_DEFAULT 3
DECL|function|get_index_format_default
r_static
r_int
r_int
id|get_index_format_default
c_func
(paren
r_void
)paren
...
c_func
(paren
l_string|&quot;index.version set, but the value is invalid.&bslash;n&quot;
l_string|&quot;Using version %i&quot;
)paren
comma
id|INDEX_FORMAT_DEFAULT
)paren
...
c_func
(paren
l_string|&quot;GIT_INDEX_VERSION set, but the value is invalid.&bslash;n&quot;
l_string|&quot;Using version %i&quot;
)paren
comma
id|INDEX_FORMAT_DEFAULT
...
multiline_comment|/*&n; * dev/ino/uid/gid/size are also just tracked to the low 32 bits&n; * Again - this is just a (very strong in practice) heuristic that&n; * the inode hasn&squot;t changed.&n; *&n; * We save the fields in big-endian order to allow using the&n; * index file over NFS transparently.&n; */
...
suffix:semicolon
multiline_comment|/*&n; * This struct is used when CE_EXTENDED bit is 1&n; * The struct must match ondisk_cache_entry exactly from&n; * ctime till flags&n; */
...
suffix:semicolon
multiline_comment|/* These are only used for v3 or lower */
DECL|macro|align_flex_name
mdefine_line|#define align_flex_name(STRUCT,len) ((offsetof(struct STRUCT,name) + (len) + 8) &amp; ~7)
DECL|macro|ondisk_cache_entry_size
mdefine_line|#define ondisk_cache_entry_size(len) align_flex_name(ondisk_cache_entry,len)
DECL|macro|ondisk_cache_entry_extended_size
mdefine_line|#define ondisk_cache_entry_extended_size(len) align_flex_name(ondisk_cache_entry_extended,len)
DECL|macro|ondisk_ce_size
mdefine_line|#define ondisk_ce_size(ce) (((ce)-&gt;ce_flags &amp; CE_EXTENDED) ? &bslash;&n;&t;&t;&t;    ondisk_cache_entry_extended_size(ce_namelen(ce)) : &bslash;&n;&t;&t;&t;    ondisk_cache_entry_size(ce_namelen(ce)))
...
suffix:semicolon
id|ce-&gt;ce_stat_data.sd_ctime.sec
op_assign
id|get_be32
c_func
(paren
...
suffix:semicolon
id|ce-&gt;ce_stat_data.sd_mtime.sec
op_assign
id|get_be32
c_func
(paren
...
suffix:semicolon
id|ce-&gt;ce_stat_data.sd_ctime.nsec
op_assign
id|get_be32
c_func
(paren
...
suffix:semicolon
id|ce-&gt;ce_stat_data.sd_mtime.nsec
op_assign
id|get_be32
c_func
(paren
...
multiline_comment|/*&n; * Adjacent cache entries tend to share the leading paths, so it makes&n; * sense to only store the differences in later entries.  In the v4&n; * on-disk format of the index, each on-disk cache entry stores the&n; * number of bytes to be stripped from the end of the previous name,&n; * and the bytes to append to the result, to come up with its name.&n; */
DECL|function|expand_name_field
r_static
r_int
r_int
id|expand_name_field
c_func
(paren
r_struct
id|strbuf
...
c_func
(paren
l_string|&quot;malformed name field in the index&quot;
)paren
suffix:semicolon
id|strbuf_remove
c_func
...
suffix:semicolon
multiline_comment|/* find the end */
id|strbuf_add
c_func
(paren
id|name
...
(brace
multiline_comment|/* v3 and earlier */
r_if
c_cond
(paren
id|len
...
id|flags
comma
id|previous_name-&gt;buf
comma
id|previous_name-&gt;len
)paren
suffix:semicolon
op_star
id|ent_size
...
c_func
(paren
l_string|&quot;unordered stage entries in index&quot;
)paren
suffix:semicolon
r_if
c_cond
...
c_func
(paren
l_string|&quot;multiple stage entries for merged file &squot;%s&squot;&quot;
comma
id|ce-&gt;name
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;unordered stage entries for &squot;%s&squot;&quot;
comma
id|ce-&gt;name
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;%s: index file open failed&quot;
comma
id|path
)paren
suffix:semicolon
...
id|ret
suffix:semicolon
multiline_comment|/* istate-&gt;initialized covers both .git/index and .git/sharedindex.xxx */
r_if
c_cond
(paren
id|istate-&gt;initialized
...
id|split_index-&gt;base_sha1
comma
id|split_index-&gt;base-&gt;sha1
)paren
)paren
id|die
c_func
(paren
l_string|&quot;broken index, expect %s in %s, got %s&quot;
comma
id|sha1_to_hex
c_func
(paren
...
multiline_comment|/* Copy miscellaneous fields but not the name */
DECL|function|copy_cache_entry_to_ondisk
r_static
r_char
op_star
id|copy_cache_entry_to_ondisk
c_func
(paren
r_struct
id|ondisk_cache_entry
...
id|saved_namelen
suffix:semicolon
multiline_comment|/* compiler workaround */
r_char
op_star
id|name
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* still matching */
id|to_remove
op_assign
id|previous_name-&gt;len
id|common
...
multiline_comment|/*&n; * This function verifies if index_state has the correct sha1 of the&n; * index file.  Don&squot;t die if we have any other failure, just return 0.&n; */
DECL|function|verify_index_from
r_static
r_int
id|verify_index_from
c_func
(paren
r_const
r_struct
...
multiline_comment|/* demote version 3 to version 2 when the latter suffices */
r_if
c_cond
(paren
id|istate-&gt;version
...
r_char
op_star
id|temporary_sharedindex
suffix:semicolon
DECL|function|remove_temporary_sharedindex
r_static
r_void
id|remove_temporary_sharedindex
c_func
(paren
r_void
)paren
...
r_static
r_void
id|remove_temporary_sharedindex_on_signal
c_func
(paren
r_int
id|signo
