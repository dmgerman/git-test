...
multiline_comment|/*&n;&t; * To prevent later ce_match_stat() from always falling into&n;&t; * check_fs(), if we have too many entries that can trigger&n;&t; * racily clean check, we are better off delaying the return.&n;&t; * We arbitrarily say if more than 20 paths or 25% of total&n;&t; * paths are very new, we delay the return until the index&n;&t; * file gets a new timestamp.&n;&t; *&n;&t; * NOTE! NOTE! NOTE!&n;&t; *&n;&t; * This assumes that nobody is touching the working tree while&n;&t; * we are updating the index.&n;&t; */
multiline_comment|/* Make sure that the new index file has st_mtime&n;&t; * that is current enough -- ce_write() batches the data&n;&t; * so it might not have written anything yet.&n;&t; */
id|ce_write_flush
c_func
(paren
op_amp
