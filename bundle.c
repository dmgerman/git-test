macro_line|#include &quot;cache.h&quot;
macro_line|#include &quot;lockfile.h&quot;
macro_line|#include &quot;bundle.h&quot;
macro_line|#include &quot;object.h&quot;
macro_line|#include &quot;commit.h&quot;
macro_line|#include &quot;diff.h&quot;
macro_line|#include &quot;revision.h&quot;
macro_line|#include &quot;list-objects.h&quot;
macro_line|#include &quot;run-command.h&quot;
macro_line|#include &quot;refs.h&quot;
macro_line|#include &quot;argv-array.h&quot;
DECL|variable|bundle_signature
r_static
r_const
r_char
id|bundle_signature
(braket
)braket
op_assign
l_string|&quot;# v2 git bundle&bslash;n&quot;
suffix:semicolon
DECL|function|add_to_ref_list
r_static
r_void
id|add_to_ref_list
c_func
(paren
r_const
r_int
r_char
op_star
id|sha1
comma
r_const
r_char
op_star
id|name
comma
r_struct
id|ref_list
op_star
id|list
)paren
(brace
id|ALLOC_GROW
c_func
(paren
id|list-&gt;list
comma
id|list-&gt;nr
op_plus
l_int|1
comma
id|list-&gt;alloc
)paren
suffix:semicolon
id|hashcpy
c_func
(paren
id|list-&gt;list
(braket
id|list-&gt;nr
)braket
dot
id|sha1
comma
id|sha1
)paren
suffix:semicolon
id|list-&gt;list
(braket
id|list-&gt;nr
)braket
dot
id|name
op_assign
id|xstrdup
c_func
(paren
id|name
)paren
suffix:semicolon
id|list-&gt;nr
op_increment
suffix:semicolon
)brace
DECL|function|parse_bundle_header
r_static
r_int
id|parse_bundle_header
c_func
(paren
r_int
id|fd
comma
r_struct
id|bundle_header
op_star
id|header
comma
r_const
r_char
op_star
id|report_path
)paren
(brace
r_struct
id|strbuf
id|buf
op_assign
id|STRBUF_INIT
suffix:semicolon
r_int
id|status
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* The bundle header begins with the signature */
r_if
c_cond
(paren
id|strbuf_getwholeline_fd
c_func
(paren
op_amp
id|buf
comma
id|fd
comma
l_char|&squot;&bslash;n&squot;
)paren
op_logical_or
id|strcmp
c_func
(paren
id|buf.buf
comma
id|bundle_signature
)paren
)paren
(brace
r_if
c_cond
(paren
id|report_path
)paren
id|error
c_func
(paren
id|_
c_func
(paren
l_string|&quot;&squot;%s&squot; does not look like a v2 bundle file&quot;
)paren
comma
id|report_path
)paren
suffix:semicolon
id|status
op_assign
l_int|1
suffix:semicolon
r_goto
m_abort
suffix:semicolon
)brace
multiline_comment|/* The bundle header ends with an empty line */
r_while
c_loop
(paren
op_logical_neg
id|strbuf_getwholeline_fd
c_func
(paren
op_amp
id|buf
comma
id|fd
comma
l_char|&squot;&bslash;n&squot;
)paren
op_logical_and
id|buf.len
op_logical_and
id|buf.buf
(braket
l_int|0
)braket
op_ne
l_char|&squot;&bslash;n&squot;
)paren
(brace
r_int
r_char
id|sha1
(braket
l_int|20
)braket
suffix:semicolon
r_int
id|is_prereq
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_star
id|buf.buf
op_eq
l_char|&squot;-&squot;
)paren
(brace
id|is_prereq
op_assign
l_int|1
suffix:semicolon
id|strbuf_remove
c_func
(paren
op_amp
id|buf
comma
l_int|0
comma
l_int|1
)paren
suffix:semicolon
)brace
id|strbuf_rtrim
c_func
(paren
op_amp
id|buf
)paren
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Tip lines have object name, SP, and refname.&n;&t;&t; * Prerequisites have object name that is optionally&n;&t;&t; * followed by SP and subject line.&n;&t;&t; */
r_if
c_cond
(paren
id|get_sha1_hex
c_func
(paren
id|buf.buf
comma
id|sha1
)paren
op_logical_or
(paren
id|buf.len
OG
l_int|40
op_logical_and
op_logical_neg
id|isspace
c_func
(paren
id|buf.buf
(braket
l_int|40
)braket
)paren
)paren
op_logical_or
(paren
op_logical_neg
id|is_prereq
op_logical_and
id|buf.len
op_le
l_int|40
)paren
)paren
(brace
r_if
c_cond
(paren
id|report_path
)paren
id|error
c_func
(paren
id|_
c_func
(paren
l_string|&quot;unrecognized header: %s%s (%d)&quot;
)paren
comma
(paren
id|is_prereq
ques
c_cond
l_string|&quot;-&quot;
suffix:colon
l_string|&quot;&quot;
)paren
comma
id|buf.buf
comma
(paren
r_int
)paren
id|buf.len
)paren
suffix:semicolon
id|status
op_assign
l_int|1
suffix:semicolon
r_break
suffix:semicolon
)brace
r_else
(brace
r_if
c_cond
(paren
id|is_prereq
)paren
id|add_to_ref_list
c_func
(paren
id|sha1
comma
l_string|&quot;&quot;
comma
op_amp
id|header-&gt;prerequisites
)paren
suffix:semicolon
r_else
id|add_to_ref_list
c_func
(paren
id|sha1
comma
id|buf.buf
op_plus
l_int|41
comma
op_amp
id|header-&gt;references
)paren
suffix:semicolon
)brace
)brace
m_abort
suffix:colon
r_if
c_cond
(paren
id|status
)paren
(brace
id|close
c_func
(paren
id|fd
)paren
suffix:semicolon
id|fd
op_assign
l_int|1
suffix:semicolon
)brace
id|strbuf_release
c_func
(paren
op_amp
id|buf
)paren
suffix:semicolon
r_return
id|fd
suffix:semicolon
)brace
DECL|function|read_bundle_header
r_int
id|read_bundle_header
c_func
(paren
r_const
r_char
op_star
id|path
comma
r_struct
id|bundle_header
op_star
id|header
)paren
(brace
r_int
id|fd
op_assign
id|open
c_func
(paren
id|path
comma
id|O_RDONLY
)paren
suffix:semicolon
r_if
c_cond
(paren
id|fd
OL
l_int|0
)paren
r_return
id|error
c_func
(paren
id|_
c_func
(paren
l_string|&quot;could not open &squot;%s&squot;&quot;
)paren
comma
id|path
)paren
suffix:semicolon
r_return
id|parse_bundle_header
c_func
(paren
id|fd
comma
id|header
comma
id|path
)paren
suffix:semicolon
)brace
DECL|function|is_bundle
r_int
id|is_bundle
c_func
(paren
r_const
r_char
op_star
id|path
comma
r_int
id|quiet
)paren
(brace
r_struct
id|bundle_header
id|header
suffix:semicolon
r_int
id|fd
op_assign
id|open
c_func
(paren
id|path
comma
id|O_RDONLY
)paren
suffix:semicolon
r_if
c_cond
(paren
id|fd
OL
l_int|0
)paren
r_return
l_int|0
suffix:semicolon
id|memset
c_func
(paren
op_amp
id|header
comma
l_int|0
comma
r_sizeof
(paren
id|header
)paren
)paren
suffix:semicolon
id|fd
op_assign
id|parse_bundle_header
c_func
(paren
id|fd
comma
op_amp
id|header
comma
id|quiet
ques
c_cond
l_int|NULL
suffix:colon
id|path
)paren
suffix:semicolon
r_if
c_cond
(paren
id|fd
op_ge
l_int|0
)paren
id|close
c_func
(paren
id|fd
)paren
suffix:semicolon
r_return
(paren
id|fd
op_ge
l_int|0
)paren
suffix:semicolon
)brace
DECL|function|list_refs
r_static
r_int
id|list_refs
c_func
(paren
r_struct
id|ref_list
op_star
id|r
comma
r_int
id|argc
comma
r_const
r_char
op_star
op_star
id|argv
)paren
(brace
r_int
id|i
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|r-&gt;nr
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
id|argc
OG
l_int|1
)paren
(brace
r_int
id|j
suffix:semicolon
r_for
c_loop
(paren
id|j
op_assign
l_int|1
suffix:semicolon
id|j
OL
id|argc
suffix:semicolon
id|j
op_increment
)paren
r_if
c_cond
(paren
op_logical_neg
id|strcmp
c_func
(paren
id|r-&gt;list
(braket
id|i
)braket
dot
id|name
comma
id|argv
(braket
id|j
)braket
)paren
)paren
r_break
suffix:semicolon
r_if
c_cond
(paren
id|j
op_eq
id|argc
)paren
r_continue
suffix:semicolon
)brace
id|printf
c_func
(paren
l_string|&quot;%s %s&bslash;n&quot;
comma
id|sha1_to_hex
c_func
(paren
id|r-&gt;list
(braket
id|i
)braket
dot
id|sha1
)paren
comma
id|r-&gt;list
(braket
id|i
)braket
dot
id|name
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* Remember to update object flag allocation in object.h */
DECL|macro|PREREQ_MARK
mdefine_line|#define PREREQ_MARK (1u&lt;&lt;16)
DECL|function|verify_bundle
r_int
id|verify_bundle
c_func
(paren
r_struct
id|bundle_header
op_star
id|header
comma
r_int
id|verbose
)paren
(brace
multiline_comment|/*&n;&t; * Do fast check, then if any prereqs are missing then go line by line&n;&t; * to be verbose about the errors&n;&t; */
r_struct
id|ref_list
op_star
id|p
op_assign
op_amp
id|header-&gt;prerequisites
suffix:semicolon
r_struct
id|rev_info
id|revs
suffix:semicolon
r_const
r_char
op_star
id|argv
(braket
)braket
op_assign
(brace
l_int|NULL
comma
l_string|&quot;--all&quot;
comma
l_int|NULL
)brace
suffix:semicolon
r_struct
id|object_array
id|refs
suffix:semicolon
r_struct
id|commit
op_star
id|commit
suffix:semicolon
r_int
id|i
comma
id|ret
op_assign
l_int|0
comma
id|req_nr
suffix:semicolon
r_const
r_char
op_star
id|message
op_assign
id|_
c_func
(paren
l_string|&quot;Repository lacks these prerequisite commits:&quot;
)paren
suffix:semicolon
id|init_revisions
c_func
(paren
op_amp
id|revs
comma
l_int|NULL
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|p-&gt;nr
suffix:semicolon
id|i
op_increment
)paren
(brace
r_struct
id|ref_list_entry
op_star
id|e
op_assign
id|p-&gt;list
op_plus
id|i
suffix:semicolon
r_struct
id|object
op_star
id|o
op_assign
id|parse_object
c_func
(paren
id|e-&gt;sha1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|o
)paren
(brace
id|o-&gt;flags
op_or_assign
id|PREREQ_MARK
suffix:semicolon
id|add_pending_object
c_func
(paren
op_amp
id|revs
comma
id|o
comma
id|e-&gt;name
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_increment
id|ret
op_eq
l_int|1
)paren
id|error
c_func
(paren
l_string|&quot;%s&quot;
comma
id|message
)paren
suffix:semicolon
id|error
c_func
(paren
l_string|&quot;%s %s&quot;
comma
id|sha1_to_hex
c_func
(paren
id|e-&gt;sha1
)paren
comma
id|e-&gt;name
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|revs.pending.nr
op_ne
id|p-&gt;nr
)paren
r_return
id|ret
suffix:semicolon
id|req_nr
op_assign
id|revs.pending.nr
suffix:semicolon
id|setup_revisions
c_func
(paren
l_int|2
comma
id|argv
comma
op_amp
id|revs
comma
l_int|NULL
)paren
suffix:semicolon
id|refs
op_assign
id|revs.pending
suffix:semicolon
id|revs.leak_pending
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|prepare_revision_walk
c_func
(paren
op_amp
id|revs
)paren
)paren
id|die
c_func
(paren
id|_
c_func
(paren
l_string|&quot;revision walk setup failed&quot;
)paren
)paren
suffix:semicolon
id|i
op_assign
id|req_nr
suffix:semicolon
r_while
c_loop
(paren
id|i
op_logical_and
(paren
id|commit
op_assign
id|get_revision
c_func
(paren
op_amp
id|revs
)paren
)paren
)paren
r_if
c_cond
(paren
id|commit-&gt;object.flags
op_amp
id|PREREQ_MARK
)paren
id|i
op_decrement
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|req_nr
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
op_logical_neg
(paren
id|refs.objects
(braket
id|i
)braket
dot
id|item-&gt;flags
op_amp
id|SHOWN
)paren
)paren
(brace
r_if
c_cond
(paren
op_increment
id|ret
op_eq
l_int|1
)paren
id|error
c_func
(paren
l_string|&quot;%s&quot;
comma
id|message
)paren
suffix:semicolon
id|error
c_func
(paren
l_string|&quot;%s %s&quot;
comma
id|oid_to_hex
c_func
(paren
op_amp
id|refs.objects
(braket
id|i
)braket
dot
id|item-&gt;oid
)paren
comma
id|refs.objects
(braket
id|i
)braket
dot
id|name
)paren
suffix:semicolon
)brace
id|clear_commit_marks_for_object_array
c_func
(paren
op_amp
id|refs
comma
id|ALL_REV_FLAGS
)paren
suffix:semicolon
id|free
c_func
(paren
id|refs.objects
)paren
suffix:semicolon
r_if
c_cond
(paren
id|verbose
)paren
(brace
r_struct
id|ref_list
op_star
id|r
suffix:semicolon
id|r
op_assign
op_amp
id|header-&gt;references
suffix:semicolon
id|printf_ln
c_func
(paren
id|Q_
c_func
(paren
l_string|&quot;The bundle contains this ref:&quot;
comma
l_string|&quot;The bundle contains these %d refs:&quot;
comma
id|r-&gt;nr
)paren
comma
id|r-&gt;nr
)paren
suffix:semicolon
id|list_refs
c_func
(paren
id|r
comma
l_int|0
comma
l_int|NULL
)paren
suffix:semicolon
id|r
op_assign
op_amp
id|header-&gt;prerequisites
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|r-&gt;nr
)paren
(brace
id|printf_ln
c_func
(paren
id|_
c_func
(paren
l_string|&quot;The bundle records a complete history.&quot;
)paren
)paren
suffix:semicolon
)brace
r_else
(brace
id|printf_ln
c_func
(paren
id|Q_
c_func
(paren
l_string|&quot;The bundle requires this ref:&quot;
comma
l_string|&quot;The bundle requires these %d refs:&quot;
comma
id|r-&gt;nr
)paren
comma
id|r-&gt;nr
)paren
suffix:semicolon
id|list_refs
c_func
(paren
id|r
comma
l_int|0
comma
l_int|NULL
)paren
suffix:semicolon
)brace
)brace
r_return
id|ret
suffix:semicolon
)brace
DECL|function|list_bundle_refs
r_int
id|list_bundle_refs
c_func
(paren
r_struct
id|bundle_header
op_star
id|header
comma
r_int
id|argc
comma
r_const
r_char
op_star
op_star
id|argv
)paren
(brace
r_return
id|list_refs
c_func
(paren
op_amp
id|header-&gt;references
comma
id|argc
comma
id|argv
)paren
suffix:semicolon
)brace
DECL|function|is_tag_in_date_range
r_static
r_int
id|is_tag_in_date_range
c_func
(paren
r_struct
id|object
op_star
id|tag
comma
r_struct
id|rev_info
op_star
id|revs
)paren
(brace
r_int
r_int
id|size
suffix:semicolon
r_enum
id|object_type
id|type
suffix:semicolon
r_char
op_star
id|buf
op_assign
l_int|NULL
comma
op_star
id|line
comma
op_star
id|lineend
suffix:semicolon
r_int
r_int
id|date
suffix:semicolon
r_int
id|result
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|revs-&gt;max_age
op_eq
l_int|1
op_logical_and
id|revs-&gt;min_age
op_eq
l_int|1
)paren
r_goto
id|out
suffix:semicolon
id|buf
op_assign
id|read_sha1_file
c_func
(paren
id|get_object_hash
c_func
(paren
op_star
id|tag
)paren
comma
op_amp
id|type
comma
op_amp
id|size
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|buf
)paren
r_goto
id|out
suffix:semicolon
id|line
op_assign
id|memmem
c_func
(paren
id|buf
comma
id|size
comma
l_string|&quot;&bslash;ntagger &quot;
comma
l_int|8
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|line
op_increment
)paren
r_goto
id|out
suffix:semicolon
id|lineend
op_assign
id|memchr
c_func
(paren
id|line
comma
l_char|&squot;&bslash;n&squot;
comma
id|buf
op_plus
id|size
id|line
)paren
suffix:semicolon
id|line
op_assign
id|memchr
c_func
(paren
id|line
comma
l_char|&squot;&gt;&squot;
comma
id|lineend
ques
c_cond
id|lineend
id|line
suffix:colon
id|buf
op_plus
id|size
id|line
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|line
op_increment
)paren
r_goto
id|out
suffix:semicolon
id|date
op_assign
id|strtoul
c_func
(paren
id|line
comma
l_int|NULL
comma
l_int|10
)paren
suffix:semicolon
id|result
op_assign
(paren
id|revs-&gt;max_age
op_eq
l_int|1
op_logical_or
id|revs-&gt;max_age
OL
id|date
)paren
op_logical_and
(paren
id|revs-&gt;min_age
op_eq
l_int|1
op_logical_or
id|revs-&gt;min_age
OG
id|date
)paren
suffix:semicolon
id|out
suffix:colon
id|free
c_func
(paren
id|buf
)paren
suffix:semicolon
r_return
id|result
suffix:semicolon
)brace
multiline_comment|/* Write the pack data to bundle_fd, then close it if it is &gt; 1. */
DECL|function|write_pack_data
r_static
r_int
id|write_pack_data
c_func
(paren
r_int
id|bundle_fd
comma
r_struct
id|rev_info
op_star
id|revs
)paren
(brace
r_struct
id|child_process
id|pack_objects
op_assign
id|CHILD_PROCESS_INIT
suffix:semicolon
r_int
id|i
suffix:semicolon
id|argv_array_pushl
c_func
(paren
op_amp
id|pack_objects.args
comma
l_string|&quot;pack-objects&quot;
comma
l_string|&quot;--all-progress-implied&quot;
comma
l_string|&quot;--stdout&quot;
comma
l_string|&quot;--thin&quot;
comma
l_string|&quot;--delta-base-offset&quot;
comma
l_int|NULL
)paren
suffix:semicolon
id|pack_objects.in
op_assign
l_int|1
suffix:semicolon
id|pack_objects.out
op_assign
id|bundle_fd
suffix:semicolon
id|pack_objects.git_cmd
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|start_command
c_func
(paren
op_amp
id|pack_objects
)paren
)paren
r_return
id|error
c_func
(paren
id|_
c_func
(paren
l_string|&quot;Could not spawn pack-objects&quot;
)paren
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|revs-&gt;pending.nr
suffix:semicolon
id|i
op_increment
)paren
(brace
r_struct
id|object
op_star
id|object
op_assign
id|revs-&gt;pending.objects
(braket
id|i
)braket
dot
id|item
suffix:semicolon
r_if
c_cond
(paren
id|object-&gt;flags
op_amp
id|UNINTERESTING
)paren
id|write_or_die
c_func
(paren
id|pack_objects.in
comma
l_string|&quot;^&quot;
comma
l_int|1
)paren
suffix:semicolon
id|write_or_die
c_func
(paren
id|pack_objects.in
comma
id|oid_to_hex
c_func
(paren
op_amp
id|object-&gt;oid
)paren
comma
id|GIT_SHA1_HEXSZ
)paren
suffix:semicolon
id|write_or_die
c_func
(paren
id|pack_objects.in
comma
l_string|&quot;&bslash;n&quot;
comma
l_int|1
)paren
suffix:semicolon
)brace
id|close
c_func
(paren
id|pack_objects.in
)paren
suffix:semicolon
r_if
c_cond
(paren
id|finish_command
c_func
(paren
op_amp
id|pack_objects
)paren
)paren
r_return
id|error
c_func
(paren
id|_
c_func
(paren
l_string|&quot;pack-objects died&quot;
)paren
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|compute_and_write_prerequisites
r_static
r_int
id|compute_and_write_prerequisites
c_func
(paren
r_int
id|bundle_fd
comma
r_struct
id|rev_info
op_star
id|revs
comma
r_int
id|argc
comma
r_const
r_char
op_star
op_star
id|argv
)paren
(brace
r_struct
id|child_process
id|rls
op_assign
id|CHILD_PROCESS_INIT
suffix:semicolon
r_struct
id|strbuf
id|buf
op_assign
id|STRBUF_INIT
suffix:semicolon
id|FILE
op_star
id|rls_fout
suffix:semicolon
r_int
id|i
suffix:semicolon
id|argv_array_pushl
c_func
(paren
op_amp
id|rls.args
comma
l_string|&quot;rev-list&quot;
comma
l_string|&quot;--boundary&quot;
comma
l_string|&quot;--pretty=oneline&quot;
comma
l_int|NULL
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|1
suffix:semicolon
id|i
OL
id|argc
suffix:semicolon
id|i
op_increment
)paren
id|argv_array_push
c_func
(paren
op_amp
id|rls.args
comma
id|argv
(braket
id|i
)braket
)paren
suffix:semicolon
id|rls.out
op_assign
l_int|1
suffix:semicolon
id|rls.git_cmd
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|start_command
c_func
(paren
op_amp
id|rls
)paren
)paren
r_return
l_int|1
suffix:semicolon
id|rls_fout
op_assign
id|xfdopen
c_func
(paren
id|rls.out
comma
l_string|&quot;r&quot;
)paren
suffix:semicolon
r_while
c_loop
(paren
id|strbuf_getwholeline
c_func
(paren
op_amp
id|buf
comma
id|rls_fout
comma
l_char|&squot;&bslash;n&squot;
)paren
op_ne
id|EOF
)paren
(brace
r_int
r_char
id|sha1
(braket
l_int|20
)braket
suffix:semicolon
r_if
c_cond
(paren
id|buf.len
OG
l_int|0
op_logical_and
id|buf.buf
(braket
l_int|0
)braket
op_eq
l_char|&squot;-&squot;
)paren
(brace
id|write_or_die
c_func
(paren
id|bundle_fd
comma
id|buf.buf
comma
id|buf.len
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|get_sha1_hex
c_func
(paren
id|buf.buf
op_plus
l_int|1
comma
id|sha1
)paren
)paren
(brace
r_struct
id|object
op_star
id|object
op_assign
id|parse_object_or_die
c_func
(paren
id|sha1
comma
id|buf.buf
)paren
suffix:semicolon
id|object-&gt;flags
op_or_assign
id|UNINTERESTING
suffix:semicolon
id|add_pending_object
c_func
(paren
id|revs
comma
id|object
comma
id|buf.buf
)paren
suffix:semicolon
)brace
)brace
r_else
r_if
c_cond
(paren
op_logical_neg
id|get_sha1_hex
c_func
(paren
id|buf.buf
comma
id|sha1
)paren
)paren
(brace
r_struct
id|object
op_star
id|object
op_assign
id|parse_object_or_die
c_func
(paren
id|sha1
comma
id|buf.buf
)paren
suffix:semicolon
id|object-&gt;flags
op_or_assign
id|SHOWN
suffix:semicolon
)brace
)brace
id|strbuf_release
c_func
(paren
op_amp
id|buf
)paren
suffix:semicolon
id|fclose
c_func
(paren
id|rls_fout
)paren
suffix:semicolon
r_if
c_cond
(paren
id|finish_command
c_func
(paren
op_amp
id|rls
)paren
)paren
r_return
id|error
c_func
(paren
id|_
c_func
(paren
l_string|&quot;rev-list died&quot;
)paren
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * Write out bundle refs based on the tips already&n; * parsed into revs.pending. As a side effect, may&n; * manipulate revs.pending to include additional&n; * necessary objects (like tags).&n; *&n; * Returns the number of refs written, or negative&n; * on error.&n; */
DECL|function|write_bundle_refs
r_static
r_int
id|write_bundle_refs
c_func
(paren
r_int
id|bundle_fd
comma
r_struct
id|rev_info
op_star
id|revs
)paren
(brace
r_int
id|i
suffix:semicolon
r_int
id|ref_count
op_assign
l_int|0
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|revs-&gt;pending.nr
suffix:semicolon
id|i
op_increment
)paren
(brace
r_struct
id|object_array_entry
op_star
id|e
op_assign
id|revs-&gt;pending.objects
op_plus
id|i
suffix:semicolon
r_struct
id|object_id
id|oid
suffix:semicolon
r_char
op_star
id|ref
suffix:semicolon
r_const
r_char
op_star
id|display_ref
suffix:semicolon
r_int
id|flag
suffix:semicolon
r_if
c_cond
(paren
id|e-&gt;item-&gt;flags
op_amp
id|UNINTERESTING
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|dwim_ref
c_func
(paren
id|e-&gt;name
comma
id|strlen
c_func
(paren
id|e-&gt;name
)paren
comma
id|oid.hash
comma
op_amp
id|ref
)paren
op_ne
l_int|1
)paren
r_goto
id|skip_write_ref
suffix:semicolon
r_if
c_cond
(paren
id|read_ref_full
c_func
(paren
id|e-&gt;name
comma
id|RESOLVE_REF_READING
comma
id|oid.hash
comma
op_amp
id|flag
)paren
)paren
id|flag
op_assign
l_int|0
suffix:semicolon
id|display_ref
op_assign
(paren
id|flag
op_amp
id|REF_ISSYMREF
)paren
ques
c_cond
id|e-&gt;name
suffix:colon
id|ref
suffix:semicolon
r_if
c_cond
(paren
id|e-&gt;item-&gt;type
op_eq
id|OBJ_TAG
op_logical_and
op_logical_neg
id|is_tag_in_date_range
c_func
(paren
id|e-&gt;item
comma
id|revs
)paren
)paren
(brace
id|e-&gt;item-&gt;flags
op_or_assign
id|UNINTERESTING
suffix:semicolon
r_goto
id|skip_write_ref
suffix:semicolon
)brace
multiline_comment|/*&n;&t;&t; * Make sure the refs we wrote out is correct; --max-count and&n;&t;&t; * other limiting options could have prevented all the tips&n;&t;&t; * from getting output.&n;&t;&t; *&n;&t;&t; * Non commit objects such as tags and blobs do not have&n;&t;&t; * this issue as they are not affected by those extra&n;&t;&t; * constraints.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
(paren
id|e-&gt;item-&gt;flags
op_amp
id|SHOWN
)paren
op_logical_and
id|e-&gt;item-&gt;type
op_eq
id|OBJ_COMMIT
)paren
(brace
id|warning
c_func
(paren
id|_
c_func
(paren
l_string|&quot;ref &squot;%s&squot; is excluded by the rev-list options&quot;
)paren
comma
id|e-&gt;name
)paren
suffix:semicolon
r_goto
id|skip_write_ref
suffix:semicolon
)brace
multiline_comment|/*&n;&t;&t; * If you run &quot;git bundle create bndl v1.0..v2.0&quot;, the&n;&t;&t; * name of the positive ref is &quot;v2.0&quot; but that is the&n;&t;&t; * commit that is referenced by the tag, and not the tag&n;&t;&t; * itself.&n;&t;&t; */
r_if
c_cond
(paren
id|oidcmp
c_func
(paren
op_amp
id|oid
comma
op_amp
id|e-&gt;item-&gt;oid
)paren
)paren
(brace
multiline_comment|/*&n;&t;&t;&t; * Is this the positive end of a range expressed&n;&t;&t;&t; * in terms of a tag (e.g. v2.0 from the range&n;&t;&t;&t; * &quot;v1.0..v2.0&quot;)?&n;&t;&t;&t; */
r_struct
id|commit
op_star
id|one
op_assign
id|lookup_commit_reference
c_func
(paren
id|oid.hash
)paren
suffix:semicolon
r_struct
id|object
op_star
id|obj
suffix:semicolon
r_if
c_cond
(paren
id|e-&gt;item
op_eq
op_amp
(paren
id|one-&gt;object
)paren
)paren
(brace
multiline_comment|/*&n;&t;&t;&t;&t; * Need to include e-&gt;name as an&n;&t;&t;&t;&t; * independent ref to the pack-objects&n;&t;&t;&t;&t; * input, so that the tag is included&n;&t;&t;&t;&t; * in the output; otherwise we would&n;&t;&t;&t;&t; * end up triggering &quot;empty bundle&quot;&n;&t;&t;&t;&t; * error.&n;&t;&t;&t;&t; */
id|obj
op_assign
id|parse_object_or_die
c_func
(paren
id|oid.hash
comma
id|e-&gt;name
)paren
suffix:semicolon
id|obj-&gt;flags
op_or_assign
id|SHOWN
suffix:semicolon
id|add_pending_object
c_func
(paren
id|revs
comma
id|obj
comma
id|e-&gt;name
)paren
suffix:semicolon
)brace
r_goto
id|skip_write_ref
suffix:semicolon
)brace
id|ref_count
op_increment
suffix:semicolon
id|write_or_die
c_func
(paren
id|bundle_fd
comma
id|oid_to_hex
c_func
(paren
op_amp
id|e-&gt;item-&gt;oid
)paren
comma
l_int|40
)paren
suffix:semicolon
id|write_or_die
c_func
(paren
id|bundle_fd
comma
l_string|&quot; &quot;
comma
l_int|1
)paren
suffix:semicolon
id|write_or_die
c_func
(paren
id|bundle_fd
comma
id|display_ref
comma
id|strlen
c_func
(paren
id|display_ref
)paren
)paren
suffix:semicolon
id|write_or_die
c_func
(paren
id|bundle_fd
comma
l_string|&quot;&bslash;n&quot;
comma
l_int|1
)paren
suffix:semicolon
id|skip_write_ref
suffix:colon
id|free
c_func
(paren
id|ref
)paren
suffix:semicolon
)brace
multiline_comment|/* end header */
id|write_or_die
c_func
(paren
id|bundle_fd
comma
l_string|&quot;&bslash;n&quot;
comma
l_int|1
)paren
suffix:semicolon
r_return
id|ref_count
suffix:semicolon
)brace
DECL|function|create_bundle
r_int
id|create_bundle
c_func
(paren
r_struct
id|bundle_header
op_star
id|header
comma
r_const
r_char
op_star
id|path
comma
r_int
id|argc
comma
r_const
r_char
op_star
op_star
id|argv
)paren
(brace
r_static
r_struct
id|lock_file
id|lock
suffix:semicolon
r_int
id|bundle_fd
op_assign
l_int|1
suffix:semicolon
r_int
id|bundle_to_stdout
suffix:semicolon
r_int
id|ref_count
op_assign
l_int|0
suffix:semicolon
r_struct
id|rev_info
id|revs
suffix:semicolon
id|bundle_to_stdout
op_assign
op_logical_neg
id|strcmp
c_func
(paren
id|path
comma
l_string|&quot;-&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|bundle_to_stdout
)paren
id|bundle_fd
op_assign
l_int|1
suffix:semicolon
r_else
(brace
id|bundle_fd
op_assign
id|hold_lock_file_for_update
c_func
(paren
op_amp
id|lock
comma
id|path
comma
id|LOCK_DIE_ON_ERROR
)paren
suffix:semicolon
multiline_comment|/*&n;&t;&t; * write_pack_data() will close the fd passed to it,&n;&t;&t; * but commit_lock_file() will also try to close the&n;&t;&t; * lockfile&squot;s fd. So make a copy of the file&n;&t;&t; * descriptor to avoid trying to close it twice.&n;&t;&t; */
id|bundle_fd
op_assign
id|dup
c_func
(paren
id|bundle_fd
)paren
suffix:semicolon
r_if
c_cond
(paren
id|bundle_fd
OL
l_int|0
)paren
id|die_errno
c_func
(paren
l_string|&quot;unable to dup file descriptor&quot;
)paren
suffix:semicolon
)brace
multiline_comment|/* write signature */
id|write_or_die
c_func
(paren
id|bundle_fd
comma
id|bundle_signature
comma
id|strlen
c_func
(paren
id|bundle_signature
)paren
)paren
suffix:semicolon
multiline_comment|/* init revs to list objects for pack-objects later */
id|save_commit_buffer
op_assign
l_int|0
suffix:semicolon
id|init_revisions
c_func
(paren
op_amp
id|revs
comma
l_int|NULL
)paren
suffix:semicolon
multiline_comment|/* write prerequisites */
r_if
c_cond
(paren
id|compute_and_write_prerequisites
c_func
(paren
id|bundle_fd
comma
op_amp
id|revs
comma
id|argc
comma
id|argv
)paren
)paren
r_return
l_int|1
suffix:semicolon
id|argc
op_assign
id|setup_revisions
c_func
(paren
id|argc
comma
id|argv
comma
op_amp
id|revs
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|argc
OG
l_int|1
)paren
r_return
id|error
c_func
(paren
id|_
c_func
(paren
l_string|&quot;unrecognized argument: %s&quot;
)paren
comma
id|argv
(braket
l_int|1
)braket
)paren
suffix:semicolon
id|object_array_remove_duplicates
c_func
(paren
op_amp
id|revs.pending
)paren
suffix:semicolon
id|ref_count
op_assign
id|write_bundle_refs
c_func
(paren
id|bundle_fd
comma
op_amp
id|revs
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ref_count
)paren
id|die
c_func
(paren
id|_
c_func
(paren
l_string|&quot;Refusing to create empty bundle.&quot;
)paren
)paren
suffix:semicolon
r_else
r_if
c_cond
(paren
id|ref_count
OL
l_int|0
)paren
r_return
l_int|1
suffix:semicolon
multiline_comment|/* write pack */
r_if
c_cond
(paren
id|write_pack_data
c_func
(paren
id|bundle_fd
comma
op_amp
id|revs
)paren
)paren
r_return
l_int|1
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bundle_to_stdout
)paren
(brace
r_if
c_cond
(paren
id|commit_lock_file
c_func
(paren
op_amp
id|lock
)paren
)paren
id|die_errno
c_func
(paren
id|_
c_func
(paren
l_string|&quot;cannot create &squot;%s&squot;&quot;
)paren
comma
id|path
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|unbundle
r_int
id|unbundle
c_func
(paren
r_struct
id|bundle_header
op_star
id|header
comma
r_int
id|bundle_fd
comma
r_int
id|flags
)paren
(brace
r_const
r_char
op_star
id|argv_index_pack
(braket
)braket
op_assign
(brace
l_string|&quot;index-pack&quot;
comma
l_string|&quot;--fix-thin&quot;
comma
l_string|&quot;--stdin&quot;
comma
l_int|NULL
comma
l_int|NULL
)brace
suffix:semicolon
r_struct
id|child_process
id|ip
op_assign
id|CHILD_PROCESS_INIT
suffix:semicolon
r_if
c_cond
(paren
id|flags
op_amp
id|BUNDLE_VERBOSE
)paren
id|argv_index_pack
(braket
l_int|3
)braket
op_assign
l_string|&quot;-v&quot;
suffix:semicolon
r_if
c_cond
(paren
id|verify_bundle
c_func
(paren
id|header
comma
l_int|0
)paren
)paren
r_return
l_int|1
suffix:semicolon
id|ip.argv
op_assign
id|argv_index_pack
suffix:semicolon
id|ip.in
op_assign
id|bundle_fd
suffix:semicolon
id|ip.no_stdout
op_assign
l_int|1
suffix:semicolon
id|ip.git_cmd
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|run_command
c_func
(paren
op_amp
id|ip
)paren
)paren
r_return
id|error
c_func
(paren
id|_
c_func
(paren
l_string|&quot;index-pack died&quot;
)paren
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
eof
