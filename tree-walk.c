...
multiline_comment|/*&n; * This is Linux&squot;s built-in max for the number of symlinks to follow.&n; * That limit, of course, does not affect git, but it&squot;s a reasonable&n; * choice.&n; */
DECL|macro|GET_TREE_ENTRY_FOLLOW_SYMLINKS_MAX_LINKS
mdefine_line|#define GET_TREE_ENTRY_FOLLOW_SYMLINKS_MAX_LINKS 40
multiline_comment|/**&n; * Find a tree entry by following symlinks in tree_sha (which is&n; * assumed to be the root of the repository).  In the event that a&n; * symlink points outside the repository (e.g. a link to /foo or a&n; * root-level link to ../foo), the portion of the link which is&n; * outside the repository will be returned in result_path, and *mode&n; * will be set to 0.  It is assumed that result_path is uninitialized.&n; * If there are no symlinks, or the end result of the symlink chain&n; * points to an object inside the repository, result will be filled in&n; * with the sha1 of the found object, and *mode will hold the mode of&n; * the object.&n; *&n; * See the code for enum follow_symlink_result for a description of&n; * the return values.&n; */
...
suffix:semicolon
r_int
id|follows_remaining
op_assign
id|GET_TREE_ENTRY_FOLLOW_SYMLINKS_MAX_LINKS
suffix:semicolon
r_int
id|i
suffix:semicolon
...
multiline_comment|/* Handle symlinks to e.g. a//b by removing leading slashes */
r_while
c_loop
(paren
id|namebuf.buf
...
multiline_comment|/* Split namebuf into a first component and a remainder */
r_if
c_cond
(paren
(paren
...
id|parent
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * We could end up with .. in the namebuf if it&n;&t;&t;&t; * appears in a symlink.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|parents_nr
...
multiline_comment|/* We could end up here via a symlink to dir/.. */
r_if
c_cond
(paren
id|namebuf.buf
...
multiline_comment|/* Look up the first (or only) path component in the tree. */
id|find_result
op_assign
id|find_tree_entry
c_func
...
multiline_comment|/* Descend the tree */
id|t.buffer
op_assign
l_int|NULL
suffix:semicolon
...
(brace
multiline_comment|/* Follow a symlink */
r_int
r_int
id|link_len
suffix:semicolon
...
(brace
multiline_comment|/* Too many symlinks followed */
id|retval
op_assign
id|SYMLINK_LOOP
suffix:semicolon
...
multiline_comment|/*&n;&t;&t;&t; * At this point, we have followed at a least&n;&t;&t;&t; * one symlink, so on error we need to report this.&n;&t;&t;&t; */
id|retval
op_assign
id|DANGLING_SYMLINK
suffix:semicolon
...
multiline_comment|/*&n;&t;&t; * &quot;Never interesting&quot; trick requires exact&n;&t;&t; * matching. We could do something clever with inexact&n;&t;&t; * matching, but it&squot;s trickier (and not to forget that&n;&t;&t; * strcasecmp is locale-dependent, at least in&n;&t;&t; * glibc). Just disable it for now. It can&squot;t be worse&n;&t;&t; * than the wildcard&squot;s codepath of &squot;[Tt][Hi][Is][Ss]&squot;&n;&t;&t; * pattern.&n;&t;&t; */
op_star
id|never_interesting
op_assign
...
multiline_comment|/*&n;&t;&t; * match_entry does not check if the prefix part is&n;&t;&t; * matched case-sensitively. If the entry is a&n;&t;&t; * directory and part of prefix, it&squot;ll be rematched&n;&t;&t; * eventually by basecmp with special treatment for&n;&t;&t; * the prefix.&n;&t;&t; */
r_return
l_int|1
suffix:semicolon
r_return
...
multiline_comment|/* :(icase)-aware string compare */
...
suffix:semicolon
multiline_comment|/*&n;&t; * case | entry | positive | negative | result&n;&t; * -----+-------+----------+----------+-------&n;&t; *   1  |  file |   -1     |  -1..2   |  -1&n;&t; *   2  |  file |    0     |  -1..2   |   0&n;&t; *   3  |  file |    1     |   -1     |   1&n;&t; *   4  |  file |    1     |    0     |   1&n;&t; *   5  |  file |    1     |    1     |   0&n;&t; *   6  |  file |    1     |    2     |   0&n;&t; *   7  |  file |    2     |   -1     |   2&n;&t; *   8  |  file |    2     |    0     |   2&n;&t; *   9  |  file |    2     |    1     |   0&n;&t; *  10  |  file |    2     |    2     |  -1&n;&t; * -----+-------+----------+----------+-------&n;&t; *  11  |  dir  |   -1     |  -1..2   |  -1&n;&t; *  12  |  dir  |    0     |  -1..2   |   0&n;&t; *  13  |  dir  |    1     |   -1     |   1&n;&t; *  14  |  dir  |    1     |    0     |   1&n;&t; *  15  |  dir  |    1     |    1     |   1 (*)&n;&t; *  16  |  dir  |    1     |    2     |   0&n;&t; *  17  |  dir  |    2     |   -1     |   2&n;&t; *  18  |  dir  |    2     |    0     |   2&n;&t; *  19  |  dir  |    2     |    1     |   1 (*)&n;&t; *  20  |  dir  |    2     |    2     |  -1&n;&t; *&n;&t; * (*) An exclude pattern interested in a directory does not&n;&t; * necessarily mean it will exclude all of the directory. In&n;&t; * wildcard case, it can&squot;t decide until looking at individual&n;&t; * files inside. So don&squot;t write such directories off yet.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* #3, #4, #7, #8, #13, #14, #17, #18 */
r_if
c_cond
(paren
id|negative
