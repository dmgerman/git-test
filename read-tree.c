...
multiline_comment|/*&n; * When we find a &quot;stage2&quot; entry in the two-way merge, that&squot;s&n; * the one that will remain. If we have an exact old match,&n; * we don&squot;t care whether the file is up-to-date or not, we just&n; * re-use the thing directly.&n; *&n; * If we didn&squot;t have an exact match, then we want to make sure&n; * that we&squot;ve seen a stage1 that matched the old, and that the&n; * old file was up-to-date. Because it will be gone after this&n; * merge..&n; */
...
(brace
multiline_comment|/*&n;&t;&t; * This also removes the UPDATE flag on&n;&t;&t; * a match&n;&t;&t; */
r_if
c_cond
(paren
id|same
...
multiline_comment|/*&n;&t; * Unmatched with a new entry? Make sure it was&n;&t; * at least uptodate in the working directory _and_&n;&t; * the original tree..&n;&t; */
r_if
c_cond
(paren
id|old
