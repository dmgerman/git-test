...
suffix:semicolon
multiline_comment|/*&n; * To find a leaf_node:&n; * 1. Start at the root node, with n = 0&n; * 2. Use the nth nibble of the key as an index into a:&n; *    - If a[n] is an int_node, recurse into that node and increment n&n; *    - If a leaf_node with matching key, return leaf_node (assert note entry)&n; *    - If a matching subtree entry, unpack that subtree entry (and remove it);&n; *      restart search at the current level.&n; *    - Otherwise, we end up at a NULL pointer, or a non-matching leaf_node.&n; *      Backtrack out of the recursion, one level at a time and check a[0]:&n; *      - If a[0] at the current level is a matching subtree entry, unpack that&n; *        subtree entry (and remove it); restart search at the current level.&n; */
...
id|l
suffix:semicolon
multiline_comment|/* return note object matching given key */
r_break
suffix:semicolon
r_case
id|PTR_TYPE_SUBTREE
...
multiline_comment|/*&n;&t; * Did not find key at this (or any lower) level.&n;&t; * Check if there&squot;s a matching subtree entry in tree-&gt;a[0].&n;&t; * If so, unpack tree and resume search.&n;&t; */
id|p
op_assign
id|tree-&gt;a
(braket
...
multiline_comment|/*&n; * To insert a leaf_node:&n; * 1. Start at the root node, with n = 0&n; * 2. Use the nth nibble of the key as an index into a:&n; *    - If a[n] is NULL, store the tweaked pointer directly into a[n]&n; *    - If a[n] is an int_node, recurse into that node and increment n&n; *    - If a[n] is a leaf_node:&n; *      1. Check if they&squot;re equal, and handle that (abort? overwrite?)&n; *      2. Create a new int_node, and store both leaf_nodes there&n; *      3. Store the new int_node into a[n].&n; */
...
l_int|1
suffix:semicolon
multiline_comment|/* abort insert on matching key */
id|new_node
op_assign
(paren
r_struct
