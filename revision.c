...
r_static
r_int
id|show_path_component_truncated
c_func
(paren
id|FILE
op_star
...
(brace
multiline_comment|/*&n;&t;&t;&t; * A missing commit is ok iff its parent is marked&n;&t;&t;&t; * uninteresting.&n;&t;&t;&t; *&n;&t;&t;&t; * We just mark such a thing parsed, so that when&n;&t;&t;&t; * it is popped next time around, we won&squot;t be trying&n;&t;&t;&t; * to parse it and get an error.&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
r_static
r_void
id|add_pending_object_with_path
c_func
(paren
r_struct
id|rev_info
...
multiline_comment|/*&n; * A definition of &quot;relevant&quot; commit that we can use to simplify limited graphs&n; * by eliminating side branches.&n; *&n; * A &quot;relevant&quot; commit is one that is !UNINTERESTING (ie we are including it&n; * in our list), or that is a specified BOTTOM commit. Then after computing&n; * a limited list, during processing we can generally ignore boundary merges&n; * coming from outside the graph, (ie from irrelevant parents), and treat&n; * those merges as if they were single-parent. TREESAME is defined to consider&n; * only relevant parents, if any. If we are TREESAME to our on-graph parents,&n; * we don&squot;t care if we were !TREESAME to non-graph parents.&n; *&n; * Treating bottom commits as relevant ensures that a limited graph&squot;s&n; * connection to the actual bottom commit is not viewed as a side branch, but&n; * treated as part of the graph. For example:&n; *&n; *   ....Z...A---X---o---o---B&n; *        .     /&n; *         W---Y&n; *&n; * When computing &quot;A..B&quot;, the A-X connection is at least as important as&n; * Y-X, despite A being flagged UNINTERESTING.&n; *&n; * And when computing --ancestry-path &quot;A..B&quot;, the A-X connection is more&n; * important than Y-X, despite both A and Y being flagged UNINTERESTING.&n; */
...
multiline_comment|/*&n; * Return a single relevant commit from a parent list. If we are a TREESAME&n; * commit, and this selects one of our parents, then we can safely simplify to&n; * that parent.&n; */
DECL|function|one_relevant_parent
r_static
r_struct
id|commit
op_star
id|one_relevant_parent
c_func
(paren
r_const
r_struct
...
l_int|NULL
suffix:semicolon
multiline_comment|/*&n;&t; * For 1-parent commits, or if first-parent-only, then return that&n;&t; * first parent (even if not &quot;relevant&quot; by the above definition).&n;&t; * TREESAME will have been set purely on that parent.&n;&t; */
r_if
c_cond
(paren
...
id|orig-&gt;item
suffix:semicolon
multiline_comment|/*&n;&t; * For multi-parent commits, identify a sole relevant parent, if any.&n;&t; * If we have only one relevant parent, then TREESAME will be set purely&n;&t; * with regard to that parent, and we can simplify accordingly.&n;&t; *&n;&t; * If we have more than one relevant parent, or no relevant parents&n;&t; * (and multiple irrelevant ones), then we can&squot;t select a parent here&n;&t; * and return NULL.&n;&t; */
r_while
c_loop
(paren
id|list
...
multiline_comment|/*&n; * Must be called immediately after removing the nth_parent from a commit&squot;s&n; * parent list, if we are maintaining the per-parent treesame[] decoration.&n; * This does not recalculate the master TREESAME flag - update_treesame()&n; * should be called to update it after a sequence of treesame[] modifications&n; * that may have affected it.&n; */
...
(brace
multiline_comment|/*&n;&t;&t; * Have just removed the only parent from a non-merge.&n;&t;&t; * Different handling, as we lack decoration.&n;&t;&t; */
r_if
c_cond
(paren
id|nth_parent
...
suffix:semicolon
multiline_comment|/*&n;&t; * If we&squot;ve just become a non-merge commit, update TREESAME&n;&t; * immediately, and remove the no-longer-needed decoration.&n;&t; * If still a merge, defer update until update_treesame().&n;&t; */
r_if
c_cond
(paren
op_decrement
...
c_func
(paren
l_string|&quot;compact_treesame parents mismatch&quot;
)paren
suffix:semicolon
r_if
c_cond
...
r_inline
r_int
id|limiting_can_increase_treesame
c_func
(paren
r_const
r_struct
...
(brace
multiline_comment|/*&n;&t; * TREESAME is irrelevant unless prune &amp;&amp; dense;&n;&t; * if simplify_history is set, we can&squot;t have a mixture of TREESAME and&n;&t; *    !TREESAME INTERESTING parents (and we don&squot;t have treesame[]&n;&t; *    decoration anyway);&n;&t; * if first_parent_only is set, then the TREESAME flag is locked&n;&t; *    against the first parent (and again we lack treesame[] decoration).&n;&t; */
r_return
id|revs-&gt;prune
op_logical_and
id|revs-&gt;dense
...
(brace
multiline_comment|/*&n;&t;&t;&t; * This our second loop iteration - so we now know&n;&t;&t;&t; * we&squot;re dealing with a merge.&n;&t;&t;&t; *&n;&t;&t;&t; * Do not compare with later parents when we care only about&n;&t;&t;&t; * the first parent chain, in order to avoid derailing the&n;&t;&t;&t; * traversal to follow a side branch that brought everything&n;&t;&t;&t; * in the path we are limited to by the pathspec.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|revs-&gt;first_parent_only
...
r_break
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * If this will remain a potentially-simplifiable&n;&t;&t;&t; * merge, remember per-parent treesame if needed.&n;&t;&t;&t; * Initialise the array with the comparison from our&n;&t;&t;&t; * first iteration.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|revs-&gt;treesame.name
op_logical_and
op_logical_neg
id|revs-&gt;simplify_history
op_logical_and
...
multiline_comment|/*&n;&t; * TREESAME is straightforward for single-parent commits. For merge&n;&t; * commits, it is most useful to define it so that &quot;irrelevant&quot;&n;&t; * parents cannot make us !TREESAME - if we have any relevant&n;&t; * parents, then we only consider TREESAMEness with respect to them,&n;&t; * allowing irrelevant merges from uninteresting branches to be&n;&t; * simplified away. Only if we have only irrelevant parents do we&n;&t; * base TREESAME on them. Note that this logic is replicated in&n;&t; * update_treesame, which should be kept in sync.&n;&t; */
r_if
c_cond
(paren
id|relevant_parents
...
multiline_comment|/*&n;&t; * Check if any commits have become TREESAME by some of their parents&n;&t; * becoming UNINTERESTING.&n;&t; */
r_if
c_cond
(paren
id|limiting_can_increase_treesame
...
id|cb-&gt;all_flags
suffix:semicolon
multiline_comment|/* ??? CMDLINEFLAGS ??? */
id|add_pending_object
c_func
(paren
id|cb-&gt;all_revs
...
c_func
(paren
l_string|&quot;unable to add index blob to traversal&quot;
)paren
suffix:semicolon
id|add_pending_object_with_path
c_func
...
(brace
multiline_comment|/*&n;&t;&t;&t; * Just &quot;..&quot;?  That is not a range but the&n;&t;&t;&t; * pathspec for the parent directory.&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
ques
c_cond
l_string|&quot;Invalid symmetric difference expression %s&quot;
suffix:colon
l_string|&quot;Invalid revision range %s&quot;
comma
id|arg
)paren
suffix:semicolon
...
r_else
(brace
multiline_comment|/* A...B -- find merge bases between the two */
r_struct
id|commit
op_star
id|a
...
id|arg
comma
l_string|&quot;--show-signature&quot;
)paren
)paren
(brace
id|revs-&gt;show_signature
op_assign
l_int|1
suffix:semicolon
)brace
...
l_string|&quot;                    ..........&quot;
suffix:semicolon
id|revs-&gt;track_linear
op_assign
l_int|1
suffix:semicolon
id|revs-&gt;track_first_time
op_assign
l_int|1
suffix:semicolon
)brace
...
(brace
id|grep_set_pattern_type_option
c_func
(paren
...
c_func
(paren
l_string|&quot;--show-linear-break and --graph are incompatible&quot;
)paren
suffix:semicolon
r_return
l_int|1
...
(brace
multiline_comment|/*&n;&t;&t; * Detached form (&quot;--no-walk X&quot; as opposed to &quot;--no-walk=X&quot;)&n;&t;&t; * not allowed, since the argument is optional.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
c_func
(paren
l_string|&quot;invalid argument to --no-walk&quot;
)paren
suffix:semicolon
)brace
r_else
...
c_func
(paren
l_string|&quot;your current branch appears to be broken&quot;
)paren
)paren
suffix:semicolon
id|skip_prefix
...
c_func
(paren
l_string|&quot;your current branch &squot;%s&squot; does not have any commits yet&quot;
)paren
comma
id|refname
)paren
...
suffix:semicolon
id|grep_commit_pattern_type
c_func
(paren
id|GREP_PATTERN_TYPE_UNSPECIFIED
comma
op_amp
id|revs-&gt;grep_filter
)paren
...
multiline_comment|/* clear the temporary mark */
r_for
c_loop
(paren
id|p
...
multiline_comment|/* no update_treesame() - removing duplicates can&squot;t affect TREESAME */
r_return
id|surviving_parents
suffix:semicolon
)brace
...
id|pn
suffix:semicolon
multiline_comment|/* Want these for sanity-checking only */
r_int
id|orig_cnt
op_assign
id|commit_list_count
...
suffix:semicolon
multiline_comment|/*&n;&t; * Not ready to remove items yet, just mark them for now, based&n;&t; * on the output of reduce_heads(). reduce_heads outputs the reduced&n;&t; * set in its original order, so this isn&squot;t too hard.&n;&t; */
id|po
op_assign
id|commit-&gt;parents
suffix:semicolon
...
c_func
(paren
l_string|&quot;mark_redundant_parents %d %d %d %d&quot;
comma
id|orig_cnt
comma
id|cnt
...
multiline_comment|/*&n; * Awkward naming - this means one parent we are TREESAME to.&n; * cf mark_treesame_root_parents: root parents that are TREESAME (to an&n; * empty tree). Better name suggestions?&n; */
DECL|function|leave_one_treesame_to_parent
r_static
r_int
id|leave_one_treesame_to_parent
c_func
(paren
r_struct
id|rev_info
...
multiline_comment|/*&n;&t; * If we are TREESAME to a marked-for-deletion parent, but not to any&n;&t; * unmarked parents, unmark the first TREESAME parent. This is the&n;&t; * parent that the default simplify_history==1 scan would have followed,&n;&t; * and it doesn&squot;t make sense to omit that path when asking for a&n;&t; * simplified full history. Retaining it improves the chances of&n;&t; * understanding odd missed merges that took an old version of a file.&n;&t; *&n;&t; * Example:&n;&t; *&n;&t; *   I--------*X       A modified the file, but mainline merge X used&n;&t; *    &bslash;       /        &quot;-s ours&quot;, so took the version from I. X is&n;&t; *     `-*A--&squot;         TREESAME to I and !TREESAME to A.&n;&t; *&n;&t; * Default log from X would produce &quot;I&quot;. Without this check,&n;&t; * --full-history --simplify-merges would produce &quot;I-A-X&quot;, showing&n;&t; * the merge commit X and that it changed A, but not making clear that&n;&t; * it had just taken the I version. With this check, the topology above&n;&t; * is retained.&n;&t; *&n;&t; * Note that it is possible that the simplification chooses a different&n;&t; * TREESAME parent from the default, in which case this test doesn&squot;t&n;&t; * activate, and we _do_ drop the default parent. Example:&n;&t; *&n;&t; *   I------X         A modified the file, but it was reverted in B,&n;&t; *    &bslash;    /          meaning mainline merge X is TREESAME to both&n;&t; *    *A-*B           parents.&n;&t; *&n;&t; * Default log would produce &quot;I&quot; by following the first parent;&n;&t; * --full-history --simplify-merges will produce &quot;I-A-B&quot;. But this is a&n;&t; * reasonable result - it presents a logical full history leading from&n;&t; * I to X, and X is not an important merge.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* Removing parents can only increase TREESAMEness */
r_if
c_cond
(paren
id|removed
...
multiline_comment|/*&n;&t; * Rewrite our list of parents. Note that this cannot&n;&t; * affect our TREESAME flags in any way - a commit is&n;&t; * always TREESAME to its simplification.&n;&t; */
r_for
c_loop
(paren
id|p
...
suffix:semicolon
multiline_comment|/*&n;&t; * It is possible that we are a merge and one side branch&n;&t; * does not have any commit that touches the given paths;&n;&t; * in such a case, the immediate parent from that branch&n;&t; * will be rewritten to be the merge base.&n;&t; *&n;&t; *      o----X&t;&t;X: the commit we are looking at;&n;&t; *     /    /&t;&t;o: a commit that touches the paths;&n;&t; * ---o----&squot;&n;&t; *&n;&t; * Further, a merge of an independent branch that doesn&squot;t&n;&t; * touch the path will reduce to a treesame root parent:&n;&t; *&n;&t; *  ----o----X&t;&t;X: the commit we are looking at;&n;&t; *          /&t;&t;o: a commit that touches the paths;&n;&t; *         r&t;&t;r: a root commit not touching the paths&n;&t; *&n;&t; * Detect and simplify both cases.&n;&t; */
r_if
c_cond
(paren
l_int|1
...
id|list-&gt;next
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Do not free(list) here yet; the original list&n;&t;&t; * is used later in this function.&n;&t;&t; */
id|commit_list_insert
c_func
(paren
id|commit
...
suffix:semicolon
multiline_comment|/* Signal whether we need per-parent treesame decoration */
r_if
c_cond
(paren
id|revs-&gt;simplify_merges
...
multiline_comment|/* Append &quot;fake&quot; message parts as needed */
r_if
c_cond
(paren
id|opt-&gt;show_notes
...
id|commit_ignore
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * If we want ancestry, then need to keep any merges&n;&t;&t;&t; * between relevant commits to tie together topology.&n;&t;&t;&t; * For consistency with TREESAME and simplification&n;&t;&t;&t; * use &quot;relevant&quot; here rather than just INTERESTING,&n;&t;&t;&t; * to treat bottom commit(s) as part of the topology.&n;&t;&t;&t; */
r_for
c_loop
(paren
id|n
...
mdefine_line|#define EMPTY_PARENT_LIST ((struct commit_list *)-1)
multiline_comment|/*&n; * You may only call save_parents() once per commit (this is checked&n; * for non-root commits).&n; */
...
suffix:semicolon
multiline_comment|/*&n;&t; * When walking with reflogs, we may visit the same commit&n;&t; * several times: once for each appearance in the reflog.&n;&t; *&n;&t; * In this case, save_parents() will be called multiple times.&n;&t; * We want to keep only the first set of parents.  We need to&n;&t; * store a sentinel value for an empty (i.e., NULL) parent&n;&t; * list to distinguish it from a not-yet-saved list, however.&n;&t; */
r_if
c_cond
(paren
op_star
...
id|pp
op_assign
id|EMPTY_PARENT_LIST
suffix:semicolon
)brace
DECL|function|free_saved_parents
r_static
r_void
id|free_saved_parents
c_func
(paren
r_struct
id|rev_info
...
(brace
multiline_comment|/*&n;&t;&t; * --full-diff on simplified parents is no good: it&n;&t;&t; * will show spurious changes from the commits that&n;&t;&t; * were elided.  So we save the parents on the side&n;&t;&t; * when --full-diff is in effect.&n;&t;&t; */
r_if
c_cond
(paren
id|revs-&gt;full_diff
...
multiline_comment|/*&n; * Return true for entries that have not yet been shown.  (This is an&n; * object_array_each_func_t.)&n; */
...
multiline_comment|/*&n; * If array is on the verge of a realloc, garbage-collect any entries&n; * that have already been shown to try to free up some space.&n; */
