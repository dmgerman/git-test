...
multiline_comment|/*&n; * Basically keep a cache of X-&gt;Y so that we can repeatedly replace&n; * the same anonymized string with another. The actual generation&n; * is farmed out to the generate function.&n; */
...
multiline_comment|/*&n; * We anonymize each component of a path individually,&n; * so that paths a/b and a/c will share a common root.&n; * The paths are cached via anonymize_mem so that repeated&n; * lookups for &quot;a&quot; will yield the same value.&n; */
...
multiline_comment|/*&n; * Ideally we would want some transformation of the blob data here&n; * that is unreversible, but would still be the same size and have&n; * the same data relationship to other blobs (so that we get the same&n; * delta and packing behavior as the original). But the first and last&n; * requirements there are probably mutually exclusive, so let&squot;s take&n; * the easy way out for now, and just generate arbitrary content.&n; *&n; * There&squot;s no need to cache this result with anonymize_mem, since&n; * we already handle blob content caching with marks.&n; */
...
c_func
(paren
l_string|&quot;sha1 mismatch in blob %s&quot;
comma
id|sha1_to_hex
c_func
(paren
...
l_int|1
suffix:semicolon
multiline_comment|/* avoid null sha1 */
r_int
r_char
op_star
id|out
...
(brace
multiline_comment|/*&n;&t; * If any of these prefixes is found, we will leave it intact&n;&t; * so that tags remain tags and so forth.&n;&t; */
r_static
r_const
r_char
op_star
...
id|i
suffix:semicolon
multiline_comment|/*&n;&t; * We also leave &quot;master&quot; as a special case, since it does not reveal&n;&t; * anything interesting.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*&n; * We do not even bother to cache commit messages, as they are unlikely&n; * to be repeated verbatim, and it is not that interesting when they are.&n; */
DECL|function|anonymize_commit_message
r_static
r_char
op_star
id|anonymize_commit_message
c_func
(paren
r_const
r_char
...
c_func
(paren
l_string|&quot;subject %d&bslash;n&bslash;nbody&bslash;n&quot;
comma
id|counter
op_increment
)paren
...
id|out
comma
l_string|&quot;User %d &lt;user%d@example.com&gt;&quot;
comma
id|counter
comma
id|counter
...
multiline_comment|/*&n; * Our strategy here is to anonymize the names and email addresses,&n; * but keep timestamps intact, as they influence things like traversal&n; * order (and by themselves should not be too revealing).&n; */
DECL|function|anonymize_ident_line
r_static
r_void
id|anonymize_ident_line
c_func
(paren
r_const
r_char
...
suffix:semicolon
multiline_comment|/* skip &quot;committer&quot;, &quot;author&quot;, &quot;tagger&quot;, etc */
id|end_of_header
op_assign
id|strchr
c_func
...
c_func
(paren
l_string|&quot;BUG: malformed line fed to anonymize_ident_line: %.*s&quot;
comma
(paren
r_int
)paren
...
id|out
comma
l_string|&quot;Malformed Ident &lt;malformed@example.com&gt; 0 -0000&quot;
)paren
suffix:semicolon
)brace
op_star
...
c_func
(paren
id|commit-&gt;parents-&gt;item-&gt;tree-&gt;object.oid.hash
comma
...
c_func
(paren
l_string|&quot;not a commit? can&squot;t happen: %s&quot;
comma
id|sha1_to_hex
c_func
(paren
...
c_func
(paren
l_string|&quot;reset %s&bslash;nfrom %s&bslash;n&bslash;n&quot;
comma
id|refspec-&gt;dst
comma
id|sha1_to_hex
...
c_func
(paren
l_string|&quot;Apply refspec to exported refs&quot;
)paren
)paren
comma
id|OPT_BOOL
