...
multiline_comment|/*&n; * include/exclude&n; */
...
c_func
(paren
l_string|&quot;BUG: caller miscounted postlen: asked %d, orig = %d, used = %d&quot;
comma
(paren
r_int
)paren
...
multiline_comment|/*&n;&t; * The preimage may extend beyond the end of the file,&n;&t; * but in this loop we will only handle the part of the&n;&t; * preimage that falls within the file.&n;&t; */
id|strbuf_init
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* Add the length if this is common with the postimage */
r_if
c_cond
(paren
id|preimage-&gt;line
...
c_func
(paren
l_string|&quot;reading from &squot;%s&squot; beyond a symbolic link&quot;
)paren
comma
id|name
)paren
...
multiline_comment|/*&n; * We need to keep track of how symlinks in the preimage are&n; * manipulated by the patches.  A patch to add a/b/c where a/b&n; * is a symlink should not be allowed to affect the directory&n; * the symlink points at, but if the same patch removes a/b,&n; * it is perfectly fine, as the patch removes a/b to make room&n; * to create a directory a/b so that a/b/c can be created.&n; */
...
mdefine_line|#define SYMLINK_GOES_AWAY 01
DECL|macro|SYMLINK_IN_RESULT
mdefine_line|#define SYMLINK_IN_RESULT 02
DECL|function|register_symlink_changes
r_static
r_uintptr
id|register_symlink_changes
c_func
(paren
r_const
r_char
...
multiline_comment|/* the symlink at patch-&gt;old_name is removed */
id|register_symlink_changes
c_func
(paren
id|patch-&gt;old_name
comma
id|SYMLINK_GOES_AWAY
)paren
suffix:semicolon
r_if
c_cond
...
multiline_comment|/* the symlink at patch-&gt;new_name is created or remains */
id|register_symlink_changes
c_func
(paren
id|patch-&gt;new_name
comma
id|SYMLINK_IN_RESULT
)paren
suffix:semicolon
)brace
)brace
...
suffix:semicolon
multiline_comment|/* scan backwards */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*&n;&t;&t;&t; * This cannot be &quot;return 0&quot;, because we may&n;&t;&t;&t; * see a new one created at a higher level.&n;&t;&t;&t; */
r_continue
suffix:semicolon
multiline_comment|/* otherwise, check the preimage */
r_if
c_cond
(paren
id|check_index
...
suffix:semicolon
multiline_comment|/*&n;&t; * An attempt to read from or delete a path that is beyond a&n;&t; * symbolic link will be prevented by load_patch_target() that&n;&t; * is called at the beginning of apply_data() so we do not&n;&t; * have to worry about a patch marked with &quot;is_delete&quot; bit&n;&t; * here.  We however need to make sure that the patch result&n;&t; * is not deposited to a path that is beyond a symbolic link&n;&t; * here.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
c_func
(paren
l_string|&quot;affected file &squot;%s&squot; is beyond a symbolic link&quot;
)paren
comma
id|patch-&gt;new_name
)paren
...
c_func
(paren
l_string|&quot;accept a patch that touches outside the working area&quot;
)paren
)paren
comma
id|OPT_BOOL
