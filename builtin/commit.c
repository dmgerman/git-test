...
r_const
r_char
id|implicit_ident_advice_noconfig
(braket
)braket
op_assign
id|N_
...
l_string|&quot;You can suppress this message by setting them explicitly. Run the&bslash;n&quot;
l_string|&quot;following command and follow the instructions in your editor to edit&bslash;n&quot;
l_string|&quot;your configuration file:&bslash;n&quot;
l_string|&quot;&bslash;n&quot;
l_string|&quot;    git config --global --edit&bslash;n&quot;
l_string|&quot;&bslash;n&quot;
...
r_const
r_char
id|implicit_ident_advice_config
(braket
)braket
op_assign
id|N_
...
r_const
r_char
id|empty_cherry_pick_advice_single
(braket
)braket
op_assign
id|N_
...
r_const
r_char
id|empty_cherry_pick_advice_multi
(braket
)braket
op_assign
id|N_
c_func
(paren
l_string|&quot;If you wish to skip this commit, use:&bslash;n&quot;
l_string|&quot;&bslash;n&quot;
l_string|&quot;    git reset&bslash;n&quot;
l_string|&quot;&bslash;n&quot;
l_string|&quot;Then &bslash;&quot;git cherry-pick --continue&bslash;&quot; will resume cherry-picking&bslash;n&quot;
l_string|&quot;the remaining commits.&bslash;n&quot;
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;unable to update temporary index&quot;
)paren
)paren
suffix:semicolon
)brace
...
c_func
(paren
l_string|&quot;Failed to update main cache tree&quot;
)paren
)paren
suffix:semicolon
id|commit_style
...
c_func
(paren
l_string|&quot;BUG: finalize_deferred_config() should have been called&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
...
l_int|0
suffix:semicolon
multiline_comment|/* no usable mail */
r_if
c_cond
(paren
op_logical_neg
id|person-&gt;date_begin
op_logical_or
op_logical_neg
id|person-&gt;date_end
op_logical_or
...
c_func
(paren
l_string|&quot;unable to select a comment character that is not used&bslash;n&quot;
l_string|&quot;in the current commit message&quot;
)paren
)paren
suffix:semicolon
id|comment_line_char
...
multiline_comment|/*&n; * Inspect sb and determine the true &quot;end&quot; of the log message, in&n; * order to find where to put a new Signed-off-by: line.  Ignored are&n; * trailing comment lines and blank lines, and also the traditional&n; * &quot;Conflicts:&quot; block that is not commented out, so that we can use&n; * &quot;git commit -s --amend&quot; on an existing commit that forgot to remove&n; * it.&n; *&n; * Returns the number of bytes from the tail to ignore, to be fed as&n; * the second parameter to append_signoff().&n; */
...
(brace
multiline_comment|/* is this the first of the run of comments? */
r_if
c_cond
(paren
op_logical_neg
...
id|bol
suffix:semicolon
multiline_comment|/* otherwise, it is just continuing */
)brace
r_else
r_if
c_cond
...
(brace
suffix:semicolon
multiline_comment|/* a pathname in the conflicts block */
)brace
r_else
r_if
c_cond
...
(brace
multiline_comment|/* the previous was not trailing comment */
id|boc
op_assign
l_int|0
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* Ignore status.displayCommentPrefix: we do need comments in COMMIT_EDITMSG. */
id|old_display_comment_prefix
op_assign
...
r_else
(brace
multiline_comment|/*&n;&t;&t;&t; * Unless the user did explicitly request a submodule&n;&t;&t;&t; * ignore mode by passing a command line option we do&n;&t;&t;&t; * not ignore any changed submodule SHA-1s when&n;&t;&t;&t; * comparing index and parent, no matter what is&n;&t;&t;&t; * configured. Otherwise we won&squot;t commit any&n;&t;&t;&t; * submodules which were manually staged, which would&n;&t;&t;&t; * be really confusing.&n;&t;&t;&t; */
r_int
id|diff_flags
op_assign
id|DIFF_OPT_OVERRIDE_SUBMODULE_CONFIG
suffix:semicolon
r_if
c_cond
(paren
...
multiline_comment|/*&n; * See if the user edited the message in the editor or left what&n; * was in the template intact&n; */
DECL|function|template_untouched
r_static
r_int
id|template_untouched
c_func
(paren
r_struct
id|strbuf
...
multiline_comment|/*&n; * Enumerate what needs to be propagated when --porcelain&n; * is not in effect here.&n; */
DECL|struct|status_deferred_config
r_static
r_struct
id|status_deferred_config
(brace
...
c_func
(paren
l_string|&quot;--long and -z are incompatible&quot;
)paren
)paren
suffix:semicolon
)brace
...
id|status_format
op_assign
id|status_deferred_config.status_format
suffix:semicolon
r_if
c_cond
(paren
...
id|s-&gt;show_branch
op_assign
id|status_deferred_config.show_branch
suffix:semicolon
r_if
c_cond
(paren
...
id|k
comma
l_string|&quot;status.displaycommentprefix&quot;
)paren
)paren
(brace
id|s-&gt;display_comment_prefix
...
c_func
(paren
l_string|&quot;show status in long format (default)&quot;
)paren
comma
id|STATUS_FORMAT_LONG
)paren
...
id|index
suffix:semicolon
multiline_comment|/*&n;&t; * Let the hook know that no editor will be launched.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
c_func
(paren
l_string|&quot;Aborting commit; you did not edit the message.&bslash;n&quot;
)paren
)paren
suffix:semicolon
m_exit
