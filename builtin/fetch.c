...
multiline_comment|/*&n; * The ref_map records the tips of the refs we are fetching. If&n; *&n; *  $ git rev-list --verify-objects --stdin --not --all&n; *&n; * (feeding all the refs in ref_map on its standard input) does not&n; * error out, that means everything reachable from these updated refs&n; * locally exists and is connected to some of our existing refs.&n; *&n; * Returns 0 if everything is connected, non-zero otherwise.&n; */
