multiline_comment|/*&n; * GIT - The information manager from hell&n; *&n; * Copyright (C) Linus Torvalds, 2005&n; */
macro_line|#include &quot;cache.h&quot;
macro_line|#include &quot;lockfile.h&quot;
macro_line|#include &quot;quote.h&quot;
macro_line|#include &quot;cache-tree.h&quot;
macro_line|#include &quot;tree-walk.h&quot;
macro_line|#include &quot;builtin.h&quot;
macro_line|#include &quot;refs.h&quot;
macro_line|#include &quot;resolve-undo.h&quot;
macro_line|#include &quot;parse-options.h&quot;
macro_line|#include &quot;pathspec.h&quot;
macro_line|#include &quot;dir.h&quot;
macro_line|#include &quot;split-index.h&quot;
multiline_comment|/*&n; * Default to not allowing changes to the list of files. The&n; * tool doesn&squot;t actually care, but this makes it harder to add&n; * files to the revision control by mistake by doing something&n; * like &quot;git update-index *&quot; and suddenly having all the object&n; * files be revision controlled.&n; */
DECL|variable|allow_add
r_static
r_int
id|allow_add
suffix:semicolon
DECL|variable|allow_remove
r_static
r_int
id|allow_remove
suffix:semicolon
DECL|variable|allow_replace
r_static
r_int
id|allow_replace
suffix:semicolon
DECL|variable|info_only
r_static
r_int
id|info_only
suffix:semicolon
DECL|variable|force_remove
r_static
r_int
id|force_remove
suffix:semicolon
DECL|variable|verbose
r_static
r_int
id|verbose
suffix:semicolon
DECL|variable|mark_valid_only
r_static
r_int
id|mark_valid_only
suffix:semicolon
DECL|variable|mark_skip_worktree_only
r_static
r_int
id|mark_skip_worktree_only
suffix:semicolon
DECL|macro|MARK_FLAG
mdefine_line|#define MARK_FLAG 1
DECL|macro|UNMARK_FLAG
mdefine_line|#define UNMARK_FLAG 2
DECL|variable|mtime_dir
r_static
r_struct
id|strbuf
id|mtime_dir
op_assign
id|STRBUF_INIT
suffix:semicolon
id|__attribute__
c_func
(paren
(paren
id|format
(paren
id|printf
comma
l_int|1
comma
l_int|2
)paren
)paren
)paren
DECL|function|report
r_static
r_void
id|report
c_func
(paren
r_const
r_char
op_star
id|fmt
comma
dot
dot
dot
)paren
(brace
id|va_list
id|vp
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|verbose
)paren
r_return
suffix:semicolon
id|va_start
c_func
(paren
id|vp
comma
id|fmt
)paren
suffix:semicolon
id|vprintf
c_func
(paren
id|fmt
comma
id|vp
)paren
suffix:semicolon
id|putchar
c_func
(paren
l_char|&squot;&bslash;n&squot;
)paren
suffix:semicolon
id|va_end
c_func
(paren
id|vp
)paren
suffix:semicolon
)brace
DECL|function|remove_test_directory
r_static
r_void
id|remove_test_directory
c_func
(paren
r_void
)paren
(brace
r_if
c_cond
(paren
id|mtime_dir.len
)paren
id|remove_dir_recursively
c_func
(paren
op_amp
id|mtime_dir
comma
l_int|0
)paren
suffix:semicolon
)brace
DECL|function|get_mtime_path
r_static
r_const
r_char
op_star
id|get_mtime_path
c_func
(paren
r_const
r_char
op_star
id|path
)paren
(brace
r_static
r_struct
id|strbuf
id|sb
op_assign
id|STRBUF_INIT
suffix:semicolon
id|strbuf_reset
c_func
(paren
op_amp
id|sb
)paren
suffix:semicolon
id|strbuf_addf
c_func
(paren
op_amp
id|sb
comma
l_string|&quot;%s/%s&quot;
comma
id|mtime_dir.buf
comma
id|path
)paren
suffix:semicolon
r_return
id|sb.buf
suffix:semicolon
)brace
DECL|function|xmkdir
r_static
r_void
id|xmkdir
c_func
(paren
r_const
r_char
op_star
id|path
)paren
(brace
id|path
op_assign
id|get_mtime_path
c_func
(paren
id|path
)paren
suffix:semicolon
r_if
c_cond
(paren
id|mkdir
c_func
(paren
id|path
comma
l_int|0700
)paren
)paren
id|die_errno
c_func
(paren
id|_
c_func
(paren
l_string|&quot;failed to create directory %s&quot;
)paren
comma
id|path
)paren
suffix:semicolon
)brace
DECL|function|xstat_mtime_dir
r_static
r_int
id|xstat_mtime_dir
c_func
(paren
r_struct
id|stat
op_star
id|st
)paren
(brace
r_if
c_cond
(paren
id|stat
c_func
(paren
id|mtime_dir.buf
comma
id|st
)paren
)paren
id|die_errno
c_func
(paren
id|_
c_func
(paren
l_string|&quot;failed to stat %s&quot;
)paren
comma
id|mtime_dir.buf
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|create_file
r_static
r_int
id|create_file
c_func
(paren
r_const
r_char
op_star
id|path
)paren
(brace
r_int
id|fd
suffix:semicolon
id|path
op_assign
id|get_mtime_path
c_func
(paren
id|path
)paren
suffix:semicolon
id|fd
op_assign
id|open
c_func
(paren
id|path
comma
id|O_CREAT
op_or
id|O_RDWR
comma
l_int|0644
)paren
suffix:semicolon
r_if
c_cond
(paren
id|fd
OL
l_int|0
)paren
id|die_errno
c_func
(paren
id|_
c_func
(paren
l_string|&quot;failed to create file %s&quot;
)paren
comma
id|path
)paren
suffix:semicolon
r_return
id|fd
suffix:semicolon
)brace
DECL|function|xunlink
r_static
r_void
id|xunlink
c_func
(paren
r_const
r_char
op_star
id|path
)paren
(brace
id|path
op_assign
id|get_mtime_path
c_func
(paren
id|path
)paren
suffix:semicolon
r_if
c_cond
(paren
id|unlink
c_func
(paren
id|path
)paren
)paren
id|die_errno
c_func
(paren
id|_
c_func
(paren
l_string|&quot;failed to delete file %s&quot;
)paren
comma
id|path
)paren
suffix:semicolon
)brace
DECL|function|xrmdir
r_static
r_void
id|xrmdir
c_func
(paren
r_const
r_char
op_star
id|path
)paren
(brace
id|path
op_assign
id|get_mtime_path
c_func
(paren
id|path
)paren
suffix:semicolon
r_if
c_cond
(paren
id|rmdir
c_func
(paren
id|path
)paren
)paren
id|die_errno
c_func
(paren
id|_
c_func
(paren
l_string|&quot;failed to delete directory %s&quot;
)paren
comma
id|path
)paren
suffix:semicolon
)brace
DECL|function|avoid_racy
r_static
r_void
id|avoid_racy
c_func
(paren
r_void
)paren
(brace
multiline_comment|/*&n;&t; * not use if we could usleep(10) if USE_NSEC is defined. The&n;&t; * field nsec could be there, but the OS could choose to&n;&t; * ignore it?&n;&t; */
id|sleep
c_func
(paren
l_int|1
)paren
suffix:semicolon
)brace
DECL|function|test_if_untracked_cache_is_supported
r_static
r_int
id|test_if_untracked_cache_is_supported
c_func
(paren
r_void
)paren
(brace
r_struct
id|stat
id|st
suffix:semicolon
r_struct
id|stat_data
id|base
suffix:semicolon
r_int
id|fd
comma
id|ret
op_assign
l_int|0
suffix:semicolon
id|strbuf_addstr
c_func
(paren
op_amp
id|mtime_dir
comma
l_string|&quot;mtime-test-XXXXXX&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|mkdtemp
c_func
(paren
id|mtime_dir.buf
)paren
)paren
id|die_errno
c_func
(paren
l_string|&quot;Could not make temporary directory&quot;
)paren
suffix:semicolon
id|fprintf
c_func
(paren
id|stderr
comma
id|_
c_func
(paren
l_string|&quot;Testing &quot;
)paren
)paren
suffix:semicolon
id|atexit
c_func
(paren
id|remove_test_directory
)paren
suffix:semicolon
id|xstat_mtime_dir
c_func
(paren
op_amp
id|st
)paren
suffix:semicolon
id|fill_stat_data
c_func
(paren
op_amp
id|base
comma
op_amp
id|st
)paren
suffix:semicolon
id|fputc
c_func
(paren
l_char|&squot;.&squot;
comma
id|stderr
)paren
suffix:semicolon
id|avoid_racy
c_func
(paren
)paren
suffix:semicolon
id|fd
op_assign
id|create_file
c_func
(paren
l_string|&quot;newfile&quot;
)paren
suffix:semicolon
id|xstat_mtime_dir
c_func
(paren
op_amp
id|st
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|match_stat_data
c_func
(paren
op_amp
id|base
comma
op_amp
id|st
)paren
)paren
(brace
id|close
c_func
(paren
id|fd
)paren
suffix:semicolon
id|fputc
c_func
(paren
l_char|&squot;&bslash;n&squot;
comma
id|stderr
)paren
suffix:semicolon
id|fprintf_ln
c_func
(paren
id|stderr
comma
id|_
c_func
(paren
l_string|&quot;directory stat info does not &quot;
l_string|&quot;change after adding a new file&quot;
)paren
)paren
suffix:semicolon
r_goto
id|done
suffix:semicolon
)brace
id|fill_stat_data
c_func
(paren
op_amp
id|base
comma
op_amp
id|st
)paren
suffix:semicolon
id|fputc
c_func
(paren
l_char|&squot;.&squot;
comma
id|stderr
)paren
suffix:semicolon
id|avoid_racy
c_func
(paren
)paren
suffix:semicolon
id|xmkdir
c_func
(paren
l_string|&quot;new-dir&quot;
)paren
suffix:semicolon
id|xstat_mtime_dir
c_func
(paren
op_amp
id|st
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|match_stat_data
c_func
(paren
op_amp
id|base
comma
op_amp
id|st
)paren
)paren
(brace
id|close
c_func
(paren
id|fd
)paren
suffix:semicolon
id|fputc
c_func
(paren
l_char|&squot;&bslash;n&squot;
comma
id|stderr
)paren
suffix:semicolon
id|fprintf_ln
c_func
(paren
id|stderr
comma
id|_
c_func
(paren
l_string|&quot;directory stat info does not change &quot;
l_string|&quot;after adding a new directory&quot;
)paren
)paren
suffix:semicolon
r_goto
id|done
suffix:semicolon
)brace
id|fill_stat_data
c_func
(paren
op_amp
id|base
comma
op_amp
id|st
)paren
suffix:semicolon
id|fputc
c_func
(paren
l_char|&squot;.&squot;
comma
id|stderr
)paren
suffix:semicolon
id|avoid_racy
c_func
(paren
)paren
suffix:semicolon
id|write_or_die
c_func
(paren
id|fd
comma
l_string|&quot;data&quot;
comma
l_int|4
)paren
suffix:semicolon
id|close
c_func
(paren
id|fd
)paren
suffix:semicolon
id|xstat_mtime_dir
c_func
(paren
op_amp
id|st
)paren
suffix:semicolon
r_if
c_cond
(paren
id|match_stat_data
c_func
(paren
op_amp
id|base
comma
op_amp
id|st
)paren
)paren
(brace
id|fputc
c_func
(paren
l_char|&squot;&bslash;n&squot;
comma
id|stderr
)paren
suffix:semicolon
id|fprintf_ln
c_func
(paren
id|stderr
comma
id|_
c_func
(paren
l_string|&quot;directory stat info changes &quot;
l_string|&quot;after updating a file&quot;
)paren
)paren
suffix:semicolon
r_goto
id|done
suffix:semicolon
)brace
id|fputc
c_func
(paren
l_char|&squot;.&squot;
comma
id|stderr
)paren
suffix:semicolon
id|avoid_racy
c_func
(paren
)paren
suffix:semicolon
id|close
c_func
(paren
id|create_file
c_func
(paren
l_string|&quot;new-dir/new&quot;
)paren
)paren
suffix:semicolon
id|xstat_mtime_dir
c_func
(paren
op_amp
id|st
)paren
suffix:semicolon
r_if
c_cond
(paren
id|match_stat_data
c_func
(paren
op_amp
id|base
comma
op_amp
id|st
)paren
)paren
(brace
id|fputc
c_func
(paren
l_char|&squot;&bslash;n&squot;
comma
id|stderr
)paren
suffix:semicolon
id|fprintf_ln
c_func
(paren
id|stderr
comma
id|_
c_func
(paren
l_string|&quot;directory stat info changes after &quot;
l_string|&quot;adding a file inside subdirectory&quot;
)paren
)paren
suffix:semicolon
r_goto
id|done
suffix:semicolon
)brace
id|fputc
c_func
(paren
l_char|&squot;.&squot;
comma
id|stderr
)paren
suffix:semicolon
id|avoid_racy
c_func
(paren
)paren
suffix:semicolon
id|xunlink
c_func
(paren
l_string|&quot;newfile&quot;
)paren
suffix:semicolon
id|xstat_mtime_dir
c_func
(paren
op_amp
id|st
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|match_stat_data
c_func
(paren
op_amp
id|base
comma
op_amp
id|st
)paren
)paren
(brace
id|fputc
c_func
(paren
l_char|&squot;&bslash;n&squot;
comma
id|stderr
)paren
suffix:semicolon
id|fprintf_ln
c_func
(paren
id|stderr
comma
id|_
c_func
(paren
l_string|&quot;directory stat info does not &quot;
l_string|&quot;change after deleting a file&quot;
)paren
)paren
suffix:semicolon
r_goto
id|done
suffix:semicolon
)brace
id|fill_stat_data
c_func
(paren
op_amp
id|base
comma
op_amp
id|st
)paren
suffix:semicolon
id|fputc
c_func
(paren
l_char|&squot;.&squot;
comma
id|stderr
)paren
suffix:semicolon
id|avoid_racy
c_func
(paren
)paren
suffix:semicolon
id|xunlink
c_func
(paren
l_string|&quot;new-dir/new&quot;
)paren
suffix:semicolon
id|xrmdir
c_func
(paren
l_string|&quot;new-dir&quot;
)paren
suffix:semicolon
id|xstat_mtime_dir
c_func
(paren
op_amp
id|st
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|match_stat_data
c_func
(paren
op_amp
id|base
comma
op_amp
id|st
)paren
)paren
(brace
id|fputc
c_func
(paren
l_char|&squot;&bslash;n&squot;
comma
id|stderr
)paren
suffix:semicolon
id|fprintf_ln
c_func
(paren
id|stderr
comma
id|_
c_func
(paren
l_string|&quot;directory stat info does not &quot;
l_string|&quot;change after deleting a directory&quot;
)paren
)paren
suffix:semicolon
r_goto
id|done
suffix:semicolon
)brace
r_if
c_cond
(paren
id|rmdir
c_func
(paren
id|mtime_dir.buf
)paren
)paren
id|die_errno
c_func
(paren
id|_
c_func
(paren
l_string|&quot;failed to delete directory %s&quot;
)paren
comma
id|mtime_dir.buf
)paren
suffix:semicolon
id|fprintf_ln
c_func
(paren
id|stderr
comma
id|_
c_func
(paren
l_string|&quot; OK&quot;
)paren
)paren
suffix:semicolon
id|ret
op_assign
l_int|1
suffix:semicolon
id|done
suffix:colon
id|strbuf_release
c_func
(paren
op_amp
id|mtime_dir
)paren
suffix:semicolon
r_return
id|ret
suffix:semicolon
)brace
DECL|function|mark_ce_flags
r_static
r_int
id|mark_ce_flags
c_func
(paren
r_const
r_char
op_star
id|path
comma
r_int
id|flag
comma
r_int
id|mark
)paren
(brace
r_int
id|namelen
op_assign
id|strlen
c_func
(paren
id|path
)paren
suffix:semicolon
r_int
id|pos
op_assign
id|cache_name_pos
c_func
(paren
id|path
comma
id|namelen
)paren
suffix:semicolon
r_if
c_cond
(paren
l_int|0
op_le
id|pos
)paren
(brace
r_if
c_cond
(paren
id|mark
)paren
id|active_cache
(braket
id|pos
)braket
op_member_access_from_pointer
id|ce_flags
op_or_assign
id|flag
suffix:semicolon
r_else
id|active_cache
(braket
id|pos
)braket
op_member_access_from_pointer
id|ce_flags
op_and_assign
op_complement
id|flag
suffix:semicolon
id|active_cache
(braket
id|pos
)braket
op_member_access_from_pointer
id|ce_flags
op_or_assign
id|CE_UPDATE_IN_BASE
suffix:semicolon
id|cache_tree_invalidate_path
c_func
(paren
op_amp
id|the_index
comma
id|path
)paren
suffix:semicolon
id|active_cache_changed
op_or_assign
id|CE_ENTRY_CHANGED
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_return
l_int|1
suffix:semicolon
)brace
DECL|function|remove_one_path
r_static
r_int
id|remove_one_path
c_func
(paren
r_const
r_char
op_star
id|path
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|allow_remove
)paren
r_return
id|error
c_func
(paren
l_string|&quot;%s: does not exist and --remove not passed&quot;
comma
id|path
)paren
suffix:semicolon
r_if
c_cond
(paren
id|remove_file_from_cache
c_func
(paren
id|path
)paren
)paren
r_return
id|error
c_func
(paren
l_string|&quot;%s: cannot remove from the index&quot;
comma
id|path
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * Handle a path that couldn&squot;t be lstat&squot;ed. It&squot;s either:&n; *  - missing file (ENOENT or ENOTDIR). That&squot;s ok if we&squot;re&n; *    supposed to be removing it and the removal actually&n; *    succeeds.&n; *  - permission error. That&squot;s never ok.&n; */
DECL|function|process_lstat_error
r_static
r_int
id|process_lstat_error
c_func
(paren
r_const
r_char
op_star
id|path
comma
r_int
id|err
)paren
(brace
r_if
c_cond
(paren
id|err
op_eq
id|ENOENT
op_logical_or
id|err
op_eq
id|ENOTDIR
)paren
r_return
id|remove_one_path
c_func
(paren
id|path
)paren
suffix:semicolon
r_return
id|error
c_func
(paren
l_string|&quot;lstat(&bslash;&quot;%s&bslash;&quot;): %s&quot;
comma
id|path
comma
id|strerror
c_func
(paren
id|errno
)paren
)paren
suffix:semicolon
)brace
DECL|function|add_one_path
r_static
r_int
id|add_one_path
c_func
(paren
r_const
r_struct
id|cache_entry
op_star
id|old
comma
r_const
r_char
op_star
id|path
comma
r_int
id|len
comma
r_struct
id|stat
op_star
id|st
)paren
(brace
r_int
id|option
comma
id|size
suffix:semicolon
r_struct
id|cache_entry
op_star
id|ce
suffix:semicolon
multiline_comment|/* Was the old index entry already up-to-date? */
r_if
c_cond
(paren
id|old
op_logical_and
op_logical_neg
id|ce_stage
c_func
(paren
id|old
)paren
op_logical_and
op_logical_neg
id|ce_match_stat
c_func
(paren
id|old
comma
id|st
comma
l_int|0
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|size
op_assign
id|cache_entry_size
c_func
(paren
id|len
)paren
suffix:semicolon
id|ce
op_assign
id|xcalloc
c_func
(paren
l_int|1
comma
id|size
)paren
suffix:semicolon
id|memcpy
c_func
(paren
id|ce-&gt;name
comma
id|path
comma
id|len
)paren
suffix:semicolon
id|ce-&gt;ce_flags
op_assign
id|create_ce_flags
c_func
(paren
l_int|0
)paren
suffix:semicolon
id|ce-&gt;ce_namelen
op_assign
id|len
suffix:semicolon
id|fill_stat_cache_info
c_func
(paren
id|ce
comma
id|st
)paren
suffix:semicolon
id|ce-&gt;ce_mode
op_assign
id|ce_mode_from_stat
c_func
(paren
id|old
comma
id|st-&gt;st_mode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|index_path
c_func
(paren
id|ce-&gt;sha1
comma
id|path
comma
id|st
comma
id|info_only
ques
c_cond
l_int|0
suffix:colon
id|HASH_WRITE_OBJECT
)paren
)paren
(brace
id|free
c_func
(paren
id|ce
)paren
suffix:semicolon
r_return
l_int|1
suffix:semicolon
)brace
id|option
op_assign
id|allow_add
ques
c_cond
id|ADD_CACHE_OK_TO_ADD
suffix:colon
l_int|0
suffix:semicolon
id|option
op_or_assign
id|allow_replace
ques
c_cond
id|ADD_CACHE_OK_TO_REPLACE
suffix:colon
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|add_cache_entry
c_func
(paren
id|ce
comma
id|option
)paren
)paren
r_return
id|error
c_func
(paren
l_string|&quot;%s: cannot add to the index - missing --add option?&quot;
comma
id|path
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * Handle a path that was a directory. Four cases:&n; *&n; *  - it&squot;s already a gitlink in the index, and we keep it that&n; *    way, and update it if we can (if we cannot find the HEAD,&n; *    we&squot;re going to keep it unchanged in the index!)&n; *&n; *  - it&squot;s a *file* in the index, in which case it should be&n; *    removed as a file if removal is allowed, since it doesn&squot;t&n; *    exist as such any more. If removal isn&squot;t allowed, it&squot;s&n; *    an error.&n; *&n; *    (NOTE! This is old and arguably fairly strange behaviour.&n; *    We might want to make this an error unconditionally, and&n; *    use &quot;--force-remove&quot; if you actually want to force removal).&n; *&n; *  - it used to exist as a subdirectory (ie multiple files with&n; *    this particular prefix) in the index, in which case it&squot;s wrong&n; *    to try to update it as a directory.&n; *&n; *  - it doesn&squot;t exist at all in the index, but it is a valid&n; *    git directory, and it should be *added* as a gitlink.&n; */
DECL|function|process_directory
r_static
r_int
id|process_directory
c_func
(paren
r_const
r_char
op_star
id|path
comma
r_int
id|len
comma
r_struct
id|stat
op_star
id|st
)paren
(brace
r_int
r_char
id|sha1
(braket
l_int|20
)braket
suffix:semicolon
r_int
id|pos
op_assign
id|cache_name_pos
c_func
(paren
id|path
comma
id|len
)paren
suffix:semicolon
multiline_comment|/* Exact match: file or existing gitlink */
r_if
c_cond
(paren
id|pos
op_ge
l_int|0
)paren
(brace
r_const
r_struct
id|cache_entry
op_star
id|ce
op_assign
id|active_cache
(braket
id|pos
)braket
suffix:semicolon
r_if
c_cond
(paren
id|S_ISGITLINK
c_func
(paren
id|ce-&gt;ce_mode
)paren
)paren
(brace
multiline_comment|/* Do nothing to the index if there is no HEAD! */
r_if
c_cond
(paren
id|resolve_gitlink_ref
c_func
(paren
id|path
comma
l_string|&quot;HEAD&quot;
comma
id|sha1
)paren
OL
l_int|0
)paren
r_return
l_int|0
suffix:semicolon
r_return
id|add_one_path
c_func
(paren
id|ce
comma
id|path
comma
id|len
comma
id|st
)paren
suffix:semicolon
)brace
multiline_comment|/* Should this be an unconditional error? */
r_return
id|remove_one_path
c_func
(paren
id|path
)paren
suffix:semicolon
)brace
multiline_comment|/* Inexact match: is there perhaps a subdirectory match? */
id|pos
op_assign
id|pos
op_minus
l_int|1
suffix:semicolon
r_while
c_loop
(paren
id|pos
OL
id|active_nr
)paren
(brace
r_const
r_struct
id|cache_entry
op_star
id|ce
op_assign
id|active_cache
(braket
id|pos
op_increment
)braket
suffix:semicolon
r_if
c_cond
(paren
id|strncmp
c_func
(paren
id|ce-&gt;name
comma
id|path
comma
id|len
)paren
)paren
r_break
suffix:semicolon
r_if
c_cond
(paren
id|ce-&gt;name
(braket
id|len
)braket
OG
l_char|&squot;/&squot;
)paren
r_break
suffix:semicolon
r_if
c_cond
(paren
id|ce-&gt;name
(braket
id|len
)braket
OL
l_char|&squot;/&squot;
)paren
r_continue
suffix:semicolon
multiline_comment|/* Subdirectory match - error out */
r_return
id|error
c_func
(paren
l_string|&quot;%s: is a directory - add individual files instead&quot;
comma
id|path
)paren
suffix:semicolon
)brace
multiline_comment|/* No match - should we add it as a gitlink? */
r_if
c_cond
(paren
op_logical_neg
id|resolve_gitlink_ref
c_func
(paren
id|path
comma
l_string|&quot;HEAD&quot;
comma
id|sha1
)paren
)paren
r_return
id|add_one_path
c_func
(paren
l_int|NULL
comma
id|path
comma
id|len
comma
id|st
)paren
suffix:semicolon
multiline_comment|/* Error out. */
r_return
id|error
c_func
(paren
l_string|&quot;%s: is a directory - add files inside instead&quot;
comma
id|path
)paren
suffix:semicolon
)brace
DECL|function|process_path
r_static
r_int
id|process_path
c_func
(paren
r_const
r_char
op_star
id|path
)paren
(brace
r_int
id|pos
comma
id|len
suffix:semicolon
r_struct
id|stat
id|st
suffix:semicolon
r_const
r_struct
id|cache_entry
op_star
id|ce
suffix:semicolon
id|len
op_assign
id|strlen
c_func
(paren
id|path
)paren
suffix:semicolon
r_if
c_cond
(paren
id|has_symlink_leading_path
c_func
(paren
id|path
comma
id|len
)paren
)paren
r_return
id|error
c_func
(paren
l_string|&quot;&squot;%s&squot; is beyond a symbolic link&quot;
comma
id|path
)paren
suffix:semicolon
id|pos
op_assign
id|cache_name_pos
c_func
(paren
id|path
comma
id|len
)paren
suffix:semicolon
id|ce
op_assign
id|pos
OL
l_int|0
ques
c_cond
l_int|NULL
suffix:colon
id|active_cache
(braket
id|pos
)braket
suffix:semicolon
r_if
c_cond
(paren
id|ce
op_logical_and
id|ce_skip_worktree
c_func
(paren
id|ce
)paren
)paren
(brace
multiline_comment|/*&n;&t;&t; * working directory version is assumed &quot;good&quot;&n;&t;&t; * so updating it does not make sense.&n;&t;&t; * On the other hand, removing it from index should work&n;&t;&t; */
r_if
c_cond
(paren
id|allow_remove
op_logical_and
id|remove_file_from_cache
c_func
(paren
id|path
)paren
)paren
r_return
id|error
c_func
(paren
l_string|&quot;%s: cannot remove from the index&quot;
comma
id|path
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * First things first: get the stat information, to decide&n;&t; * what to do about the pathname!&n;&t; */
r_if
c_cond
(paren
id|lstat
c_func
(paren
id|path
comma
op_amp
id|st
)paren
OL
l_int|0
)paren
r_return
id|process_lstat_error
c_func
(paren
id|path
comma
id|errno
)paren
suffix:semicolon
r_if
c_cond
(paren
id|S_ISDIR
c_func
(paren
id|st.st_mode
)paren
)paren
r_return
id|process_directory
c_func
(paren
id|path
comma
id|len
comma
op_amp
id|st
)paren
suffix:semicolon
r_return
id|add_one_path
c_func
(paren
id|ce
comma
id|path
comma
id|len
comma
op_amp
id|st
)paren
suffix:semicolon
)brace
DECL|function|add_cacheinfo
r_static
r_int
id|add_cacheinfo
c_func
(paren
r_int
r_int
id|mode
comma
r_const
r_int
r_char
op_star
id|sha1
comma
r_const
r_char
op_star
id|path
comma
r_int
id|stage
)paren
(brace
r_int
id|size
comma
id|len
comma
id|option
suffix:semicolon
r_struct
id|cache_entry
op_star
id|ce
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|verify_path
c_func
(paren
id|path
)paren
)paren
r_return
id|error
c_func
(paren
l_string|&quot;Invalid path &squot;%s&squot;&quot;
comma
id|path
)paren
suffix:semicolon
id|len
op_assign
id|strlen
c_func
(paren
id|path
)paren
suffix:semicolon
id|size
op_assign
id|cache_entry_size
c_func
(paren
id|len
)paren
suffix:semicolon
id|ce
op_assign
id|xcalloc
c_func
(paren
l_int|1
comma
id|size
)paren
suffix:semicolon
id|hashcpy
c_func
(paren
id|ce-&gt;sha1
comma
id|sha1
)paren
suffix:semicolon
id|memcpy
c_func
(paren
id|ce-&gt;name
comma
id|path
comma
id|len
)paren
suffix:semicolon
id|ce-&gt;ce_flags
op_assign
id|create_ce_flags
c_func
(paren
id|stage
)paren
suffix:semicolon
id|ce-&gt;ce_namelen
op_assign
id|len
suffix:semicolon
id|ce-&gt;ce_mode
op_assign
id|create_ce_mode
c_func
(paren
id|mode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|assume_unchanged
)paren
id|ce-&gt;ce_flags
op_or_assign
id|CE_VALID
suffix:semicolon
id|option
op_assign
id|allow_add
ques
c_cond
id|ADD_CACHE_OK_TO_ADD
suffix:colon
l_int|0
suffix:semicolon
id|option
op_or_assign
id|allow_replace
ques
c_cond
id|ADD_CACHE_OK_TO_REPLACE
suffix:colon
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|add_cache_entry
c_func
(paren
id|ce
comma
id|option
)paren
)paren
r_return
id|error
c_func
(paren
l_string|&quot;%s: cannot add to the index - missing --add option?&quot;
comma
id|path
)paren
suffix:semicolon
id|report
c_func
(paren
l_string|&quot;add &squot;%s&squot;&quot;
comma
id|path
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|chmod_path
r_static
r_void
id|chmod_path
c_func
(paren
r_int
id|flip
comma
r_const
r_char
op_star
id|path
)paren
(brace
r_int
id|pos
suffix:semicolon
r_struct
id|cache_entry
op_star
id|ce
suffix:semicolon
r_int
r_int
id|mode
suffix:semicolon
id|pos
op_assign
id|cache_name_pos
c_func
(paren
id|path
comma
id|strlen
c_func
(paren
id|path
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|pos
OL
l_int|0
)paren
r_goto
id|fail
suffix:semicolon
id|ce
op_assign
id|active_cache
(braket
id|pos
)braket
suffix:semicolon
id|mode
op_assign
id|ce-&gt;ce_mode
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|S_ISREG
c_func
(paren
id|mode
)paren
)paren
r_goto
id|fail
suffix:semicolon
r_switch
c_cond
(paren
id|flip
)paren
(brace
r_case
l_char|&squot;+&squot;
suffix:colon
id|ce-&gt;ce_mode
op_or_assign
l_int|0111
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;-&squot;
suffix:colon
id|ce-&gt;ce_mode
op_and_assign
op_complement
l_int|0111
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
r_goto
id|fail
suffix:semicolon
)brace
id|cache_tree_invalidate_path
c_func
(paren
op_amp
id|the_index
comma
id|path
)paren
suffix:semicolon
id|ce-&gt;ce_flags
op_or_assign
id|CE_UPDATE_IN_BASE
suffix:semicolon
id|active_cache_changed
op_or_assign
id|CE_ENTRY_CHANGED
suffix:semicolon
id|report
c_func
(paren
l_string|&quot;chmod %cx &squot;%s&squot;&quot;
comma
id|flip
comma
id|path
)paren
suffix:semicolon
r_return
suffix:semicolon
id|fail
suffix:colon
id|die
c_func
(paren
l_string|&quot;git update-index: cannot chmod %cx &squot;%s&squot;&quot;
comma
id|flip
comma
id|path
)paren
suffix:semicolon
)brace
DECL|function|update_one
r_static
r_void
id|update_one
c_func
(paren
r_const
r_char
op_star
id|path
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|verify_path
c_func
(paren
id|path
)paren
)paren
(brace
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;Ignoring path %s&bslash;n&quot;
comma
id|path
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
r_if
c_cond
(paren
id|mark_valid_only
)paren
(brace
r_if
c_cond
(paren
id|mark_ce_flags
c_func
(paren
id|path
comma
id|CE_VALID
comma
id|mark_valid_only
op_eq
id|MARK_FLAG
)paren
)paren
id|die
c_func
(paren
l_string|&quot;Unable to mark file %s&quot;
comma
id|path
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
r_if
c_cond
(paren
id|mark_skip_worktree_only
)paren
(brace
r_if
c_cond
(paren
id|mark_ce_flags
c_func
(paren
id|path
comma
id|CE_SKIP_WORKTREE
comma
id|mark_skip_worktree_only
op_eq
id|MARK_FLAG
)paren
)paren
id|die
c_func
(paren
l_string|&quot;Unable to mark file %s&quot;
comma
id|path
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
r_if
c_cond
(paren
id|force_remove
)paren
(brace
r_if
c_cond
(paren
id|remove_file_from_cache
c_func
(paren
id|path
)paren
)paren
id|die
c_func
(paren
l_string|&quot;git update-index: unable to remove %s&quot;
comma
id|path
)paren
suffix:semicolon
id|report
c_func
(paren
l_string|&quot;remove &squot;%s&squot;&quot;
comma
id|path
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
r_if
c_cond
(paren
id|process_path
c_func
(paren
id|path
)paren
)paren
id|die
c_func
(paren
l_string|&quot;Unable to process path %s&quot;
comma
id|path
)paren
suffix:semicolon
id|report
c_func
(paren
l_string|&quot;add &squot;%s&squot;&quot;
comma
id|path
)paren
suffix:semicolon
)brace
DECL|function|read_index_info
r_static
r_void
id|read_index_info
c_func
(paren
r_int
id|line_termination
)paren
(brace
r_struct
id|strbuf
id|buf
op_assign
id|STRBUF_INIT
suffix:semicolon
r_struct
id|strbuf
id|uq
op_assign
id|STRBUF_INIT
suffix:semicolon
r_while
c_loop
(paren
id|strbuf_getline
c_func
(paren
op_amp
id|buf
comma
id|stdin
comma
id|line_termination
)paren
op_ne
id|EOF
)paren
(brace
r_char
op_star
id|ptr
comma
op_star
id|tab
suffix:semicolon
r_char
op_star
id|path_name
suffix:semicolon
r_int
r_char
id|sha1
(braket
l_int|20
)braket
suffix:semicolon
r_int
r_int
id|mode
suffix:semicolon
r_int
r_int
id|ul
suffix:semicolon
r_int
id|stage
suffix:semicolon
multiline_comment|/* This reads lines formatted in one of three formats:&n;&t;&t; *&n;&t;&t; * (1) mode         SP sha1          TAB path&n;&t;&t; * The first format is what &quot;git apply --index-info&quot;&n;&t;&t; * reports, and used to reconstruct a partial tree&n;&t;&t; * that is used for phony merge base tree when falling&n;&t;&t; * back on 3-way merge.&n;&t;&t; *&n;&t;&t; * (2) mode SP type SP sha1          TAB path&n;&t;&t; * The second format is to stuff &quot;git ls-tree&quot; output&n;&t;&t; * into the index file.&n;&t;&t; *&n;&t;&t; * (3) mode         SP sha1 SP stage TAB path&n;&t;&t; * This format is to put higher order stages into the&n;&t;&t; * index file and matches &quot;git ls-files --stage&quot; output.&n;&t;&t; */
id|errno
op_assign
l_int|0
suffix:semicolon
id|ul
op_assign
id|strtoul
c_func
(paren
id|buf.buf
comma
op_amp
id|ptr
comma
l_int|8
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ptr
op_eq
id|buf.buf
op_logical_or
op_star
id|ptr
op_ne
l_char|&squot; &squot;
op_logical_or
id|errno
op_logical_or
(paren
r_int
r_int
)paren
id|ul
op_ne
id|ul
)paren
r_goto
id|bad_line
suffix:semicolon
id|mode
op_assign
id|ul
suffix:semicolon
id|tab
op_assign
id|strchr
c_func
(paren
id|ptr
comma
l_char|&squot;&bslash;t&squot;
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|tab
op_logical_or
id|tab
id|ptr
OL
l_int|41
)paren
r_goto
id|bad_line
suffix:semicolon
r_if
c_cond
(paren
id|tab
(braket
l_int|2
)braket
op_eq
l_char|&squot; &squot;
op_logical_and
l_char|&squot;0&squot;
op_le
id|tab
(braket
l_int|1
)braket
op_logical_and
id|tab
(braket
l_int|1
)braket
op_le
l_char|&squot;3&squot;
)paren
(brace
id|stage
op_assign
id|tab
(braket
l_int|1
)braket
l_char|&squot;0&squot;
suffix:semicolon
id|ptr
op_assign
id|tab
op_plus
l_int|1
suffix:semicolon
multiline_comment|/* point at the head of path */
id|tab
op_assign
id|tab
l_int|2
suffix:semicolon
multiline_comment|/* point at tail of sha1 */
)brace
r_else
(brace
id|stage
op_assign
l_int|0
suffix:semicolon
id|ptr
op_assign
id|tab
op_plus
l_int|1
suffix:semicolon
multiline_comment|/* point at the head of path */
)brace
r_if
c_cond
(paren
id|get_sha1_hex
c_func
(paren
id|tab
l_int|40
comma
id|sha1
)paren
op_logical_or
id|tab
(braket
l_int|41
)braket
op_ne
l_char|&squot; &squot;
)paren
r_goto
id|bad_line
suffix:semicolon
id|path_name
op_assign
id|ptr
suffix:semicolon
r_if
c_cond
(paren
id|line_termination
op_logical_and
id|path_name
(braket
l_int|0
)braket
op_eq
l_char|&squot;&quot;&squot;
)paren
(brace
id|strbuf_reset
c_func
(paren
op_amp
id|uq
)paren
suffix:semicolon
r_if
c_cond
(paren
id|unquote_c_style
c_func
(paren
op_amp
id|uq
comma
id|path_name
comma
l_int|NULL
)paren
)paren
(brace
id|die
c_func
(paren
l_string|&quot;git update-index: bad quoting of path name&quot;
)paren
suffix:semicolon
)brace
id|path_name
op_assign
id|uq.buf
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|verify_path
c_func
(paren
id|path_name
)paren
)paren
(brace
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;Ignoring path %s&bslash;n&quot;
comma
id|path_name
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|mode
)paren
(brace
multiline_comment|/* mode == 0 means there is no such path -- remove */
r_if
c_cond
(paren
id|remove_file_from_cache
c_func
(paren
id|path_name
)paren
)paren
id|die
c_func
(paren
l_string|&quot;git update-index: unable to remove %s&quot;
comma
id|ptr
)paren
suffix:semicolon
)brace
r_else
(brace
multiline_comment|/* mode &squot; &squot; sha1 &squot;&bslash;t&squot; name&n;&t;&t;&t; * ptr[-1] points at tab,&n;&t;&t;&t; * ptr[-41] is at the beginning of sha1&n;&t;&t;&t; */
id|ptr
(braket
l_int|42
)braket
op_assign
id|ptr
(braket
l_int|1
)braket
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|add_cacheinfo
c_func
(paren
id|mode
comma
id|sha1
comma
id|path_name
comma
id|stage
)paren
)paren
id|die
c_func
(paren
l_string|&quot;git update-index: unable to update %s&quot;
comma
id|path_name
)paren
suffix:semicolon
)brace
r_continue
suffix:semicolon
id|bad_line
suffix:colon
id|die
c_func
(paren
l_string|&quot;malformed index info %s&quot;
comma
id|buf.buf
)paren
suffix:semicolon
)brace
id|strbuf_release
c_func
(paren
op_amp
id|buf
)paren
suffix:semicolon
id|strbuf_release
c_func
(paren
op_amp
id|uq
)paren
suffix:semicolon
)brace
DECL|variable|update_index_usage
r_static
r_const
r_char
op_star
r_const
id|update_index_usage
(braket
)braket
op_assign
(brace
id|N_
c_func
(paren
l_string|&quot;git update-index [&lt;options&gt;] [--] [&lt;file&gt;...]&quot;
)paren
comma
l_int|NULL
)brace
suffix:semicolon
DECL|variable|head_sha1
r_static
r_int
r_char
id|head_sha1
(braket
l_int|20
)braket
suffix:semicolon
DECL|variable|merge_head_sha1
r_static
r_int
r_char
id|merge_head_sha1
(braket
l_int|20
)braket
suffix:semicolon
DECL|function|read_one_ent
r_static
r_struct
id|cache_entry
op_star
id|read_one_ent
c_func
(paren
r_const
r_char
op_star
id|which
comma
r_int
r_char
op_star
id|ent
comma
r_const
r_char
op_star
id|path
comma
r_int
id|namelen
comma
r_int
id|stage
)paren
(brace
r_int
id|mode
suffix:semicolon
r_int
r_char
id|sha1
(braket
l_int|20
)braket
suffix:semicolon
r_int
id|size
suffix:semicolon
r_struct
id|cache_entry
op_star
id|ce
suffix:semicolon
r_if
c_cond
(paren
id|get_tree_entry
c_func
(paren
id|ent
comma
id|path
comma
id|sha1
comma
op_amp
id|mode
)paren
)paren
(brace
r_if
c_cond
(paren
id|which
)paren
id|error
c_func
(paren
l_string|&quot;%s: not in %s branch.&quot;
comma
id|path
comma
id|which
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
r_if
c_cond
(paren
id|mode
op_eq
id|S_IFDIR
)paren
(brace
r_if
c_cond
(paren
id|which
)paren
id|error
c_func
(paren
l_string|&quot;%s: not a blob in %s branch.&quot;
comma
id|path
comma
id|which
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
id|size
op_assign
id|cache_entry_size
c_func
(paren
id|namelen
)paren
suffix:semicolon
id|ce
op_assign
id|xcalloc
c_func
(paren
l_int|1
comma
id|size
)paren
suffix:semicolon
id|hashcpy
c_func
(paren
id|ce-&gt;sha1
comma
id|sha1
)paren
suffix:semicolon
id|memcpy
c_func
(paren
id|ce-&gt;name
comma
id|path
comma
id|namelen
)paren
suffix:semicolon
id|ce-&gt;ce_flags
op_assign
id|create_ce_flags
c_func
(paren
id|stage
)paren
suffix:semicolon
id|ce-&gt;ce_namelen
op_assign
id|namelen
suffix:semicolon
id|ce-&gt;ce_mode
op_assign
id|create_ce_mode
c_func
(paren
id|mode
)paren
suffix:semicolon
r_return
id|ce
suffix:semicolon
)brace
DECL|function|unresolve_one
r_static
r_int
id|unresolve_one
c_func
(paren
r_const
r_char
op_star
id|path
)paren
(brace
r_int
id|namelen
op_assign
id|strlen
c_func
(paren
id|path
)paren
suffix:semicolon
r_int
id|pos
suffix:semicolon
r_int
id|ret
op_assign
l_int|0
suffix:semicolon
r_struct
id|cache_entry
op_star
id|ce_2
op_assign
l_int|NULL
comma
op_star
id|ce_3
op_assign
l_int|NULL
suffix:semicolon
multiline_comment|/* See if there is such entry in the index. */
id|pos
op_assign
id|cache_name_pos
c_func
(paren
id|path
comma
id|namelen
)paren
suffix:semicolon
r_if
c_cond
(paren
l_int|0
op_le
id|pos
)paren
(brace
multiline_comment|/* already merged */
id|pos
op_assign
id|unmerge_cache_entry_at
c_func
(paren
id|pos
)paren
suffix:semicolon
r_if
c_cond
(paren
id|pos
OL
id|active_nr
)paren
(brace
r_const
r_struct
id|cache_entry
op_star
id|ce
op_assign
id|active_cache
(braket
id|pos
)braket
suffix:semicolon
r_if
c_cond
(paren
id|ce_stage
c_func
(paren
id|ce
)paren
op_logical_and
id|ce_namelen
c_func
(paren
id|ce
)paren
op_eq
id|namelen
op_logical_and
op_logical_neg
id|memcmp
c_func
(paren
id|ce-&gt;name
comma
id|path
comma
id|namelen
)paren
)paren
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* no resolve-undo information; fall back */
)brace
r_else
(brace
multiline_comment|/* If there isn&squot;t, either it is unmerged, or&n;&t;&t; * resolved as &quot;removed&quot; by mistake.  We do not&n;&t;&t; * want to do anything in the former case.&n;&t;&t; */
id|pos
op_assign
id|pos
op_minus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|pos
OL
id|active_nr
)paren
(brace
r_const
r_struct
id|cache_entry
op_star
id|ce
op_assign
id|active_cache
(braket
id|pos
)braket
suffix:semicolon
r_if
c_cond
(paren
id|ce_namelen
c_func
(paren
id|ce
)paren
op_eq
id|namelen
op_logical_and
op_logical_neg
id|memcmp
c_func
(paren
id|ce-&gt;name
comma
id|path
comma
id|namelen
)paren
)paren
(brace
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;%s: skipping still unmerged path.&bslash;n&quot;
comma
id|path
)paren
suffix:semicolon
r_goto
id|free_return
suffix:semicolon
)brace
)brace
)brace
multiline_comment|/* Grab blobs from given path from HEAD and MERGE_HEAD,&n;&t; * stuff HEAD version in stage #2,&n;&t; * stuff MERGE_HEAD version in stage #3.&n;&t; */
id|ce_2
op_assign
id|read_one_ent
c_func
(paren
l_string|&quot;our&quot;
comma
id|head_sha1
comma
id|path
comma
id|namelen
comma
l_int|2
)paren
suffix:semicolon
id|ce_3
op_assign
id|read_one_ent
c_func
(paren
l_string|&quot;their&quot;
comma
id|merge_head_sha1
comma
id|path
comma
id|namelen
comma
l_int|3
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ce_2
op_logical_or
op_logical_neg
id|ce_3
)paren
(brace
id|ret
op_assign
l_int|1
suffix:semicolon
r_goto
id|free_return
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|hashcmp
c_func
(paren
id|ce_2-&gt;sha1
comma
id|ce_3-&gt;sha1
)paren
op_logical_and
id|ce_2-&gt;ce_mode
op_eq
id|ce_3-&gt;ce_mode
)paren
(brace
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;%s: identical in both, skipping.&bslash;n&quot;
comma
id|path
)paren
suffix:semicolon
r_goto
id|free_return
suffix:semicolon
)brace
id|remove_file_from_cache
c_func
(paren
id|path
)paren
suffix:semicolon
r_if
c_cond
(paren
id|add_cache_entry
c_func
(paren
id|ce_2
comma
id|ADD_CACHE_OK_TO_ADD
)paren
)paren
(brace
id|error
c_func
(paren
l_string|&quot;%s: cannot add our version to the index.&quot;
comma
id|path
)paren
suffix:semicolon
id|ret
op_assign
l_int|1
suffix:semicolon
r_goto
id|free_return
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|add_cache_entry
c_func
(paren
id|ce_3
comma
id|ADD_CACHE_OK_TO_ADD
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|error
c_func
(paren
l_string|&quot;%s: cannot add their version to the index.&quot;
comma
id|path
)paren
suffix:semicolon
id|ret
op_assign
l_int|1
suffix:semicolon
id|free_return
suffix:colon
id|free
c_func
(paren
id|ce_2
)paren
suffix:semicolon
id|free
c_func
(paren
id|ce_3
)paren
suffix:semicolon
r_return
id|ret
suffix:semicolon
)brace
DECL|function|read_head_pointers
r_static
r_void
id|read_head_pointers
c_func
(paren
r_void
)paren
(brace
r_if
c_cond
(paren
id|read_ref
c_func
(paren
l_string|&quot;HEAD&quot;
comma
id|head_sha1
)paren
)paren
id|die
c_func
(paren
l_string|&quot;No HEAD -- no initial commit yet?&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|read_ref
c_func
(paren
l_string|&quot;MERGE_HEAD&quot;
comma
id|merge_head_sha1
)paren
)paren
(brace
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;Not in the middle of a merge.&bslash;n&quot;
)paren
suffix:semicolon
m_exit
(paren
l_int|0
)paren
suffix:semicolon
)brace
)brace
DECL|function|do_unresolve
r_static
r_int
id|do_unresolve
c_func
(paren
r_int
id|ac
comma
r_const
r_char
op_star
op_star
id|av
comma
r_const
r_char
op_star
id|prefix
comma
r_int
id|prefix_length
)paren
(brace
r_int
id|i
suffix:semicolon
r_int
id|err
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* Read HEAD and MERGE_HEAD; if MERGE_HEAD does not exist, we&n;&t; * are not doing a merge, so exit with success status.&n;&t; */
id|read_head_pointers
c_func
(paren
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|1
suffix:semicolon
id|i
OL
id|ac
suffix:semicolon
id|i
op_increment
)paren
(brace
r_const
r_char
op_star
id|arg
op_assign
id|av
(braket
id|i
)braket
suffix:semicolon
r_const
r_char
op_star
id|p
op_assign
id|prefix_path
c_func
(paren
id|prefix
comma
id|prefix_length
comma
id|arg
)paren
suffix:semicolon
id|err
op_or_assign
id|unresolve_one
c_func
(paren
id|p
)paren
suffix:semicolon
r_if
c_cond
(paren
id|p
template_param
id|arg
op_plus
id|strlen
c_func
(paren
id|arg
)paren
)paren
id|free
c_func
(paren
(paren
r_char
op_star
)paren
id|p
)paren
suffix:semicolon
)brace
r_return
id|err
suffix:semicolon
)brace
DECL|function|do_reupdate
r_static
r_int
id|do_reupdate
c_func
(paren
r_int
id|ac
comma
r_const
r_char
op_star
op_star
id|av
comma
r_const
r_char
op_star
id|prefix
comma
r_int
id|prefix_length
)paren
(brace
multiline_comment|/* Read HEAD and run update-index on paths that are&n;&t; * merged and already different between index and HEAD.&n;&t; */
r_int
id|pos
suffix:semicolon
r_int
id|has_head
op_assign
l_int|1
suffix:semicolon
r_struct
id|pathspec
id|pathspec
suffix:semicolon
id|parse_pathspec
c_func
(paren
op_amp
id|pathspec
comma
l_int|0
comma
id|PATHSPEC_PREFER_CWD
comma
id|prefix
comma
id|av
op_plus
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|read_ref
c_func
(paren
l_string|&quot;HEAD&quot;
comma
id|head_sha1
)paren
)paren
multiline_comment|/* If there is no HEAD, that means it is an initial&n;&t;&t; * commit.  Update everything in the index.&n;&t;&t; */
id|has_head
op_assign
l_int|0
suffix:semicolon
id|redo
suffix:colon
r_for
c_loop
(paren
id|pos
op_assign
l_int|0
suffix:semicolon
id|pos
OL
id|active_nr
suffix:semicolon
id|pos
op_increment
)paren
(brace
r_const
r_struct
id|cache_entry
op_star
id|ce
op_assign
id|active_cache
(braket
id|pos
)braket
suffix:semicolon
r_struct
id|cache_entry
op_star
id|old
op_assign
l_int|NULL
suffix:semicolon
r_int
id|save_nr
suffix:semicolon
r_char
op_star
id|path
suffix:semicolon
r_if
c_cond
(paren
id|ce_stage
c_func
(paren
id|ce
)paren
op_logical_or
op_logical_neg
id|ce_path_match
c_func
(paren
id|ce
comma
op_amp
id|pathspec
comma
l_int|NULL
)paren
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|has_head
)paren
id|old
op_assign
id|read_one_ent
c_func
(paren
l_int|NULL
comma
id|head_sha1
comma
id|ce-&gt;name
comma
id|ce_namelen
c_func
(paren
id|ce
)paren
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
id|old
op_logical_and
id|ce-&gt;ce_mode
op_eq
id|old-&gt;ce_mode
op_logical_and
op_logical_neg
id|hashcmp
c_func
(paren
id|ce-&gt;sha1
comma
id|old-&gt;sha1
)paren
)paren
(brace
id|free
c_func
(paren
id|old
)paren
suffix:semicolon
r_continue
suffix:semicolon
multiline_comment|/* unchanged */
)brace
multiline_comment|/* Be careful.  The working tree may not have the&n;&t;&t; * path anymore, in which case, under &squot;allow_remove&squot;,&n;&t;&t; * or worse yet &squot;allow_replace&squot;, active_nr may decrease.&n;&t;&t; */
id|save_nr
op_assign
id|active_nr
suffix:semicolon
id|path
op_assign
id|xstrdup
c_func
(paren
id|ce-&gt;name
)paren
suffix:semicolon
id|update_one
c_func
(paren
id|path
)paren
suffix:semicolon
id|free
c_func
(paren
id|path
)paren
suffix:semicolon
r_if
c_cond
(paren
id|save_nr
op_ne
id|active_nr
)paren
r_goto
id|redo
suffix:semicolon
)brace
id|free_pathspec
c_func
(paren
op_amp
id|pathspec
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|struct|refresh_params
r_struct
id|refresh_params
(brace
DECL|member|flags
r_int
r_int
id|flags
suffix:semicolon
DECL|member|has_errors
r_int
op_star
id|has_errors
suffix:semicolon
)brace
suffix:semicolon
DECL|function|refresh
r_static
r_int
id|refresh
c_func
(paren
r_struct
id|refresh_params
op_star
id|o
comma
r_int
r_int
id|flag
)paren
(brace
id|setup_work_tree
c_func
(paren
)paren
suffix:semicolon
id|read_cache_preload
c_func
(paren
l_int|NULL
)paren
suffix:semicolon
op_star
id|o-&gt;has_errors
op_or_assign
id|refresh_cache
c_func
(paren
id|o-&gt;flags
op_or
id|flag
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|refresh_callback
r_static
r_int
id|refresh_callback
c_func
(paren
r_const
r_struct
id|option
op_star
id|opt
comma
r_const
r_char
op_star
id|arg
comma
r_int
id|unset
)paren
(brace
r_return
id|refresh
c_func
(paren
id|opt-&gt;value
comma
l_int|0
)paren
suffix:semicolon
)brace
DECL|function|really_refresh_callback
r_static
r_int
id|really_refresh_callback
c_func
(paren
r_const
r_struct
id|option
op_star
id|opt
comma
r_const
r_char
op_star
id|arg
comma
r_int
id|unset
)paren
(brace
r_return
id|refresh
c_func
(paren
id|opt-&gt;value
comma
id|REFRESH_REALLY
)paren
suffix:semicolon
)brace
DECL|function|chmod_callback
r_static
r_int
id|chmod_callback
c_func
(paren
r_const
r_struct
id|option
op_star
id|opt
comma
r_const
r_char
op_star
id|arg
comma
r_int
id|unset
)paren
(brace
r_char
op_star
id|flip
op_assign
id|opt-&gt;value
suffix:semicolon
r_if
c_cond
(paren
(paren
id|arg
(braket
l_int|0
)braket
op_ne
l_char|&squot;-&squot;
op_logical_and
id|arg
(braket
l_int|0
)braket
op_ne
l_char|&squot;+&squot;
)paren
op_logical_or
id|arg
(braket
l_int|1
)braket
op_ne
l_char|&squot;x&squot;
op_logical_or
id|arg
(braket
l_int|2
)braket
)paren
r_return
id|error
c_func
(paren
l_string|&quot;option &squot;chmod&squot; expects &bslash;&quot;+x&bslash;&quot; or &bslash;&quot;-x&bslash;&quot;&quot;
)paren
suffix:semicolon
op_star
id|flip
op_assign
id|arg
(braket
l_int|0
)braket
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|resolve_undo_clear_callback
r_static
r_int
id|resolve_undo_clear_callback
c_func
(paren
r_const
r_struct
id|option
op_star
id|opt
comma
r_const
r_char
op_star
id|arg
comma
r_int
id|unset
)paren
(brace
id|resolve_undo_clear
c_func
(paren
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|parse_new_style_cacheinfo
r_static
r_int
id|parse_new_style_cacheinfo
c_func
(paren
r_const
r_char
op_star
id|arg
comma
r_int
r_int
op_star
id|mode
comma
r_int
r_char
id|sha1
(braket
)braket
comma
r_const
r_char
op_star
op_star
id|path
)paren
(brace
r_int
r_int
id|ul
suffix:semicolon
r_char
op_star
id|endp
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|arg
)paren
r_return
l_int|1
suffix:semicolon
id|errno
op_assign
l_int|0
suffix:semicolon
id|ul
op_assign
id|strtoul
c_func
(paren
id|arg
comma
op_amp
id|endp
comma
l_int|8
)paren
suffix:semicolon
r_if
c_cond
(paren
id|errno
op_logical_or
id|endp
op_eq
id|arg
op_logical_or
op_star
id|endp
op_ne
l_char|&squot;,&squot;
op_logical_or
(paren
r_int
r_int
)paren
id|ul
op_ne
id|ul
)paren
r_return
l_int|1
suffix:semicolon
multiline_comment|/* not a new-style cacheinfo */
op_star
id|mode
op_assign
id|ul
suffix:semicolon
id|endp
op_increment
suffix:semicolon
r_if
c_cond
(paren
id|get_sha1_hex
c_func
(paren
id|endp
comma
id|sha1
)paren
op_logical_or
id|endp
(braket
l_int|40
)braket
op_ne
l_char|&squot;,&squot;
)paren
r_return
l_int|1
suffix:semicolon
op_star
id|path
op_assign
id|endp
op_plus
l_int|41
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|cacheinfo_callback
r_static
r_int
id|cacheinfo_callback
c_func
(paren
r_struct
id|parse_opt_ctx_t
op_star
id|ctx
comma
r_const
r_struct
id|option
op_star
id|opt
comma
r_int
id|unset
)paren
(brace
r_int
r_char
id|sha1
(braket
l_int|20
)braket
suffix:semicolon
r_int
r_int
id|mode
suffix:semicolon
r_const
r_char
op_star
id|path
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|parse_new_style_cacheinfo
c_func
(paren
id|ctx-&gt;argv
(braket
l_int|1
)braket
comma
op_amp
id|mode
comma
id|sha1
comma
op_amp
id|path
)paren
)paren
(brace
r_if
c_cond
(paren
id|add_cacheinfo
c_func
(paren
id|mode
comma
id|sha1
comma
id|path
comma
l_int|0
)paren
)paren
id|die
c_func
(paren
l_string|&quot;git update-index: --cacheinfo cannot add %s&quot;
comma
id|path
)paren
suffix:semicolon
id|ctx-&gt;argv
op_increment
suffix:semicolon
id|ctx-&gt;argc
op_decrement
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
id|ctx-&gt;argc
op_le
l_int|3
)paren
r_return
id|error
c_func
(paren
l_string|&quot;option &squot;cacheinfo&squot; expects &lt;mode&gt;,&lt;sha1&gt;,&lt;path&gt;&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|strtoul_ui
c_func
(paren
op_star
op_increment
id|ctx-&gt;argv
comma
l_int|8
comma
op_amp
id|mode
)paren
op_logical_or
id|get_sha1_hex
c_func
(paren
op_star
op_increment
id|ctx-&gt;argv
comma
id|sha1
)paren
op_logical_or
id|add_cacheinfo
c_func
(paren
id|mode
comma
id|sha1
comma
op_star
op_increment
id|ctx-&gt;argv
comma
l_int|0
)paren
)paren
id|die
c_func
(paren
l_string|&quot;git update-index: --cacheinfo cannot add %s&quot;
comma
op_star
id|ctx-&gt;argv
)paren
suffix:semicolon
id|ctx-&gt;argc
op_sub_assign
l_int|3
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|stdin_cacheinfo_callback
r_static
r_int
id|stdin_cacheinfo_callback
c_func
(paren
r_struct
id|parse_opt_ctx_t
op_star
id|ctx
comma
r_const
r_struct
id|option
op_star
id|opt
comma
r_int
id|unset
)paren
(brace
r_int
op_star
id|line_termination
op_assign
id|opt-&gt;value
suffix:semicolon
r_if
c_cond
(paren
id|ctx-&gt;argc
op_ne
l_int|1
)paren
r_return
id|error
c_func
(paren
l_string|&quot;option &squot;%s&squot; must be the last argument&quot;
comma
id|opt-&gt;long_name
)paren
suffix:semicolon
id|allow_add
op_assign
id|allow_replace
op_assign
id|allow_remove
op_assign
l_int|1
suffix:semicolon
id|read_index_info
c_func
(paren
op_star
id|line_termination
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|stdin_callback
r_static
r_int
id|stdin_callback
c_func
(paren
r_struct
id|parse_opt_ctx_t
op_star
id|ctx
comma
r_const
r_struct
id|option
op_star
id|opt
comma
r_int
id|unset
)paren
(brace
r_int
op_star
id|read_from_stdin
op_assign
id|opt-&gt;value
suffix:semicolon
r_if
c_cond
(paren
id|ctx-&gt;argc
op_ne
l_int|1
)paren
r_return
id|error
c_func
(paren
l_string|&quot;option &squot;%s&squot; must be the last argument&quot;
comma
id|opt-&gt;long_name
)paren
suffix:semicolon
op_star
id|read_from_stdin
op_assign
l_int|1
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|unresolve_callback
r_static
r_int
id|unresolve_callback
c_func
(paren
r_struct
id|parse_opt_ctx_t
op_star
id|ctx
comma
r_const
r_struct
id|option
op_star
id|opt
comma
r_int
id|flags
)paren
(brace
r_int
op_star
id|has_errors
op_assign
id|opt-&gt;value
suffix:semicolon
r_const
r_char
op_star
id|prefix
op_assign
id|startup_info-&gt;prefix
suffix:semicolon
multiline_comment|/* consume remaining arguments. */
op_star
id|has_errors
op_assign
id|do_unresolve
c_func
(paren
id|ctx-&gt;argc
comma
id|ctx-&gt;argv
comma
id|prefix
comma
id|prefix
ques
c_cond
id|strlen
c_func
(paren
id|prefix
)paren
suffix:colon
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
op_star
id|has_errors
)paren
id|active_cache_changed
op_assign
l_int|0
suffix:semicolon
id|ctx-&gt;argv
op_add_assign
id|ctx-&gt;argc
l_int|1
suffix:semicolon
id|ctx-&gt;argc
op_assign
l_int|1
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|reupdate_callback
r_static
r_int
id|reupdate_callback
c_func
(paren
r_struct
id|parse_opt_ctx_t
op_star
id|ctx
comma
r_const
r_struct
id|option
op_star
id|opt
comma
r_int
id|flags
)paren
(brace
r_int
op_star
id|has_errors
op_assign
id|opt-&gt;value
suffix:semicolon
r_const
r_char
op_star
id|prefix
op_assign
id|startup_info-&gt;prefix
suffix:semicolon
multiline_comment|/* consume remaining arguments. */
id|setup_work_tree
c_func
(paren
)paren
suffix:semicolon
op_star
id|has_errors
op_assign
id|do_reupdate
c_func
(paren
id|ctx-&gt;argc
comma
id|ctx-&gt;argv
comma
id|prefix
comma
id|prefix
ques
c_cond
id|strlen
c_func
(paren
id|prefix
)paren
suffix:colon
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
op_star
id|has_errors
)paren
id|active_cache_changed
op_assign
l_int|0
suffix:semicolon
id|ctx-&gt;argv
op_add_assign
id|ctx-&gt;argc
l_int|1
suffix:semicolon
id|ctx-&gt;argc
op_assign
l_int|1
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|cmd_update_index
r_int
id|cmd_update_index
c_func
(paren
r_int
id|argc
comma
r_const
r_char
op_star
op_star
id|argv
comma
r_const
r_char
op_star
id|prefix
)paren
(brace
r_int
id|newfd
comma
id|entries
comma
id|has_errors
op_assign
l_int|0
comma
id|line_termination
op_assign
l_char|&squot;&bslash;n&squot;
suffix:semicolon
r_int
id|untracked_cache
op_assign
l_int|1
suffix:semicolon
r_int
id|read_from_stdin
op_assign
l_int|0
suffix:semicolon
r_int
id|prefix_length
op_assign
id|prefix
ques
c_cond
id|strlen
c_func
(paren
id|prefix
)paren
suffix:colon
l_int|0
suffix:semicolon
r_int
id|preferred_index_format
op_assign
l_int|0
suffix:semicolon
r_char
id|set_executable_bit
op_assign
l_int|0
suffix:semicolon
r_struct
id|refresh_params
id|refresh_args
op_assign
(brace
l_int|0
comma
op_amp
id|has_errors
)brace
suffix:semicolon
r_int
id|lock_error
op_assign
l_int|0
suffix:semicolon
r_int
id|split_index
op_assign
l_int|1
suffix:semicolon
r_struct
id|lock_file
op_star
id|lock_file
suffix:semicolon
r_struct
id|parse_opt_ctx_t
id|ctx
suffix:semicolon
r_int
id|parseopt_state
op_assign
id|PARSE_OPT_UNKNOWN
suffix:semicolon
r_struct
id|option
id|options
(braket
)braket
op_assign
(brace
id|OPT_BIT
c_func
(paren
l_char|&squot;q&squot;
comma
l_int|NULL
comma
op_amp
id|refresh_args.flags
comma
id|N_
c_func
(paren
l_string|&quot;continue refresh even when index needs update&quot;
)paren
comma
id|REFRESH_QUIET
)paren
comma
id|OPT_BIT
c_func
(paren
l_int|0
comma
l_string|&quot;ignore-submodules&quot;
comma
op_amp
id|refresh_args.flags
comma
id|N_
c_func
(paren
l_string|&quot;refresh: ignore submodules&quot;
)paren
comma
id|REFRESH_IGNORE_SUBMODULES
)paren
comma
id|OPT_SET_INT
c_func
(paren
l_int|0
comma
l_string|&quot;add&quot;
comma
op_amp
id|allow_add
comma
id|N_
c_func
(paren
l_string|&quot;do not ignore new files&quot;
)paren
comma
l_int|1
)paren
comma
id|OPT_SET_INT
c_func
(paren
l_int|0
comma
l_string|&quot;replace&quot;
comma
op_amp
id|allow_replace
comma
id|N_
c_func
(paren
l_string|&quot;let files replace directories and vice-versa&quot;
)paren
comma
l_int|1
)paren
comma
id|OPT_SET_INT
c_func
(paren
l_int|0
comma
l_string|&quot;remove&quot;
comma
op_amp
id|allow_remove
comma
id|N_
c_func
(paren
l_string|&quot;notice files missing from worktree&quot;
)paren
comma
l_int|1
)paren
comma
id|OPT_BIT
c_func
(paren
l_int|0
comma
l_string|&quot;unmerged&quot;
comma
op_amp
id|refresh_args.flags
comma
id|N_
c_func
(paren
l_string|&quot;refresh even if index contains unmerged entries&quot;
)paren
comma
id|REFRESH_UNMERGED
)paren
comma
(brace
id|OPTION_CALLBACK
comma
l_int|0
comma
l_string|&quot;refresh&quot;
comma
op_amp
id|refresh_args
comma
l_int|NULL
comma
id|N_
c_func
(paren
l_string|&quot;refresh stat information&quot;
)paren
comma
id|PARSE_OPT_NOARG
op_or
id|PARSE_OPT_NONEG
comma
id|refresh_callback
)brace
comma
(brace
id|OPTION_CALLBACK
comma
l_int|0
comma
l_string|&quot;really-refresh&quot;
comma
op_amp
id|refresh_args
comma
l_int|NULL
comma
id|N_
c_func
(paren
l_string|&quot;like --refresh, but ignore assume-unchanged setting&quot;
)paren
comma
id|PARSE_OPT_NOARG
op_or
id|PARSE_OPT_NONEG
comma
id|really_refresh_callback
)brace
comma
(brace
id|OPTION_LOWLEVEL_CALLBACK
comma
l_int|0
comma
l_string|&quot;cacheinfo&quot;
comma
l_int|NULL
comma
id|N_
c_func
(paren
l_string|&quot;&lt;mode&gt;,&lt;object&gt;,&lt;path&gt;&quot;
)paren
comma
id|N_
c_func
(paren
l_string|&quot;add the specified entry to the index&quot;
)paren
comma
id|PARSE_OPT_NOARG
op_or
multiline_comment|/* disallow --cacheinfo=&lt;mode&gt; form */
id|PARSE_OPT_NONEG
op_or
id|PARSE_OPT_LITERAL_ARGHELP
comma
(paren
id|parse_opt_cb
op_star
)paren
id|cacheinfo_callback
)brace
comma
(brace
id|OPTION_CALLBACK
comma
l_int|0
comma
l_string|&quot;chmod&quot;
comma
op_amp
id|set_executable_bit
comma
id|N_
c_func
(paren
l_string|&quot;(+/-)x&quot;
)paren
comma
id|N_
c_func
(paren
l_string|&quot;override the executable bit of the listed files&quot;
)paren
comma
id|PARSE_OPT_NONEG
op_or
id|PARSE_OPT_LITERAL_ARGHELP
comma
id|chmod_callback
)brace
comma
(brace
id|OPTION_SET_INT
comma
l_int|0
comma
l_string|&quot;assume-unchanged&quot;
comma
op_amp
id|mark_valid_only
comma
l_int|NULL
comma
id|N_
c_func
(paren
l_string|&quot;mark files as &bslash;&quot;not changing&bslash;&quot;&quot;
)paren
comma
id|PARSE_OPT_NOARG
op_or
id|PARSE_OPT_NONEG
comma
l_int|NULL
comma
id|MARK_FLAG
)brace
comma
(brace
id|OPTION_SET_INT
comma
l_int|0
comma
l_string|&quot;no-assume-unchanged&quot;
comma
op_amp
id|mark_valid_only
comma
l_int|NULL
comma
id|N_
c_func
(paren
l_string|&quot;clear assumed-unchanged bit&quot;
)paren
comma
id|PARSE_OPT_NOARG
op_or
id|PARSE_OPT_NONEG
comma
l_int|NULL
comma
id|UNMARK_FLAG
)brace
comma
(brace
id|OPTION_SET_INT
comma
l_int|0
comma
l_string|&quot;skip-worktree&quot;
comma
op_amp
id|mark_skip_worktree_only
comma
l_int|NULL
comma
id|N_
c_func
(paren
l_string|&quot;mark files as &bslash;&quot;index-only&bslash;&quot;&quot;
)paren
comma
id|PARSE_OPT_NOARG
op_or
id|PARSE_OPT_NONEG
comma
l_int|NULL
comma
id|MARK_FLAG
)brace
comma
(brace
id|OPTION_SET_INT
comma
l_int|0
comma
l_string|&quot;no-skip-worktree&quot;
comma
op_amp
id|mark_skip_worktree_only
comma
l_int|NULL
comma
id|N_
c_func
(paren
l_string|&quot;clear skip-worktree bit&quot;
)paren
comma
id|PARSE_OPT_NOARG
op_or
id|PARSE_OPT_NONEG
comma
l_int|NULL
comma
id|UNMARK_FLAG
)brace
comma
id|OPT_SET_INT
c_func
(paren
l_int|0
comma
l_string|&quot;info-only&quot;
comma
op_amp
id|info_only
comma
id|N_
c_func
(paren
l_string|&quot;add to index only; do not add content to object database&quot;
)paren
comma
l_int|1
)paren
comma
id|OPT_SET_INT
c_func
(paren
l_int|0
comma
l_string|&quot;force-remove&quot;
comma
op_amp
id|force_remove
comma
id|N_
c_func
(paren
l_string|&quot;remove named paths even if present in worktree&quot;
)paren
comma
l_int|1
)paren
comma
id|OPT_SET_INT
c_func
(paren
l_char|&squot;z&squot;
comma
l_int|NULL
comma
op_amp
id|line_termination
comma
id|N_
c_func
(paren
l_string|&quot;with --stdin: input lines are terminated by null bytes&quot;
)paren
comma
l_char|&squot;&bslash;0&squot;
)paren
comma
(brace
id|OPTION_LOWLEVEL_CALLBACK
comma
l_int|0
comma
l_string|&quot;stdin&quot;
comma
op_amp
id|read_from_stdin
comma
l_int|NULL
comma
id|N_
c_func
(paren
l_string|&quot;read list of paths to be updated from standard input&quot;
)paren
comma
id|PARSE_OPT_NONEG
op_or
id|PARSE_OPT_NOARG
comma
(paren
id|parse_opt_cb
op_star
)paren
id|stdin_callback
)brace
comma
(brace
id|OPTION_LOWLEVEL_CALLBACK
comma
l_int|0
comma
l_string|&quot;index-info&quot;
comma
op_amp
id|line_termination
comma
l_int|NULL
comma
id|N_
c_func
(paren
l_string|&quot;add entries from standard input to the index&quot;
)paren
comma
id|PARSE_OPT_NONEG
op_or
id|PARSE_OPT_NOARG
comma
(paren
id|parse_opt_cb
op_star
)paren
id|stdin_cacheinfo_callback
)brace
comma
(brace
id|OPTION_LOWLEVEL_CALLBACK
comma
l_int|0
comma
l_string|&quot;unresolve&quot;
comma
op_amp
id|has_errors
comma
l_int|NULL
comma
id|N_
c_func
(paren
l_string|&quot;repopulate stages #2 and #3 for the listed paths&quot;
)paren
comma
id|PARSE_OPT_NONEG
op_or
id|PARSE_OPT_NOARG
comma
(paren
id|parse_opt_cb
op_star
)paren
id|unresolve_callback
)brace
comma
(brace
id|OPTION_LOWLEVEL_CALLBACK
comma
l_char|&squot;g&squot;
comma
l_string|&quot;again&quot;
comma
op_amp
id|has_errors
comma
l_int|NULL
comma
id|N_
c_func
(paren
l_string|&quot;only update entries that differ from HEAD&quot;
)paren
comma
id|PARSE_OPT_NONEG
op_or
id|PARSE_OPT_NOARG
comma
(paren
id|parse_opt_cb
op_star
)paren
id|reupdate_callback
)brace
comma
id|OPT_BIT
c_func
(paren
l_int|0
comma
l_string|&quot;ignore-missing&quot;
comma
op_amp
id|refresh_args.flags
comma
id|N_
c_func
(paren
l_string|&quot;ignore files missing from worktree&quot;
)paren
comma
id|REFRESH_IGNORE_MISSING
)paren
comma
id|OPT_SET_INT
c_func
(paren
l_int|0
comma
l_string|&quot;verbose&quot;
comma
op_amp
id|verbose
comma
id|N_
c_func
(paren
l_string|&quot;report actions to standard output&quot;
)paren
comma
l_int|1
)paren
comma
(brace
id|OPTION_CALLBACK
comma
l_int|0
comma
l_string|&quot;clear-resolve-undo&quot;
comma
l_int|NULL
comma
l_int|NULL
comma
id|N_
c_func
(paren
l_string|&quot;(for porcelains) forget saved unresolved conflicts&quot;
)paren
comma
id|PARSE_OPT_NOARG
op_or
id|PARSE_OPT_NONEG
comma
id|resolve_undo_clear_callback
)brace
comma
id|OPT_INTEGER
c_func
(paren
l_int|0
comma
l_string|&quot;index-version&quot;
comma
op_amp
id|preferred_index_format
comma
id|N_
c_func
(paren
l_string|&quot;write index in this format&quot;
)paren
)paren
comma
id|OPT_BOOL
c_func
(paren
l_int|0
comma
l_string|&quot;split-index&quot;
comma
op_amp
id|split_index
comma
id|N_
c_func
(paren
l_string|&quot;enable or disable split index&quot;
)paren
)paren
comma
id|OPT_BOOL
c_func
(paren
l_int|0
comma
l_string|&quot;untracked-cache&quot;
comma
op_amp
id|untracked_cache
comma
id|N_
c_func
(paren
l_string|&quot;enable/disable untracked cache&quot;
)paren
)paren
comma
id|OPT_SET_INT
c_func
(paren
l_int|0
comma
l_string|&quot;force-untracked-cache&quot;
comma
op_amp
id|untracked_cache
comma
id|N_
c_func
(paren
l_string|&quot;enable untracked cache without testing the filesystem&quot;
)paren
comma
l_int|2
)paren
comma
id|OPT_END
c_func
(paren
)paren
)brace
suffix:semicolon
r_if
c_cond
(paren
id|argc
op_eq
l_int|2
op_logical_and
op_logical_neg
id|strcmp
c_func
(paren
id|argv
(braket
l_int|1
)braket
comma
l_string|&quot;-h&quot;
)paren
)paren
id|usage_with_options
c_func
(paren
id|update_index_usage
comma
id|options
)paren
suffix:semicolon
id|git_config
c_func
(paren
id|git_default_config
comma
l_int|NULL
)paren
suffix:semicolon
multiline_comment|/* We can&squot;t free this memory, it becomes part of a linked list parsed atexit() */
id|lock_file
op_assign
id|xcalloc
c_func
(paren
l_int|1
comma
r_sizeof
(paren
r_struct
id|lock_file
)paren
)paren
suffix:semicolon
id|newfd
op_assign
id|hold_locked_index
c_func
(paren
id|lock_file
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
id|newfd
OL
l_int|0
)paren
id|lock_error
op_assign
id|errno
suffix:semicolon
id|entries
op_assign
id|read_cache
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|entries
OL
l_int|0
)paren
id|die
c_func
(paren
l_string|&quot;cache corrupted&quot;
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * Custom copy of parse_options() because we want to handle&n;&t; * filename arguments as they come.&n;&t; */
id|parse_options_start
c_func
(paren
op_amp
id|ctx
comma
id|argc
comma
id|argv
comma
id|prefix
comma
id|options
comma
id|PARSE_OPT_STOP_AT_NON_OPTION
)paren
suffix:semicolon
r_while
c_loop
(paren
id|ctx.argc
)paren
(brace
r_if
c_cond
(paren
id|parseopt_state
op_ne
id|PARSE_OPT_DONE
)paren
id|parseopt_state
op_assign
id|parse_options_step
c_func
(paren
op_amp
id|ctx
comma
id|options
comma
id|update_index_usage
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ctx.argc
)paren
r_break
suffix:semicolon
r_switch
c_cond
(paren
id|parseopt_state
)paren
(brace
r_case
id|PARSE_OPT_HELP
suffix:colon
m_exit
(paren
l_int|129
)paren
suffix:semicolon
r_case
id|PARSE_OPT_NON_OPTION
suffix:colon
r_case
id|PARSE_OPT_DONE
suffix:colon
(brace
r_const
r_char
op_star
id|path
op_assign
id|ctx.argv
(braket
l_int|0
)braket
suffix:semicolon
r_const
r_char
op_star
id|p
suffix:semicolon
id|setup_work_tree
c_func
(paren
)paren
suffix:semicolon
id|p
op_assign
id|prefix_path
c_func
(paren
id|prefix
comma
id|prefix_length
comma
id|path
)paren
suffix:semicolon
id|update_one
c_func
(paren
id|p
)paren
suffix:semicolon
r_if
c_cond
(paren
id|set_executable_bit
)paren
id|chmod_path
c_func
(paren
id|set_executable_bit
comma
id|p
)paren
suffix:semicolon
id|free
c_func
(paren
(paren
r_char
op_star
)paren
id|p
)paren
suffix:semicolon
id|ctx.argc
op_decrement
suffix:semicolon
id|ctx.argv
op_increment
suffix:semicolon
r_break
suffix:semicolon
)brace
r_case
id|PARSE_OPT_UNKNOWN
suffix:colon
r_if
c_cond
(paren
id|ctx.argv
(braket
l_int|0
)braket
(braket
l_int|1
)braket
op_eq
l_char|&squot;-&squot;
)paren
id|error
c_func
(paren
l_string|&quot;unknown option &squot;%s&squot;&quot;
comma
id|ctx.argv
(braket
l_int|0
)braket
op_plus
l_int|2
)paren
suffix:semicolon
r_else
id|error
c_func
(paren
l_string|&quot;unknown switch &squot;%c&squot;&quot;
comma
op_star
id|ctx.opt
)paren
suffix:semicolon
id|usage_with_options
c_func
(paren
id|update_index_usage
comma
id|options
)paren
suffix:semicolon
)brace
)brace
id|argc
op_assign
id|parse_options_end
c_func
(paren
op_amp
id|ctx
)paren
suffix:semicolon
r_if
c_cond
(paren
id|preferred_index_format
)paren
(brace
r_if
c_cond
(paren
id|preferred_index_format
OL
id|INDEX_FORMAT_LB
op_logical_or
id|INDEX_FORMAT_UB
OL
id|preferred_index_format
)paren
id|die
c_func
(paren
l_string|&quot;index-version %d not in range: %d..%d&quot;
comma
id|preferred_index_format
comma
id|INDEX_FORMAT_LB
comma
id|INDEX_FORMAT_UB
)paren
suffix:semicolon
r_if
c_cond
(paren
id|the_index.version
op_ne
id|preferred_index_format
)paren
id|active_cache_changed
op_or_assign
id|SOMETHING_CHANGED
suffix:semicolon
id|the_index.version
op_assign
id|preferred_index_format
suffix:semicolon
)brace
r_if
c_cond
(paren
id|read_from_stdin
)paren
(brace
r_struct
id|strbuf
id|buf
op_assign
id|STRBUF_INIT
comma
id|nbuf
op_assign
id|STRBUF_INIT
suffix:semicolon
id|setup_work_tree
c_func
(paren
)paren
suffix:semicolon
r_while
c_loop
(paren
id|strbuf_getline
c_func
(paren
op_amp
id|buf
comma
id|stdin
comma
id|line_termination
)paren
op_ne
id|EOF
)paren
(brace
r_const
r_char
op_star
id|p
suffix:semicolon
r_if
c_cond
(paren
id|line_termination
op_logical_and
id|buf.buf
(braket
l_int|0
)braket
op_eq
l_char|&squot;&quot;&squot;
)paren
(brace
id|strbuf_reset
c_func
(paren
op_amp
id|nbuf
)paren
suffix:semicolon
r_if
c_cond
(paren
id|unquote_c_style
c_func
(paren
op_amp
id|nbuf
comma
id|buf.buf
comma
l_int|NULL
)paren
)paren
id|die
c_func
(paren
l_string|&quot;line is badly quoted&quot;
)paren
suffix:semicolon
id|strbuf_swap
c_func
(paren
op_amp
id|buf
comma
op_amp
id|nbuf
)paren
suffix:semicolon
)brace
id|p
op_assign
id|prefix_path
c_func
(paren
id|prefix
comma
id|prefix_length
comma
id|buf.buf
)paren
suffix:semicolon
id|update_one
c_func
(paren
id|p
)paren
suffix:semicolon
r_if
c_cond
(paren
id|set_executable_bit
)paren
id|chmod_path
c_func
(paren
id|set_executable_bit
comma
id|p
)paren
suffix:semicolon
id|free
c_func
(paren
(paren
r_char
op_star
)paren
id|p
)paren
suffix:semicolon
)brace
id|strbuf_release
c_func
(paren
op_amp
id|nbuf
)paren
suffix:semicolon
id|strbuf_release
c_func
(paren
op_amp
id|buf
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|split_index
OG
l_int|0
)paren
(brace
id|init_split_index
c_func
(paren
op_amp
id|the_index
)paren
suffix:semicolon
id|the_index.cache_changed
op_or_assign
id|SPLIT_INDEX_ORDERED
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
op_logical_neg
id|split_index
op_logical_and
id|the_index.split_index
)paren
(brace
multiline_comment|/*&n;&t;&t; * can&squot;t discard_split_index(&amp;the_index); because that&n;&t;&t; * will destroy split_index-&gt;base-&gt;cache[], which may&n;&t;&t; * be shared with the_index.cache[]. So yeah we&squot;re&n;&t;&t; * leaking a bit here.&n;&t;&t; */
id|the_index.split_index
op_assign
l_int|NULL
suffix:semicolon
id|the_index.cache_changed
op_or_assign
id|SOMETHING_CHANGED
suffix:semicolon
)brace
r_if
c_cond
(paren
id|untracked_cache
OG
l_int|0
)paren
(brace
r_struct
id|untracked_cache
op_star
id|uc
suffix:semicolon
r_if
c_cond
(paren
id|untracked_cache
OL
l_int|2
)paren
(brace
id|setup_work_tree
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|test_if_untracked_cache_is_supported
c_func
(paren
)paren
)paren
r_return
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|the_index.untracked
)paren
(brace
id|uc
op_assign
id|xcalloc
c_func
(paren
l_int|1
comma
r_sizeof
(paren
op_star
id|uc
)paren
)paren
suffix:semicolon
id|strbuf_init
c_func
(paren
op_amp
id|uc-&gt;ident
comma
l_int|100
)paren
suffix:semicolon
id|uc-&gt;exclude_per_dir
op_assign
l_string|&quot;.gitignore&quot;
suffix:semicolon
multiline_comment|/* should be the same flags used by git-status */
id|uc-&gt;dir_flags
op_assign
id|DIR_SHOW_OTHER_DIRECTORIES
op_or
id|DIR_HIDE_EMPTY_DIRECTORIES
suffix:semicolon
id|the_index.untracked
op_assign
id|uc
suffix:semicolon
)brace
id|add_untracked_ident
c_func
(paren
id|the_index.untracked
)paren
suffix:semicolon
id|the_index.cache_changed
op_or_assign
id|UNTRACKED_CHANGED
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
op_logical_neg
id|untracked_cache
op_logical_and
id|the_index.untracked
)paren
(brace
id|the_index.untracked
op_assign
l_int|NULL
suffix:semicolon
id|the_index.cache_changed
op_or_assign
id|UNTRACKED_CHANGED
suffix:semicolon
)brace
r_if
c_cond
(paren
id|active_cache_changed
)paren
(brace
r_if
c_cond
(paren
id|newfd
OL
l_int|0
)paren
(brace
r_if
c_cond
(paren
id|refresh_args.flags
op_amp
id|REFRESH_QUIET
)paren
m_exit
(paren
l_int|128
)paren
suffix:semicolon
id|unable_to_lock_die
c_func
(paren
id|get_index_file
c_func
(paren
)paren
comma
id|lock_error
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|write_locked_index
c_func
(paren
op_amp
id|the_index
comma
id|lock_file
comma
id|COMMIT_LOCK
)paren
)paren
id|die
c_func
(paren
l_string|&quot;Unable to write new index file&quot;
)paren
suffix:semicolon
)brace
id|rollback_lock_file
c_func
(paren
id|lock_file
)paren
suffix:semicolon
r_return
id|has_errors
ques
c_cond
l_int|1
suffix:colon
l_int|0
suffix:semicolon
)brace
eof
