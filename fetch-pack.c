macro_line|#include &quot;cache.h&quot;
macro_line|#include &quot;refs.h&quot;
macro_line|#include &quot;pkt-line.h&quot;
macro_line|#include &quot;commit.h&quot;
macro_line|#include &quot;tag.h&quot;
macro_line|#include &quot;exec_cmd.h&quot;
macro_line|#include &quot;pack.h&quot;
macro_line|#include &quot;sideband.h&quot;
macro_line|#include &quot;fetch-pack.h&quot;
macro_line|#include &quot;remote.h&quot;
macro_line|#include &quot;run-command.h&quot;
macro_line|#include &quot;connect.h&quot;
macro_line|#include &quot;transport.h&quot;
macro_line|#include &quot;version.h&quot;
macro_line|#include &quot;prio-queue.h&quot;
macro_line|#include &quot;sha1-array.h&quot;
DECL|variable|transfer_unpack_limit
r_static
r_int
id|transfer_unpack_limit
op_assign
l_int|1
suffix:semicolon
DECL|variable|fetch_unpack_limit
r_static
r_int
id|fetch_unpack_limit
op_assign
l_int|1
suffix:semicolon
DECL|variable|unpack_limit
r_static
r_int
id|unpack_limit
op_assign
l_int|100
suffix:semicolon
DECL|variable|prefer_ofs_delta
r_static
r_int
id|prefer_ofs_delta
op_assign
l_int|1
suffix:semicolon
DECL|variable|no_done
r_static
r_int
id|no_done
suffix:semicolon
DECL|variable|fetch_fsck_objects
r_static
r_int
id|fetch_fsck_objects
op_assign
l_int|1
suffix:semicolon
DECL|variable|transfer_fsck_objects
r_static
r_int
id|transfer_fsck_objects
op_assign
l_int|1
suffix:semicolon
DECL|variable|agent_supported
r_static
r_int
id|agent_supported
suffix:semicolon
DECL|variable|shallow_lock
r_static
r_struct
id|lock_file
id|shallow_lock
suffix:semicolon
DECL|variable|alternate_shallow_file
r_static
r_const
r_char
op_star
id|alternate_shallow_file
suffix:semicolon
DECL|macro|COMPLETE
mdefine_line|#define COMPLETE&t;(1U &lt;&lt; 0)
DECL|macro|COMMON
mdefine_line|#define COMMON&t;&t;(1U &lt;&lt; 1)
DECL|macro|COMMON_REF
mdefine_line|#define COMMON_REF&t;(1U &lt;&lt; 2)
DECL|macro|SEEN
mdefine_line|#define SEEN&t;&t;(1U &lt;&lt; 3)
DECL|macro|POPPED
mdefine_line|#define POPPED&t;&t;(1U &lt;&lt; 4)
DECL|variable|marked
r_static
r_int
id|marked
suffix:semicolon
multiline_comment|/*&n; * After sending this many &quot;have&quot;s if we do not get any new ACK , we&n; * give up traversing our history.&n; */
DECL|macro|MAX_IN_VAIN
mdefine_line|#define MAX_IN_VAIN 256
DECL|variable|rev_list
r_static
r_struct
id|prio_queue
id|rev_list
op_assign
(brace
id|compare_commits_by_commit_date
)brace
suffix:semicolon
DECL|variable|non_common_revs
DECL|variable|multi_ack
DECL|variable|use_sideband
DECL|variable|allow_tip_sha1_in_want
r_static
r_int
id|non_common_revs
comma
id|multi_ack
comma
id|use_sideband
comma
id|allow_tip_sha1_in_want
suffix:semicolon
DECL|function|rev_list_push
r_static
r_void
id|rev_list_push
c_func
(paren
r_struct
id|commit
op_star
id|commit
comma
r_int
id|mark
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|commit-&gt;object.flags
op_amp
id|mark
)paren
)paren
(brace
id|commit-&gt;object.flags
op_or_assign
id|mark
suffix:semicolon
r_if
c_cond
(paren
id|parse_commit
c_func
(paren
id|commit
)paren
)paren
r_return
suffix:semicolon
id|prio_queue_put
c_func
(paren
op_amp
id|rev_list
comma
id|commit
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|commit-&gt;object.flags
op_amp
id|COMMON
)paren
)paren
id|non_common_revs
op_increment
suffix:semicolon
)brace
)brace
DECL|function|rev_list_insert_ref
r_static
r_int
id|rev_list_insert_ref
c_func
(paren
r_const
r_char
op_star
id|refname
comma
r_const
r_int
r_char
op_star
id|sha1
comma
r_int
id|flag
comma
r_void
op_star
id|cb_data
)paren
(brace
r_struct
id|object
op_star
id|o
op_assign
id|deref_tag
c_func
(paren
id|parse_object
c_func
(paren
id|sha1
)paren
comma
id|refname
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
id|o
op_logical_and
id|o-&gt;type
op_eq
id|OBJ_COMMIT
)paren
id|rev_list_push
c_func
(paren
(paren
r_struct
id|commit
op_star
)paren
id|o
comma
id|SEEN
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|clear_marks
r_static
r_int
id|clear_marks
c_func
(paren
r_const
r_char
op_star
id|refname
comma
r_const
r_int
r_char
op_star
id|sha1
comma
r_int
id|flag
comma
r_void
op_star
id|cb_data
)paren
(brace
r_struct
id|object
op_star
id|o
op_assign
id|deref_tag
c_func
(paren
id|parse_object
c_func
(paren
id|sha1
)paren
comma
id|refname
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
id|o
op_logical_and
id|o-&gt;type
op_eq
id|OBJ_COMMIT
)paren
id|clear_commit_marks
c_func
(paren
(paren
r_struct
id|commit
op_star
)paren
id|o
comma
id|COMMON
op_or
id|COMMON_REF
op_or
id|SEEN
op_or
id|POPPED
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n;   This function marks a rev and its ancestors as common.&n;   In some cases, it is desirable to mark only the ancestors (for example&n;   when only the server does not yet know that they are common).&n;*/
DECL|function|mark_common
r_static
r_void
id|mark_common
c_func
(paren
r_struct
id|commit
op_star
id|commit
comma
r_int
id|ancestors_only
comma
r_int
id|dont_parse
)paren
(brace
r_if
c_cond
(paren
id|commit
op_ne
l_int|NULL
op_logical_and
op_logical_neg
(paren
id|commit-&gt;object.flags
op_amp
id|COMMON
)paren
)paren
(brace
r_struct
id|object
op_star
id|o
op_assign
(paren
r_struct
id|object
op_star
)paren
id|commit
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ancestors_only
)paren
id|o-&gt;flags
op_or_assign
id|COMMON
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|o-&gt;flags
op_amp
id|SEEN
)paren
)paren
id|rev_list_push
c_func
(paren
id|commit
comma
id|SEEN
)paren
suffix:semicolon
r_else
(brace
r_struct
id|commit_list
op_star
id|parents
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ancestors_only
op_logical_and
op_logical_neg
(paren
id|o-&gt;flags
op_amp
id|POPPED
)paren
)paren
id|non_common_revs
op_decrement
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|o-&gt;parsed
op_logical_and
op_logical_neg
id|dont_parse
)paren
r_if
c_cond
(paren
id|parse_commit
c_func
(paren
id|commit
)paren
)paren
r_return
suffix:semicolon
r_for
c_loop
(paren
id|parents
op_assign
id|commit-&gt;parents
suffix:semicolon
id|parents
suffix:semicolon
id|parents
op_assign
id|parents-&gt;next
)paren
id|mark_common
c_func
(paren
id|parents-&gt;item
comma
l_int|0
comma
id|dont_parse
)paren
suffix:semicolon
)brace
)brace
)brace
multiline_comment|/*&n;  Get the next rev to send, ignoring the common.&n;*/
DECL|function|get_rev
r_static
r_const
r_int
r_char
op_star
id|get_rev
c_func
(paren
r_void
)paren
(brace
r_struct
id|commit
op_star
id|commit
op_assign
l_int|NULL
suffix:semicolon
r_while
c_loop
(paren
id|commit
op_eq
l_int|NULL
)paren
(brace
r_int
r_int
id|mark
suffix:semicolon
r_struct
id|commit_list
op_star
id|parents
suffix:semicolon
r_if
c_cond
(paren
id|rev_list.nr
op_eq
l_int|0
op_logical_or
id|non_common_revs
op_eq
l_int|0
)paren
r_return
l_int|NULL
suffix:semicolon
id|commit
op_assign
id|prio_queue_get
c_func
(paren
op_amp
id|rev_list
)paren
suffix:semicolon
id|parse_commit
c_func
(paren
id|commit
)paren
suffix:semicolon
id|parents
op_assign
id|commit-&gt;parents
suffix:semicolon
id|commit-&gt;object.flags
op_or_assign
id|POPPED
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|commit-&gt;object.flags
op_amp
id|COMMON
)paren
)paren
id|non_common_revs
op_decrement
suffix:semicolon
r_if
c_cond
(paren
id|commit-&gt;object.flags
op_amp
id|COMMON
)paren
(brace
multiline_comment|/* do not send &quot;have&quot;, and ignore ancestors */
id|commit
op_assign
l_int|NULL
suffix:semicolon
id|mark
op_assign
id|COMMON
op_or
id|SEEN
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|commit-&gt;object.flags
op_amp
id|COMMON_REF
)paren
multiline_comment|/* send &quot;have&quot;, and ignore ancestors */
id|mark
op_assign
id|COMMON
op_or
id|SEEN
suffix:semicolon
r_else
multiline_comment|/* send &quot;have&quot;, also for its ancestors */
id|mark
op_assign
id|SEEN
suffix:semicolon
r_while
c_loop
(paren
id|parents
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|parents-&gt;item-&gt;object.flags
op_amp
id|SEEN
)paren
)paren
id|rev_list_push
c_func
(paren
id|parents-&gt;item
comma
id|mark
)paren
suffix:semicolon
r_if
c_cond
(paren
id|mark
op_amp
id|COMMON
)paren
id|mark_common
c_func
(paren
id|parents-&gt;item
comma
l_int|1
comma
l_int|0
)paren
suffix:semicolon
id|parents
op_assign
id|parents-&gt;next
suffix:semicolon
)brace
)brace
r_return
id|commit-&gt;object.sha1
suffix:semicolon
)brace
DECL|enum|ack_type
r_enum
id|ack_type
(brace
DECL|enumerator|NAK
id|NAK
op_assign
l_int|0
comma
DECL|enumerator|ACK
id|ACK
comma
DECL|enumerator|ACK_continue
id|ACK_continue
comma
DECL|enumerator|ACK_common
id|ACK_common
comma
DECL|enumerator|ACK_ready
id|ACK_ready
)brace
suffix:semicolon
DECL|function|consume_shallow_list
r_static
r_void
id|consume_shallow_list
c_func
(paren
r_struct
id|fetch_pack_args
op_star
id|args
comma
r_int
id|fd
)paren
(brace
r_if
c_cond
(paren
id|args-&gt;stateless_rpc
op_logical_and
id|args-&gt;depth
OG
l_int|0
)paren
(brace
multiline_comment|/* If we sent a depth we will get back &quot;duplicate&quot;&n;&t;&t; * shallow and unshallow commands every time there&n;&t;&t; * is a block of have lines exchanged.&n;&t;&t; */
r_char
op_star
id|line
suffix:semicolon
r_while
c_loop
(paren
(paren
id|line
op_assign
id|packet_read_line
c_func
(paren
id|fd
comma
l_int|NULL
)paren
)paren
)paren
(brace
r_if
c_cond
(paren
id|starts_with
c_func
(paren
id|line
comma
l_string|&quot;shallow &quot;
)paren
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|starts_with
c_func
(paren
id|line
comma
l_string|&quot;unshallow &quot;
)paren
)paren
r_continue
suffix:semicolon
id|die
c_func
(paren
l_string|&quot;git fetch-pack: expected shallow list&quot;
)paren
suffix:semicolon
)brace
)brace
)brace
DECL|function|get_ack
r_static
r_enum
id|ack_type
id|get_ack
c_func
(paren
r_int
id|fd
comma
r_int
r_char
op_star
id|result_sha1
)paren
(brace
r_int
id|len
suffix:semicolon
r_char
op_star
id|line
op_assign
id|packet_read_line
c_func
(paren
id|fd
comma
op_amp
id|len
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|len
)paren
id|die
c_func
(paren
l_string|&quot;git fetch-pack: expected ACK/NAK, got EOF&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|strcmp
c_func
(paren
id|line
comma
l_string|&quot;NAK&quot;
)paren
)paren
r_return
id|NAK
suffix:semicolon
r_if
c_cond
(paren
id|starts_with
c_func
(paren
id|line
comma
l_string|&quot;ACK &quot;
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|get_sha1_hex
c_func
(paren
id|line
op_plus
l_int|4
comma
id|result_sha1
)paren
)paren
(brace
r_if
c_cond
(paren
id|len
OL
l_int|45
)paren
r_return
id|ACK
suffix:semicolon
r_if
c_cond
(paren
id|strstr
c_func
(paren
id|line
op_plus
l_int|45
comma
l_string|&quot;continue&quot;
)paren
)paren
r_return
id|ACK_continue
suffix:semicolon
r_if
c_cond
(paren
id|strstr
c_func
(paren
id|line
op_plus
l_int|45
comma
l_string|&quot;common&quot;
)paren
)paren
r_return
id|ACK_common
suffix:semicolon
r_if
c_cond
(paren
id|strstr
c_func
(paren
id|line
op_plus
l_int|45
comma
l_string|&quot;ready&quot;
)paren
)paren
r_return
id|ACK_ready
suffix:semicolon
r_return
id|ACK
suffix:semicolon
)brace
)brace
id|die
c_func
(paren
l_string|&quot;git fetch_pack: expected ACK/NAK, got &squot;%s&squot;&quot;
comma
id|line
)paren
suffix:semicolon
)brace
DECL|function|send_request
r_static
r_void
id|send_request
c_func
(paren
r_struct
id|fetch_pack_args
op_star
id|args
comma
r_int
id|fd
comma
r_struct
id|strbuf
op_star
id|buf
)paren
(brace
r_if
c_cond
(paren
id|args-&gt;stateless_rpc
)paren
(brace
id|send_sideband
c_func
(paren
id|fd
comma
l_int|1
comma
id|buf-&gt;buf
comma
id|buf-&gt;len
comma
id|LARGE_PACKET_MAX
)paren
suffix:semicolon
id|packet_flush
c_func
(paren
id|fd
)paren
suffix:semicolon
)brace
r_else
id|write_or_die
c_func
(paren
id|fd
comma
id|buf-&gt;buf
comma
id|buf-&gt;len
)paren
suffix:semicolon
)brace
DECL|function|insert_one_alternate_ref
r_static
r_void
id|insert_one_alternate_ref
c_func
(paren
r_const
r_struct
id|ref
op_star
id|ref
comma
r_void
op_star
id|unused
)paren
(brace
id|rev_list_insert_ref
c_func
(paren
l_int|NULL
comma
id|ref-&gt;old_sha1
comma
l_int|0
comma
l_int|NULL
)paren
suffix:semicolon
)brace
DECL|macro|INITIAL_FLUSH
mdefine_line|#define INITIAL_FLUSH 16
DECL|macro|PIPESAFE_FLUSH
mdefine_line|#define PIPESAFE_FLUSH 32
DECL|macro|LARGE_FLUSH
mdefine_line|#define LARGE_FLUSH 1024
DECL|function|next_flush
r_static
r_int
id|next_flush
c_func
(paren
r_struct
id|fetch_pack_args
op_star
id|args
comma
r_int
id|count
)paren
(brace
r_int
id|flush_limit
op_assign
id|args-&gt;stateless_rpc
ques
c_cond
id|LARGE_FLUSH
suffix:colon
id|PIPESAFE_FLUSH
suffix:semicolon
r_if
c_cond
(paren
id|count
OL
id|flush_limit
)paren
id|count
op_lshift_assign
l_int|1
suffix:semicolon
r_else
id|count
op_add_assign
id|flush_limit
suffix:semicolon
r_return
id|count
suffix:semicolon
)brace
DECL|function|find_common
r_static
r_int
id|find_common
c_func
(paren
r_struct
id|fetch_pack_args
op_star
id|args
comma
r_int
id|fd
(braket
l_int|2
)braket
comma
r_int
r_char
op_star
id|result_sha1
comma
r_struct
id|ref
op_star
id|refs
)paren
(brace
r_int
id|fetching
suffix:semicolon
r_int
id|count
op_assign
l_int|0
comma
id|flushes
op_assign
l_int|0
comma
id|flush_at
op_assign
id|INITIAL_FLUSH
comma
id|retval
suffix:semicolon
r_const
r_int
r_char
op_star
id|sha1
suffix:semicolon
r_int
id|in_vain
op_assign
l_int|0
suffix:semicolon
r_int
id|got_continue
op_assign
l_int|0
suffix:semicolon
r_int
id|got_ready
op_assign
l_int|0
suffix:semicolon
r_struct
id|strbuf
id|req_buf
op_assign
id|STRBUF_INIT
suffix:semicolon
r_int
id|state_len
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;stateless_rpc
op_logical_and
id|multi_ack
op_eq
l_int|1
)paren
id|die
c_func
(paren
l_string|&quot;--stateless-rpc requires multi_ack_detailed&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|marked
)paren
id|for_each_ref
c_func
(paren
id|clear_marks
comma
l_int|NULL
)paren
suffix:semicolon
id|marked
op_assign
l_int|1
suffix:semicolon
id|for_each_ref
c_func
(paren
id|rev_list_insert_ref
comma
l_int|NULL
)paren
suffix:semicolon
id|for_each_alternate_ref
c_func
(paren
id|insert_one_alternate_ref
comma
l_int|NULL
)paren
suffix:semicolon
id|fetching
op_assign
l_int|0
suffix:semicolon
r_for
c_loop
(paren
suffix:semicolon
id|refs
suffix:semicolon
id|refs
op_assign
id|refs-&gt;next
)paren
(brace
r_int
r_char
op_star
id|remote
op_assign
id|refs-&gt;old_sha1
suffix:semicolon
r_const
r_char
op_star
id|remote_hex
suffix:semicolon
r_struct
id|object
op_star
id|o
suffix:semicolon
multiline_comment|/*&n;&t;&t; * If that object is complete (i.e. it is an ancestor of a&n;&t;&t; * local ref), we tell them we have it but do not have to&n;&t;&t; * tell them about its ancestors, which they already know&n;&t;&t; * about.&n;&t;&t; *&n;&t;&t; * We use lookup_object here because we are only&n;&t;&t; * interested in the case we *know* the object is&n;&t;&t; * reachable and we have already scanned it.&n;&t;&t; */
r_if
c_cond
(paren
(paren
(paren
id|o
op_assign
id|lookup_object
c_func
(paren
id|remote
)paren
)paren
op_ne
l_int|NULL
)paren
op_logical_and
(paren
id|o-&gt;flags
op_amp
id|COMPLETE
)paren
)paren
(brace
r_continue
suffix:semicolon
)brace
id|remote_hex
op_assign
id|sha1_to_hex
c_func
(paren
id|remote
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|fetching
)paren
(brace
r_struct
id|strbuf
id|c
op_assign
id|STRBUF_INIT
suffix:semicolon
r_if
c_cond
(paren
id|multi_ack
op_eq
l_int|2
)paren
id|strbuf_addstr
c_func
(paren
op_amp
id|c
comma
l_string|&quot; multi_ack_detailed&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|multi_ack
op_eq
l_int|1
)paren
id|strbuf_addstr
c_func
(paren
op_amp
id|c
comma
l_string|&quot; multi_ack&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|no_done
)paren
id|strbuf_addstr
c_func
(paren
op_amp
id|c
comma
l_string|&quot; no-done&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|use_sideband
op_eq
l_int|2
)paren
id|strbuf_addstr
c_func
(paren
op_amp
id|c
comma
l_string|&quot; side-band-64k&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|use_sideband
op_eq
l_int|1
)paren
id|strbuf_addstr
c_func
(paren
op_amp
id|c
comma
l_string|&quot; side-band&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;use_thin_pack
)paren
id|strbuf_addstr
c_func
(paren
op_amp
id|c
comma
l_string|&quot; thin-pack&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;no_progress
)paren
id|strbuf_addstr
c_func
(paren
op_amp
id|c
comma
l_string|&quot; no-progress&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;include_tag
)paren
id|strbuf_addstr
c_func
(paren
op_amp
id|c
comma
l_string|&quot; include-tag&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|prefer_ofs_delta
)paren
id|strbuf_addstr
c_func
(paren
op_amp
id|c
comma
l_string|&quot; ofs-delta&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|agent_supported
)paren
id|strbuf_addf
c_func
(paren
op_amp
id|c
comma
l_string|&quot; agent=%s&quot;
comma
id|git_user_agent_sanitized
c_func
(paren
)paren
)paren
suffix:semicolon
id|packet_buf_write
c_func
(paren
op_amp
id|req_buf
comma
l_string|&quot;want %s%s&bslash;n&quot;
comma
id|remote_hex
comma
id|c.buf
)paren
suffix:semicolon
id|strbuf_release
c_func
(paren
op_amp
id|c
)paren
suffix:semicolon
)brace
r_else
id|packet_buf_write
c_func
(paren
op_amp
id|req_buf
comma
l_string|&quot;want %s&bslash;n&quot;
comma
id|remote_hex
)paren
suffix:semicolon
id|fetching
op_increment
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|fetching
)paren
(brace
id|strbuf_release
c_func
(paren
op_amp
id|req_buf
)paren
suffix:semicolon
id|packet_flush
c_func
(paren
id|fd
(braket
l_int|1
)braket
)paren
suffix:semicolon
r_return
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|is_repository_shallow
c_func
(paren
)paren
)paren
id|write_shallow_commits
c_func
(paren
op_amp
id|req_buf
comma
l_int|1
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;depth
OG
l_int|0
)paren
id|packet_buf_write
c_func
(paren
op_amp
id|req_buf
comma
l_string|&quot;deepen %d&quot;
comma
id|args-&gt;depth
)paren
suffix:semicolon
id|packet_buf_flush
c_func
(paren
op_amp
id|req_buf
)paren
suffix:semicolon
id|state_len
op_assign
id|req_buf.len
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;depth
OG
l_int|0
)paren
(brace
r_char
op_star
id|line
suffix:semicolon
r_int
r_char
id|sha1
(braket
l_int|20
)braket
suffix:semicolon
id|send_request
c_func
(paren
id|args
comma
id|fd
(braket
l_int|1
)braket
comma
op_amp
id|req_buf
)paren
suffix:semicolon
r_while
c_loop
(paren
(paren
id|line
op_assign
id|packet_read_line
c_func
(paren
id|fd
(braket
l_int|0
)braket
comma
l_int|NULL
)paren
)paren
)paren
(brace
r_if
c_cond
(paren
id|starts_with
c_func
(paren
id|line
comma
l_string|&quot;shallow &quot;
)paren
)paren
(brace
r_if
c_cond
(paren
id|get_sha1_hex
c_func
(paren
id|line
op_plus
l_int|8
comma
id|sha1
)paren
)paren
id|die
c_func
(paren
l_string|&quot;invalid shallow line: %s&quot;
comma
id|line
)paren
suffix:semicolon
id|register_shallow
c_func
(paren
id|sha1
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
id|starts_with
c_func
(paren
id|line
comma
l_string|&quot;unshallow &quot;
)paren
)paren
(brace
r_if
c_cond
(paren
id|get_sha1_hex
c_func
(paren
id|line
op_plus
l_int|10
comma
id|sha1
)paren
)paren
id|die
c_func
(paren
l_string|&quot;invalid unshallow line: %s&quot;
comma
id|line
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|lookup_object
c_func
(paren
id|sha1
)paren
)paren
id|die
c_func
(paren
l_string|&quot;object not found: %s&quot;
comma
id|line
)paren
suffix:semicolon
multiline_comment|/* make sure that it is parsed as shallow */
r_if
c_cond
(paren
op_logical_neg
id|parse_object
c_func
(paren
id|sha1
)paren
)paren
id|die
c_func
(paren
l_string|&quot;error in object: %s&quot;
comma
id|line
)paren
suffix:semicolon
r_if
c_cond
(paren
id|unregister_shallow
c_func
(paren
id|sha1
)paren
)paren
id|die
c_func
(paren
l_string|&quot;no shallow found: %s&quot;
comma
id|line
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
id|die
c_func
(paren
l_string|&quot;expected shallow/unshallow, got %s&quot;
comma
id|line
)paren
suffix:semicolon
)brace
)brace
r_else
r_if
c_cond
(paren
op_logical_neg
id|args-&gt;stateless_rpc
)paren
id|send_request
c_func
(paren
id|args
comma
id|fd
(braket
l_int|1
)braket
comma
op_amp
id|req_buf
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|args-&gt;stateless_rpc
)paren
(brace
multiline_comment|/* If we aren&squot;t using the stateless-rpc interface&n;&t;&t; * we don&squot;t need to retain the headers.&n;&t;&t; */
id|strbuf_setlen
c_func
(paren
op_amp
id|req_buf
comma
l_int|0
)paren
suffix:semicolon
id|state_len
op_assign
l_int|0
suffix:semicolon
)brace
id|flushes
op_assign
l_int|0
suffix:semicolon
id|retval
op_assign
l_int|1
suffix:semicolon
r_while
c_loop
(paren
(paren
id|sha1
op_assign
id|get_rev
c_func
(paren
)paren
)paren
)paren
(brace
id|packet_buf_write
c_func
(paren
op_amp
id|req_buf
comma
l_string|&quot;have %s&bslash;n&quot;
comma
id|sha1_to_hex
c_func
(paren
id|sha1
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;verbose
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;have %s&bslash;n&quot;
comma
id|sha1_to_hex
c_func
(paren
id|sha1
)paren
)paren
suffix:semicolon
id|in_vain
op_increment
suffix:semicolon
r_if
c_cond
(paren
id|flush_at
op_le
op_increment
id|count
)paren
(brace
r_int
id|ack
suffix:semicolon
id|packet_buf_flush
c_func
(paren
op_amp
id|req_buf
)paren
suffix:semicolon
id|send_request
c_func
(paren
id|args
comma
id|fd
(braket
l_int|1
)braket
comma
op_amp
id|req_buf
)paren
suffix:semicolon
id|strbuf_setlen
c_func
(paren
op_amp
id|req_buf
comma
id|state_len
)paren
suffix:semicolon
id|flushes
op_increment
suffix:semicolon
id|flush_at
op_assign
id|next_flush
c_func
(paren
id|args
comma
id|count
)paren
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * We keep one window &quot;ahead&quot; of the other side, and&n;&t;&t;&t; * will wait for an ACK only on the next one&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
id|args-&gt;stateless_rpc
op_logical_and
id|count
op_eq
id|INITIAL_FLUSH
)paren
r_continue
suffix:semicolon
id|consume_shallow_list
c_func
(paren
id|args
comma
id|fd
(braket
l_int|0
)braket
)paren
suffix:semicolon
r_do
(brace
id|ack
op_assign
id|get_ack
c_func
(paren
id|fd
(braket
l_int|0
)braket
comma
id|result_sha1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;verbose
op_logical_and
id|ack
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;got ack %d %s&bslash;n&quot;
comma
id|ack
comma
id|sha1_to_hex
c_func
(paren
id|result_sha1
)paren
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|ack
)paren
(brace
r_case
id|ACK
suffix:colon
id|flushes
op_assign
l_int|0
suffix:semicolon
id|multi_ack
op_assign
l_int|0
suffix:semicolon
id|retval
op_assign
l_int|0
suffix:semicolon
r_goto
id|done
suffix:semicolon
r_case
id|ACK_common
suffix:colon
r_case
id|ACK_ready
suffix:colon
r_case
id|ACK_continue
suffix:colon
(brace
r_struct
id|commit
op_star
id|commit
op_assign
id|lookup_commit
c_func
(paren
id|result_sha1
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|commit
)paren
id|die
c_func
(paren
l_string|&quot;invalid commit %s&quot;
comma
id|sha1_to_hex
c_func
(paren
id|result_sha1
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;stateless_rpc
op_logical_and
id|ack
op_eq
id|ACK_common
op_logical_and
op_logical_neg
(paren
id|commit-&gt;object.flags
op_amp
id|COMMON
)paren
)paren
(brace
multiline_comment|/* We need to replay the have for this object&n;&t;&t;&t;&t;&t;&t; * on the next RPC request so the peer knows&n;&t;&t;&t;&t;&t;&t; * it is in common with us.&n;&t;&t;&t;&t;&t;&t; */
r_const
r_char
op_star
id|hex
op_assign
id|sha1_to_hex
c_func
(paren
id|result_sha1
)paren
suffix:semicolon
id|packet_buf_write
c_func
(paren
op_amp
id|req_buf
comma
l_string|&quot;have %s&bslash;n&quot;
comma
id|hex
)paren
suffix:semicolon
id|state_len
op_assign
id|req_buf.len
suffix:semicolon
)brace
id|mark_common
c_func
(paren
id|commit
comma
l_int|0
comma
l_int|1
)paren
suffix:semicolon
id|retval
op_assign
l_int|0
suffix:semicolon
id|in_vain
op_assign
l_int|0
suffix:semicolon
id|got_continue
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|ack
op_eq
id|ACK_ready
)paren
(brace
id|clear_prio_queue
c_func
(paren
op_amp
id|rev_list
)paren
suffix:semicolon
id|got_ready
op_assign
l_int|1
suffix:semicolon
)brace
r_break
suffix:semicolon
)brace
)brace
)brace
r_while
c_loop
(paren
id|ack
)paren
suffix:semicolon
id|flushes
op_decrement
suffix:semicolon
r_if
c_cond
(paren
id|got_continue
op_logical_and
id|MAX_IN_VAIN
OL
id|in_vain
)paren
(brace
r_if
c_cond
(paren
id|args-&gt;verbose
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;giving up&bslash;n&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
multiline_comment|/* give up */
)brace
)brace
)brace
id|done
suffix:colon
r_if
c_cond
(paren
op_logical_neg
id|got_ready
op_logical_or
op_logical_neg
id|no_done
)paren
(brace
id|packet_buf_write
c_func
(paren
op_amp
id|req_buf
comma
l_string|&quot;done&bslash;n&quot;
)paren
suffix:semicolon
id|send_request
c_func
(paren
id|args
comma
id|fd
(braket
l_int|1
)braket
comma
op_amp
id|req_buf
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|args-&gt;verbose
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;done&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|retval
op_ne
l_int|0
)paren
(brace
id|multi_ack
op_assign
l_int|0
suffix:semicolon
id|flushes
op_increment
suffix:semicolon
)brace
id|strbuf_release
c_func
(paren
op_amp
id|req_buf
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|got_ready
op_logical_or
op_logical_neg
id|no_done
)paren
id|consume_shallow_list
c_func
(paren
id|args
comma
id|fd
(braket
l_int|0
)braket
)paren
suffix:semicolon
r_while
c_loop
(paren
id|flushes
op_logical_or
id|multi_ack
)paren
(brace
r_int
id|ack
op_assign
id|get_ack
c_func
(paren
id|fd
(braket
l_int|0
)braket
comma
id|result_sha1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ack
)paren
(brace
r_if
c_cond
(paren
id|args-&gt;verbose
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;got ack (%d) %s&bslash;n&quot;
comma
id|ack
comma
id|sha1_to_hex
c_func
(paren
id|result_sha1
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ack
op_eq
id|ACK
)paren
r_return
l_int|0
suffix:semicolon
id|multi_ack
op_assign
l_int|1
suffix:semicolon
r_continue
suffix:semicolon
)brace
id|flushes
op_decrement
suffix:semicolon
)brace
multiline_comment|/* it is no error to fetch into a completely empty repo */
r_return
id|count
ques
c_cond
id|retval
suffix:colon
l_int|0
suffix:semicolon
)brace
DECL|variable|complete
r_static
r_struct
id|commit_list
op_star
id|complete
suffix:semicolon
DECL|function|mark_complete
r_static
r_int
id|mark_complete
c_func
(paren
r_const
r_char
op_star
id|refname
comma
r_const
r_int
r_char
op_star
id|sha1
comma
r_int
id|flag
comma
r_void
op_star
id|cb_data
)paren
(brace
r_struct
id|object
op_star
id|o
op_assign
id|parse_object
c_func
(paren
id|sha1
)paren
suffix:semicolon
r_while
c_loop
(paren
id|o
op_logical_and
id|o-&gt;type
op_eq
id|OBJ_TAG
)paren
(brace
r_struct
id|tag
op_star
id|t
op_assign
(paren
r_struct
id|tag
op_star
)paren
id|o
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|t-&gt;tagged
)paren
r_break
suffix:semicolon
multiline_comment|/* broken repository */
id|o-&gt;flags
op_or_assign
id|COMPLETE
suffix:semicolon
id|o
op_assign
id|parse_object
c_func
(paren
id|t-&gt;tagged-&gt;sha1
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|o
op_logical_and
id|o-&gt;type
op_eq
id|OBJ_COMMIT
)paren
(brace
r_struct
id|commit
op_star
id|commit
op_assign
(paren
r_struct
id|commit
op_star
)paren
id|o
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|commit-&gt;object.flags
op_amp
id|COMPLETE
)paren
)paren
(brace
id|commit-&gt;object.flags
op_or_assign
id|COMPLETE
suffix:semicolon
id|commit_list_insert
c_func
(paren
id|commit
comma
op_amp
id|complete
)paren
suffix:semicolon
)brace
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|mark_recent_complete_commits
r_static
r_void
id|mark_recent_complete_commits
c_func
(paren
r_struct
id|fetch_pack_args
op_star
id|args
comma
r_int
r_int
id|cutoff
)paren
(brace
r_while
c_loop
(paren
id|complete
op_logical_and
id|cutoff
op_le
id|complete-&gt;item-&gt;date
)paren
(brace
r_if
c_cond
(paren
id|args-&gt;verbose
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;Marking %s as complete&bslash;n&quot;
comma
id|sha1_to_hex
c_func
(paren
id|complete-&gt;item-&gt;object.sha1
)paren
)paren
suffix:semicolon
id|pop_most_recent_commit
c_func
(paren
op_amp
id|complete
comma
id|COMPLETE
)paren
suffix:semicolon
)brace
)brace
DECL|function|filter_refs
r_static
r_void
id|filter_refs
c_func
(paren
r_struct
id|fetch_pack_args
op_star
id|args
comma
r_struct
id|ref
op_star
op_star
id|refs
comma
r_struct
id|ref
op_star
op_star
id|sought
comma
r_int
id|nr_sought
)paren
(brace
r_struct
id|ref
op_star
id|newlist
op_assign
l_int|NULL
suffix:semicolon
r_struct
id|ref
op_star
op_star
id|newtail
op_assign
op_amp
id|newlist
suffix:semicolon
r_struct
id|ref
op_star
id|ref
comma
op_star
id|next
suffix:semicolon
r_int
id|i
suffix:semicolon
id|i
op_assign
l_int|0
suffix:semicolon
r_for
c_loop
(paren
id|ref
op_assign
op_star
id|refs
suffix:semicolon
id|ref
suffix:semicolon
id|ref
op_assign
id|next
)paren
(brace
r_int
id|keep
op_assign
l_int|0
suffix:semicolon
id|next
op_assign
id|ref-&gt;next
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|memcmp
c_func
(paren
id|ref-&gt;name
comma
l_string|&quot;refs/&quot;
comma
l_int|5
)paren
op_logical_and
id|check_refname_format
c_func
(paren
id|ref-&gt;name
comma
l_int|0
)paren
)paren
suffix:semicolon
multiline_comment|/* trash */
r_else
(brace
r_while
c_loop
(paren
id|i
OL
id|nr_sought
)paren
(brace
r_int
id|cmp
op_assign
id|strcmp
c_func
(paren
id|ref-&gt;name
comma
id|sought
(braket
id|i
)braket
op_member_access_from_pointer
id|name
)paren
suffix:semicolon
r_if
c_cond
(paren
id|cmp
OL
l_int|0
)paren
r_break
suffix:semicolon
multiline_comment|/* definitely do not have it */
r_else
r_if
c_cond
(paren
id|cmp
op_eq
l_int|0
)paren
(brace
id|keep
op_assign
l_int|1
suffix:semicolon
multiline_comment|/* definitely have it */
id|sought
(braket
id|i
)braket
op_member_access_from_pointer
id|matched
op_assign
l_int|1
suffix:semicolon
)brace
id|i
op_increment
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
op_logical_neg
id|keep
op_logical_and
id|args-&gt;fetch_all
op_logical_and
(paren
op_logical_neg
id|args-&gt;depth
op_logical_or
op_logical_neg
id|starts_with
c_func
(paren
id|ref-&gt;name
comma
l_string|&quot;refs/tags/&quot;
)paren
)paren
)paren
id|keep
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|keep
)paren
(brace
op_star
id|newtail
op_assign
id|ref
suffix:semicolon
id|ref-&gt;next
op_assign
l_int|NULL
suffix:semicolon
id|newtail
op_assign
op_amp
id|ref-&gt;next
suffix:semicolon
)brace
r_else
(brace
id|free
c_func
(paren
id|ref
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/* Append unmatched requests to the list */
r_if
c_cond
(paren
id|allow_tip_sha1_in_want
)paren
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|nr_sought
suffix:semicolon
id|i
op_increment
)paren
(brace
id|ref
op_assign
id|sought
(braket
id|i
)braket
suffix:semicolon
r_if
c_cond
(paren
id|ref-&gt;matched
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|get_sha1_hex
c_func
(paren
id|ref-&gt;name
comma
id|ref-&gt;old_sha1
)paren
)paren
r_continue
suffix:semicolon
id|ref-&gt;matched
op_assign
l_int|1
suffix:semicolon
op_star
id|newtail
op_assign
id|ref
suffix:semicolon
id|ref-&gt;next
op_assign
l_int|NULL
suffix:semicolon
id|newtail
op_assign
op_amp
id|ref-&gt;next
suffix:semicolon
)brace
)brace
op_star
id|refs
op_assign
id|newlist
suffix:semicolon
)brace
DECL|function|mark_alternate_complete
r_static
r_void
id|mark_alternate_complete
c_func
(paren
r_const
r_struct
id|ref
op_star
id|ref
comma
r_void
op_star
id|unused
)paren
(brace
id|mark_complete
c_func
(paren
l_int|NULL
comma
id|ref-&gt;old_sha1
comma
l_int|0
comma
l_int|NULL
)paren
suffix:semicolon
)brace
DECL|function|everything_local
r_static
r_int
id|everything_local
c_func
(paren
r_struct
id|fetch_pack_args
op_star
id|args
comma
r_struct
id|ref
op_star
op_star
id|refs
comma
r_struct
id|ref
op_star
op_star
id|sought
comma
r_int
id|nr_sought
)paren
(brace
r_struct
id|ref
op_star
id|ref
suffix:semicolon
r_int
id|retval
suffix:semicolon
r_int
r_int
id|cutoff
op_assign
l_int|0
suffix:semicolon
id|save_commit_buffer
op_assign
l_int|0
suffix:semicolon
r_for
c_loop
(paren
id|ref
op_assign
op_star
id|refs
suffix:semicolon
id|ref
suffix:semicolon
id|ref
op_assign
id|ref-&gt;next
)paren
(brace
r_struct
id|object
op_star
id|o
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|has_sha1_file
c_func
(paren
id|ref-&gt;old_sha1
)paren
)paren
r_continue
suffix:semicolon
id|o
op_assign
id|parse_object
c_func
(paren
id|ref-&gt;old_sha1
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|o
)paren
r_continue
suffix:semicolon
multiline_comment|/* We already have it -- which may mean that we were&n;&t;&t; * in sync with the other side at some time after&n;&t;&t; * that (it is OK if we guess wrong here).&n;&t;&t; */
r_if
c_cond
(paren
id|o-&gt;type
op_eq
id|OBJ_COMMIT
)paren
(brace
r_struct
id|commit
op_star
id|commit
op_assign
(paren
r_struct
id|commit
op_star
)paren
id|o
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|cutoff
op_logical_or
id|cutoff
OL
id|commit-&gt;date
)paren
id|cutoff
op_assign
id|commit-&gt;date
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
op_logical_neg
id|args-&gt;depth
)paren
(brace
id|for_each_ref
c_func
(paren
id|mark_complete
comma
l_int|NULL
)paren
suffix:semicolon
id|for_each_alternate_ref
c_func
(paren
id|mark_alternate_complete
comma
l_int|NULL
)paren
suffix:semicolon
id|commit_list_sort_by_date
c_func
(paren
op_amp
id|complete
)paren
suffix:semicolon
r_if
c_cond
(paren
id|cutoff
)paren
id|mark_recent_complete_commits
c_func
(paren
id|args
comma
id|cutoff
)paren
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * Mark all complete remote refs as common refs.&n;&t; * Don&squot;t mark them common yet; the server has to be told so first.&n;&t; */
r_for
c_loop
(paren
id|ref
op_assign
op_star
id|refs
suffix:semicolon
id|ref
suffix:semicolon
id|ref
op_assign
id|ref-&gt;next
)paren
(brace
r_struct
id|object
op_star
id|o
op_assign
id|deref_tag
c_func
(paren
id|lookup_object
c_func
(paren
id|ref-&gt;old_sha1
)paren
comma
l_int|NULL
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|o
op_logical_or
id|o-&gt;type
op_ne
id|OBJ_COMMIT
op_logical_or
op_logical_neg
(paren
id|o-&gt;flags
op_amp
id|COMPLETE
)paren
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|o-&gt;flags
op_amp
id|SEEN
)paren
)paren
(brace
id|rev_list_push
c_func
(paren
(paren
r_struct
id|commit
op_star
)paren
id|o
comma
id|COMMON_REF
op_or
id|SEEN
)paren
suffix:semicolon
id|mark_common
c_func
(paren
(paren
r_struct
id|commit
op_star
)paren
id|o
comma
l_int|1
comma
l_int|1
)paren
suffix:semicolon
)brace
)brace
id|filter_refs
c_func
(paren
id|args
comma
id|refs
comma
id|sought
comma
id|nr_sought
)paren
suffix:semicolon
r_for
c_loop
(paren
id|retval
op_assign
l_int|1
comma
id|ref
op_assign
op_star
id|refs
suffix:semicolon
id|ref
suffix:semicolon
id|ref
op_assign
id|ref-&gt;next
)paren
(brace
r_const
r_int
r_char
op_star
id|remote
op_assign
id|ref-&gt;old_sha1
suffix:semicolon
r_int
r_char
id|local
(braket
l_int|20
)braket
suffix:semicolon
r_struct
id|object
op_star
id|o
suffix:semicolon
id|o
op_assign
id|lookup_object
c_func
(paren
id|remote
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|o
op_logical_or
op_logical_neg
(paren
id|o-&gt;flags
op_amp
id|COMPLETE
)paren
)paren
(brace
id|retval
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|args-&gt;verbose
)paren
r_continue
suffix:semicolon
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;want %s (%s)&bslash;n&quot;
comma
id|sha1_to_hex
c_func
(paren
id|remote
)paren
comma
id|ref-&gt;name
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
id|hashcpy
c_func
(paren
id|ref-&gt;new_sha1
comma
id|local
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|args-&gt;verbose
)paren
r_continue
suffix:semicolon
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;already have %s (%s)&bslash;n&quot;
comma
id|sha1_to_hex
c_func
(paren
id|remote
)paren
comma
id|ref-&gt;name
)paren
suffix:semicolon
)brace
r_return
id|retval
suffix:semicolon
)brace
DECL|function|sideband_demux
r_static
r_int
id|sideband_demux
c_func
(paren
r_int
id|in
comma
r_int
id|out
comma
r_void
op_star
id|data
)paren
(brace
r_int
op_star
id|xd
op_assign
id|data
suffix:semicolon
r_int
id|ret
op_assign
id|recv_sideband
c_func
(paren
l_string|&quot;fetch-pack&quot;
comma
id|xd
(braket
l_int|0
)braket
comma
id|out
)paren
suffix:semicolon
id|close
c_func
(paren
id|out
)paren
suffix:semicolon
r_return
id|ret
suffix:semicolon
)brace
DECL|function|get_pack
r_static
r_int
id|get_pack
c_func
(paren
r_struct
id|fetch_pack_args
op_star
id|args
comma
r_int
id|xd
(braket
l_int|2
)braket
comma
r_char
op_star
op_star
id|pack_lockfile
)paren
(brace
r_struct
id|async
id|demux
suffix:semicolon
r_const
r_char
op_star
id|argv
(braket
l_int|22
)braket
suffix:semicolon
r_char
id|keep_arg
(braket
l_int|256
)braket
suffix:semicolon
r_char
id|hdr_arg
(braket
l_int|256
)braket
suffix:semicolon
r_const
r_char
op_star
op_star
id|av
comma
op_star
id|cmd_name
suffix:semicolon
r_int
id|do_keep
op_assign
id|args-&gt;keep_pack
suffix:semicolon
r_struct
id|child_process
id|cmd
suffix:semicolon
r_int
id|ret
suffix:semicolon
id|memset
c_func
(paren
op_amp
id|demux
comma
l_int|0
comma
r_sizeof
(paren
id|demux
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|use_sideband
)paren
(brace
multiline_comment|/* xd[] is talking with upload-pack; subprocess reads from&n;&t;&t; * xd[0], spits out band#2 to stderr, and feeds us band#1&n;&t;&t; * through demux-&gt;out.&n;&t;&t; */
id|demux.proc
op_assign
id|sideband_demux
suffix:semicolon
id|demux.data
op_assign
id|xd
suffix:semicolon
id|demux.out
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|start_async
c_func
(paren
op_amp
id|demux
)paren
)paren
id|die
c_func
(paren
l_string|&quot;fetch-pack: unable to fork off sideband&quot;
l_string|&quot; demultiplexer&quot;
)paren
suffix:semicolon
)brace
r_else
id|demux.out
op_assign
id|xd
(braket
l_int|0
)braket
suffix:semicolon
id|memset
c_func
(paren
op_amp
id|cmd
comma
l_int|0
comma
r_sizeof
(paren
id|cmd
)paren
)paren
suffix:semicolon
id|cmd.argv
op_assign
id|argv
suffix:semicolon
id|av
op_assign
id|argv
suffix:semicolon
op_star
id|hdr_arg
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|args-&gt;keep_pack
op_logical_and
id|unpack_limit
)paren
(brace
r_struct
id|pack_header
id|header
suffix:semicolon
r_if
c_cond
(paren
id|read_pack_header
c_func
(paren
id|demux.out
comma
op_amp
id|header
)paren
)paren
id|die
c_func
(paren
l_string|&quot;protocol error: bad pack header&quot;
)paren
suffix:semicolon
id|snprintf
c_func
(paren
id|hdr_arg
comma
r_sizeof
(paren
id|hdr_arg
)paren
comma
l_string|&quot;--pack_header=%&quot;
id|PRIu32
l_string|&quot;,%&quot;
id|PRIu32
comma
id|ntohl
c_func
(paren
id|header.hdr_version
)paren
comma
id|ntohl
c_func
(paren
id|header.hdr_entries
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ntohl
c_func
(paren
id|header.hdr_entries
)paren
OL
id|unpack_limit
)paren
id|do_keep
op_assign
l_int|0
suffix:semicolon
r_else
id|do_keep
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|alternate_shallow_file
)paren
(brace
op_star
id|av
op_increment
op_assign
l_string|&quot;--shallow-file&quot;
suffix:semicolon
op_star
id|av
op_increment
op_assign
id|alternate_shallow_file
suffix:semicolon
)brace
r_if
c_cond
(paren
id|do_keep
)paren
(brace
r_if
c_cond
(paren
id|pack_lockfile
)paren
id|cmd.out
op_assign
l_int|1
suffix:semicolon
op_star
id|av
op_increment
op_assign
id|cmd_name
op_assign
l_string|&quot;index-pack&quot;
suffix:semicolon
op_star
id|av
op_increment
op_assign
l_string|&quot;--stdin&quot;
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|args-&gt;quiet
op_logical_and
op_logical_neg
id|args-&gt;no_progress
)paren
op_star
id|av
op_increment
op_assign
l_string|&quot;-v&quot;
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;use_thin_pack
)paren
op_star
id|av
op_increment
op_assign
l_string|&quot;--fix-thin&quot;
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;lock_pack
op_logical_or
id|unpack_limit
)paren
(brace
r_int
id|s
op_assign
id|sprintf
c_func
(paren
id|keep_arg
comma
l_string|&quot;--keep=fetch-pack %&quot;
id|PRIuMAX
l_string|&quot; on &quot;
comma
(paren
r_uintmax
)paren
id|getpid
c_func
(paren
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|gethostname
c_func
(paren
id|keep_arg
op_plus
id|s
comma
r_sizeof
(paren
id|keep_arg
)paren
id|s
)paren
)paren
id|strcpy
c_func
(paren
id|keep_arg
op_plus
id|s
comma
l_string|&quot;localhost&quot;
)paren
suffix:semicolon
op_star
id|av
op_increment
op_assign
id|keep_arg
suffix:semicolon
)brace
r_if
c_cond
(paren
id|args-&gt;check_self_contained_and_connected
)paren
op_star
id|av
op_increment
op_assign
l_string|&quot;--check-self-contained-and-connected&quot;
suffix:semicolon
)brace
r_else
(brace
op_star
id|av
op_increment
op_assign
id|cmd_name
op_assign
l_string|&quot;unpack-objects&quot;
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;quiet
op_logical_or
id|args-&gt;no_progress
)paren
op_star
id|av
op_increment
op_assign
l_string|&quot;-q&quot;
suffix:semicolon
id|args-&gt;check_self_contained_and_connected
op_assign
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
op_star
id|hdr_arg
)paren
op_star
id|av
op_increment
op_assign
id|hdr_arg
suffix:semicolon
r_if
c_cond
(paren
id|fetch_fsck_objects
op_ge
l_int|0
ques
c_cond
id|fetch_fsck_objects
suffix:colon
id|transfer_fsck_objects
op_ge
l_int|0
ques
c_cond
id|transfer_fsck_objects
suffix:colon
l_int|0
)paren
op_star
id|av
op_increment
op_assign
l_string|&quot;--strict&quot;
suffix:semicolon
op_star
id|av
op_increment
op_assign
l_int|NULL
suffix:semicolon
id|cmd.in
op_assign
id|demux.out
suffix:semicolon
id|cmd.git_cmd
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|start_command
c_func
(paren
op_amp
id|cmd
)paren
)paren
id|die
c_func
(paren
l_string|&quot;fetch-pack: unable to fork off %s&quot;
comma
id|cmd_name
)paren
suffix:semicolon
r_if
c_cond
(paren
id|do_keep
op_logical_and
id|pack_lockfile
)paren
(brace
op_star
id|pack_lockfile
op_assign
id|index_pack_lockfile
c_func
(paren
id|cmd.out
)paren
suffix:semicolon
id|close
c_func
(paren
id|cmd.out
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|use_sideband
)paren
multiline_comment|/* Closed by start_command() */
id|xd
(braket
l_int|0
)braket
op_assign
l_int|1
suffix:semicolon
id|ret
op_assign
id|finish_command
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ret
op_logical_or
(paren
id|args-&gt;check_self_contained_and_connected
op_logical_and
id|ret
op_eq
l_int|1
)paren
)paren
id|args-&gt;self_contained_and_connected
op_assign
id|args-&gt;check_self_contained_and_connected
op_logical_and
id|ret
op_eq
l_int|0
suffix:semicolon
r_else
id|die
c_func
(paren
l_string|&quot;%s failed&quot;
comma
id|cmd_name
)paren
suffix:semicolon
r_if
c_cond
(paren
id|use_sideband
op_logical_and
id|finish_async
c_func
(paren
op_amp
id|demux
)paren
)paren
id|die
c_func
(paren
l_string|&quot;error in sideband demultiplexer&quot;
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|cmp_ref_by_name
r_static
r_int
id|cmp_ref_by_name
c_func
(paren
r_const
r_void
op_star
id|a_
comma
r_const
r_void
op_star
id|b_
)paren
(brace
r_const
r_struct
id|ref
op_star
id|a
op_assign
op_star
(paren
(paren
r_const
r_struct
id|ref
op_star
op_star
)paren
id|a_
)paren
suffix:semicolon
r_const
r_struct
id|ref
op_star
id|b
op_assign
op_star
(paren
(paren
r_const
r_struct
id|ref
op_star
op_star
)paren
id|b_
)paren
suffix:semicolon
r_return
id|strcmp
c_func
(paren
id|a-&gt;name
comma
id|b-&gt;name
)paren
suffix:semicolon
)brace
DECL|function|do_fetch_pack
r_static
r_struct
id|ref
op_star
id|do_fetch_pack
c_func
(paren
r_struct
id|fetch_pack_args
op_star
id|args
comma
r_int
id|fd
(braket
l_int|2
)braket
comma
r_const
r_struct
id|ref
op_star
id|orig_ref
comma
r_struct
id|ref
op_star
op_star
id|sought
comma
r_int
id|nr_sought
comma
r_struct
id|shallow_info
op_star
id|si
comma
r_char
op_star
op_star
id|pack_lockfile
)paren
(brace
r_struct
id|ref
op_star
id|ref
op_assign
id|copy_ref_list
c_func
(paren
id|orig_ref
)paren
suffix:semicolon
r_int
r_char
id|sha1
(braket
l_int|20
)braket
suffix:semicolon
r_const
r_char
op_star
id|agent_feature
suffix:semicolon
r_int
id|agent_len
suffix:semicolon
id|sort_ref_list
c_func
(paren
op_amp
id|ref
comma
id|ref_compare_name
)paren
suffix:semicolon
id|qsort
c_func
(paren
id|sought
comma
id|nr_sought
comma
r_sizeof
(paren
op_star
id|sought
)paren
comma
id|cmp_ref_by_name
)paren
suffix:semicolon
r_if
c_cond
(paren
id|is_repository_shallow
c_func
(paren
)paren
op_logical_and
op_logical_neg
id|server_supports
c_func
(paren
l_string|&quot;shallow&quot;
)paren
)paren
id|die
c_func
(paren
l_string|&quot;Server does not support shallow clients&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|server_supports
c_func
(paren
l_string|&quot;multi_ack_detailed&quot;
)paren
)paren
(brace
r_if
c_cond
(paren
id|args-&gt;verbose
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;Server supports multi_ack_detailed&bslash;n&quot;
)paren
suffix:semicolon
id|multi_ack
op_assign
l_int|2
suffix:semicolon
r_if
c_cond
(paren
id|server_supports
c_func
(paren
l_string|&quot;no-done&quot;
)paren
)paren
(brace
r_if
c_cond
(paren
id|args-&gt;verbose
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;Server supports no-done&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;stateless_rpc
)paren
id|no_done
op_assign
l_int|1
suffix:semicolon
)brace
)brace
r_else
r_if
c_cond
(paren
id|server_supports
c_func
(paren
l_string|&quot;multi_ack&quot;
)paren
)paren
(brace
r_if
c_cond
(paren
id|args-&gt;verbose
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;Server supports multi_ack&bslash;n&quot;
)paren
suffix:semicolon
id|multi_ack
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|server_supports
c_func
(paren
l_string|&quot;side-band-64k&quot;
)paren
)paren
(brace
r_if
c_cond
(paren
id|args-&gt;verbose
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;Server supports side-band-64k&bslash;n&quot;
)paren
suffix:semicolon
id|use_sideband
op_assign
l_int|2
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|server_supports
c_func
(paren
l_string|&quot;side-band&quot;
)paren
)paren
(brace
r_if
c_cond
(paren
id|args-&gt;verbose
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;Server supports side-band&bslash;n&quot;
)paren
suffix:semicolon
id|use_sideband
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|server_supports
c_func
(paren
l_string|&quot;allow-tip-sha1-in-want&quot;
)paren
)paren
(brace
r_if
c_cond
(paren
id|args-&gt;verbose
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;Server supports allow-tip-sha1-in-want&bslash;n&quot;
)paren
suffix:semicolon
id|allow_tip_sha1_in_want
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|server_supports
c_func
(paren
l_string|&quot;thin-pack&quot;
)paren
)paren
id|args-&gt;use_thin_pack
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|server_supports
c_func
(paren
l_string|&quot;no-progress&quot;
)paren
)paren
id|args-&gt;no_progress
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|server_supports
c_func
(paren
l_string|&quot;include-tag&quot;
)paren
)paren
id|args-&gt;include_tag
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|server_supports
c_func
(paren
l_string|&quot;ofs-delta&quot;
)paren
)paren
(brace
r_if
c_cond
(paren
id|args-&gt;verbose
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;Server supports ofs-delta&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_else
id|prefer_ofs_delta
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
(paren
id|agent_feature
op_assign
id|server_feature_value
c_func
(paren
l_string|&quot;agent&quot;
comma
op_amp
id|agent_len
)paren
)paren
)paren
(brace
id|agent_supported
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;verbose
op_logical_and
id|agent_len
)paren
id|fprintf
c_func
(paren
id|stderr
comma
l_string|&quot;Server version is %.*s&bslash;n&quot;
comma
id|agent_len
comma
id|agent_feature
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|everything_local
c_func
(paren
id|args
comma
op_amp
id|ref
comma
id|sought
comma
id|nr_sought
)paren
)paren
(brace
id|packet_flush
c_func
(paren
id|fd
(braket
l_int|1
)braket
)paren
suffix:semicolon
r_goto
id|all_done
suffix:semicolon
)brace
r_if
c_cond
(paren
id|find_common
c_func
(paren
id|args
comma
id|fd
comma
id|sha1
comma
id|ref
)paren
OL
l_int|0
)paren
r_if
c_cond
(paren
op_logical_neg
id|args-&gt;keep_pack
)paren
multiline_comment|/* When cloning, it is not unusual to have&n;&t;&t;&t; * no common commit.&n;&t;&t;&t; */
id|warning
c_func
(paren
l_string|&quot;no common commits&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;stateless_rpc
)paren
id|packet_flush
c_func
(paren
id|fd
(braket
l_int|1
)braket
)paren
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;depth
OG
l_int|0
)paren
id|setup_alternate_shallow
c_func
(paren
op_amp
id|shallow_lock
comma
op_amp
id|alternate_shallow_file
comma
l_int|NULL
)paren
suffix:semicolon
r_else
r_if
c_cond
(paren
id|si-&gt;nr_ours
op_logical_or
id|si-&gt;nr_theirs
)paren
id|alternate_shallow_file
op_assign
id|setup_temporary_shallow
c_func
(paren
id|si-&gt;shallow
)paren
suffix:semicolon
r_else
id|alternate_shallow_file
op_assign
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
id|get_pack
c_func
(paren
id|args
comma
id|fd
comma
id|pack_lockfile
)paren
)paren
id|die
c_func
(paren
l_string|&quot;git fetch-pack: fetch failed.&quot;
)paren
suffix:semicolon
id|all_done
suffix:colon
r_return
id|ref
suffix:semicolon
)brace
DECL|function|fetch_pack_config
r_static
r_int
id|fetch_pack_config
c_func
(paren
r_const
r_char
op_star
id|var
comma
r_const
r_char
op_star
id|value
comma
r_void
op_star
id|cb
)paren
(brace
r_if
c_cond
(paren
id|strcmp
c_func
(paren
id|var
comma
l_string|&quot;fetch.unpacklimit&quot;
)paren
op_eq
l_int|0
)paren
(brace
id|fetch_unpack_limit
op_assign
id|git_config_int
c_func
(paren
id|var
comma
id|value
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
id|strcmp
c_func
(paren
id|var
comma
l_string|&quot;transfer.unpacklimit&quot;
)paren
op_eq
l_int|0
)paren
(brace
id|transfer_unpack_limit
op_assign
id|git_config_int
c_func
(paren
id|var
comma
id|value
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
id|strcmp
c_func
(paren
id|var
comma
l_string|&quot;repack.usedeltabaseoffset&quot;
)paren
op_eq
l_int|0
)paren
(brace
id|prefer_ofs_delta
op_assign
id|git_config_bool
c_func
(paren
id|var
comma
id|value
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strcmp
c_func
(paren
id|var
comma
l_string|&quot;fetch.fsckobjects&quot;
)paren
)paren
(brace
id|fetch_fsck_objects
op_assign
id|git_config_bool
c_func
(paren
id|var
comma
id|value
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strcmp
c_func
(paren
id|var
comma
l_string|&quot;transfer.fsckobjects&quot;
)paren
)paren
(brace
id|transfer_fsck_objects
op_assign
id|git_config_bool
c_func
(paren
id|var
comma
id|value
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_return
id|git_default_config
c_func
(paren
id|var
comma
id|value
comma
id|cb
)paren
suffix:semicolon
)brace
DECL|function|fetch_pack_setup
r_static
r_void
id|fetch_pack_setup
c_func
(paren
r_void
)paren
(brace
r_static
r_int
id|did_setup
suffix:semicolon
r_if
c_cond
(paren
id|did_setup
)paren
r_return
suffix:semicolon
id|git_config
c_func
(paren
id|fetch_pack_config
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
l_int|0
op_le
id|transfer_unpack_limit
)paren
id|unpack_limit
op_assign
id|transfer_unpack_limit
suffix:semicolon
r_else
r_if
c_cond
(paren
l_int|0
op_le
id|fetch_unpack_limit
)paren
id|unpack_limit
op_assign
id|fetch_unpack_limit
suffix:semicolon
id|did_setup
op_assign
l_int|1
suffix:semicolon
)brace
DECL|function|remove_duplicates_in_refs
r_static
r_int
id|remove_duplicates_in_refs
c_func
(paren
r_struct
id|ref
op_star
op_star
id|ref
comma
r_int
id|nr
)paren
(brace
r_struct
id|string_list
id|names
op_assign
id|STRING_LIST_INIT_NODUP
suffix:semicolon
r_int
id|src
comma
id|dst
suffix:semicolon
r_for
c_loop
(paren
id|src
op_assign
id|dst
op_assign
l_int|0
suffix:semicolon
id|src
OL
id|nr
suffix:semicolon
id|src
op_increment
)paren
(brace
r_struct
id|string_list_item
op_star
id|item
suffix:semicolon
id|item
op_assign
id|string_list_insert
c_func
(paren
op_amp
id|names
comma
id|ref
(braket
id|src
)braket
op_member_access_from_pointer
id|name
)paren
suffix:semicolon
r_if
c_cond
(paren
id|item-&gt;util
)paren
r_continue
suffix:semicolon
multiline_comment|/* already have it */
id|item-&gt;util
op_assign
id|ref
(braket
id|src
)braket
suffix:semicolon
r_if
c_cond
(paren
id|src
op_ne
id|dst
)paren
id|ref
(braket
id|dst
)braket
op_assign
id|ref
(braket
id|src
)braket
suffix:semicolon
id|dst
op_increment
suffix:semicolon
)brace
r_for
c_loop
(paren
id|src
op_assign
id|dst
suffix:semicolon
id|src
OL
id|nr
suffix:semicolon
id|src
op_increment
)paren
id|ref
(braket
id|src
)braket
op_assign
l_int|NULL
suffix:semicolon
id|string_list_clear
c_func
(paren
op_amp
id|names
comma
l_int|0
)paren
suffix:semicolon
r_return
id|dst
suffix:semicolon
)brace
DECL|function|update_shallow
r_static
r_void
id|update_shallow
c_func
(paren
r_struct
id|fetch_pack_args
op_star
id|args
comma
r_struct
id|ref
op_star
op_star
id|sought
comma
r_int
id|nr_sought
comma
r_struct
id|shallow_info
op_star
id|si
)paren
(brace
r_struct
id|sha1_array
id|ref
op_assign
id|SHA1_ARRAY_INIT
suffix:semicolon
r_int
op_star
id|status
suffix:semicolon
r_int
id|i
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;depth
OG
l_int|0
op_logical_and
id|alternate_shallow_file
)paren
(brace
r_if
c_cond
(paren
op_star
id|alternate_shallow_file
op_eq
l_char|&squot;&bslash;0&squot;
)paren
(brace
multiline_comment|/* --unshallow */
id|unlink_or_warn
c_func
(paren
id|git_path
c_func
(paren
l_string|&quot;shallow&quot;
)paren
)paren
suffix:semicolon
id|rollback_lock_file
c_func
(paren
op_amp
id|shallow_lock
)paren
suffix:semicolon
)brace
r_else
id|commit_lock_file
c_func
(paren
op_amp
id|shallow_lock
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|si-&gt;shallow
op_logical_or
op_logical_neg
id|si-&gt;shallow-&gt;nr
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
id|alternate_shallow_file
)paren
(brace
multiline_comment|/*&n;&t;&t; * The temporary shallow file is only useful for&n;&t;&t; * index-pack and unpack-objects because it may&n;&t;&t; * contain more roots than we want. Delete it.&n;&t;&t; */
r_if
c_cond
(paren
op_star
id|alternate_shallow_file
)paren
id|unlink
c_func
(paren
id|alternate_shallow_file
)paren
suffix:semicolon
id|free
c_func
(paren
(paren
r_char
op_star
)paren
id|alternate_shallow_file
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|args-&gt;cloning
)paren
(brace
multiline_comment|/*&n;&t;&t; * remote is shallow, but this is a clone, there are&n;&t;&t; * no objects in repo to worry about. Accept any&n;&t;&t; * shallow points that exist in the pack (iow in repo&n;&t;&t; * after get_pack() and reprepare_packed_git())&n;&t;&t; */
r_struct
id|sha1_array
id|extra
op_assign
id|SHA1_ARRAY_INIT
suffix:semicolon
r_int
r_char
(paren
op_star
id|sha1
)paren
(braket
l_int|20
)braket
op_assign
id|si-&gt;shallow-&gt;sha1
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|si-&gt;shallow-&gt;nr
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|has_sha1_file
c_func
(paren
id|sha1
(braket
id|i
)braket
)paren
)paren
id|sha1_array_append
c_func
(paren
op_amp
id|extra
comma
id|sha1
(braket
id|i
)braket
)paren
suffix:semicolon
r_if
c_cond
(paren
id|extra.nr
)paren
(brace
id|setup_alternate_shallow
c_func
(paren
op_amp
id|shallow_lock
comma
op_amp
id|alternate_shallow_file
comma
op_amp
id|extra
)paren
suffix:semicolon
id|commit_lock_file
c_func
(paren
op_amp
id|shallow_lock
)paren
suffix:semicolon
)brace
id|sha1_array_clear
c_func
(paren
op_amp
id|extra
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|si-&gt;nr_ours
op_logical_and
op_logical_neg
id|si-&gt;nr_theirs
)paren
r_return
suffix:semicolon
id|remove_nonexistent_theirs_shallow
c_func
(paren
id|si
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|si-&gt;nr_ours
op_logical_and
op_logical_neg
id|si-&gt;nr_theirs
)paren
r_return
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|nr_sought
suffix:semicolon
id|i
op_increment
)paren
id|sha1_array_append
c_func
(paren
op_amp
id|ref
comma
id|sought
(braket
id|i
)braket
op_member_access_from_pointer
id|old_sha1
)paren
suffix:semicolon
id|si-&gt;ref
op_assign
op_amp
id|ref
suffix:semicolon
r_if
c_cond
(paren
id|args-&gt;update_shallow
)paren
(brace
multiline_comment|/*&n;&t;&t; * remote is also shallow, .git/shallow may be updated&n;&t;&t; * so all refs can be accepted. Make sure we only add&n;&t;&t; * shallow roots that are actually reachable from new&n;&t;&t; * refs.&n;&t;&t; */
r_struct
id|sha1_array
id|extra
op_assign
id|SHA1_ARRAY_INIT
suffix:semicolon
r_int
r_char
(paren
op_star
id|sha1
)paren
(braket
l_int|20
)braket
op_assign
id|si-&gt;shallow-&gt;sha1
suffix:semicolon
id|assign_shallow_commits_to_refs
c_func
(paren
id|si
comma
l_int|NULL
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|si-&gt;nr_ours
op_logical_and
op_logical_neg
id|si-&gt;nr_theirs
)paren
(brace
id|sha1_array_clear
c_func
(paren
op_amp
id|ref
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|si-&gt;nr_ours
suffix:semicolon
id|i
op_increment
)paren
id|sha1_array_append
c_func
(paren
op_amp
id|extra
comma
id|sha1
(braket
id|si-&gt;ours
(braket
id|i
)braket
)braket
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|si-&gt;nr_theirs
suffix:semicolon
id|i
op_increment
)paren
id|sha1_array_append
c_func
(paren
op_amp
id|extra
comma
id|sha1
(braket
id|si-&gt;theirs
(braket
id|i
)braket
)braket
)paren
suffix:semicolon
id|setup_alternate_shallow
c_func
(paren
op_amp
id|shallow_lock
comma
op_amp
id|alternate_shallow_file
comma
op_amp
id|extra
)paren
suffix:semicolon
id|commit_lock_file
c_func
(paren
op_amp
id|shallow_lock
)paren
suffix:semicolon
id|sha1_array_clear
c_func
(paren
op_amp
id|extra
)paren
suffix:semicolon
id|sha1_array_clear
c_func
(paren
op_amp
id|ref
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * remote is also shallow, check what ref is safe to update&n;&t; * without updating .git/shallow&n;&t; */
id|status
op_assign
id|xcalloc
c_func
(paren
id|nr_sought
comma
r_sizeof
(paren
op_star
id|status
)paren
)paren
suffix:semicolon
id|assign_shallow_commits_to_refs
c_func
(paren
id|si
comma
l_int|NULL
comma
id|status
)paren
suffix:semicolon
r_if
c_cond
(paren
id|si-&gt;nr_ours
op_logical_or
id|si-&gt;nr_theirs
)paren
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|nr_sought
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|status
(braket
id|i
)braket
)paren
id|sought
(braket
id|i
)braket
op_member_access_from_pointer
id|status
op_assign
id|REF_STATUS_REJECT_SHALLOW
suffix:semicolon
)brace
id|free
c_func
(paren
id|status
)paren
suffix:semicolon
id|sha1_array_clear
c_func
(paren
op_amp
id|ref
)paren
suffix:semicolon
)brace
DECL|function|fetch_pack
r_struct
id|ref
op_star
id|fetch_pack
c_func
(paren
r_struct
id|fetch_pack_args
op_star
id|args
comma
r_int
id|fd
(braket
)braket
comma
r_struct
id|child_process
op_star
id|conn
comma
r_const
r_struct
id|ref
op_star
id|ref
comma
r_const
r_char
op_star
id|dest
comma
r_struct
id|ref
op_star
op_star
id|sought
comma
r_int
id|nr_sought
comma
r_struct
id|sha1_array
op_star
id|shallow
comma
r_char
op_star
op_star
id|pack_lockfile
)paren
(brace
r_struct
id|ref
op_star
id|ref_cpy
suffix:semicolon
r_struct
id|shallow_info
id|si
suffix:semicolon
id|fetch_pack_setup
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|nr_sought
)paren
id|nr_sought
op_assign
id|remove_duplicates_in_refs
c_func
(paren
id|sought
comma
id|nr_sought
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ref
)paren
(brace
id|packet_flush
c_func
(paren
id|fd
(braket
l_int|1
)braket
)paren
suffix:semicolon
id|die
c_func
(paren
l_string|&quot;no matching remote head&quot;
)paren
suffix:semicolon
)brace
id|prepare_shallow_info
c_func
(paren
op_amp
id|si
comma
id|shallow
)paren
suffix:semicolon
id|ref_cpy
op_assign
id|do_fetch_pack
c_func
(paren
id|args
comma
id|fd
comma
id|ref
comma
id|sought
comma
id|nr_sought
comma
op_amp
id|si
comma
id|pack_lockfile
)paren
suffix:semicolon
id|reprepare_packed_git
c_func
(paren
)paren
suffix:semicolon
id|update_shallow
c_func
(paren
id|args
comma
id|sought
comma
id|nr_sought
comma
op_amp
id|si
)paren
suffix:semicolon
id|clear_shallow_info
c_func
(paren
op_amp
id|si
)paren
suffix:semicolon
r_return
id|ref_cpy
suffix:semicolon
)brace
eof
