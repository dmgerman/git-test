...
multiline_comment|/* Add a ref_entry to the end of the ref_dir (unsorted). */
...
multiline_comment|/*&n; * Sort the entries in dir (if they are not already sorted).&n; */
...
suffix:semicolon
multiline_comment|/* Remove any duplicates from the ref_dir */
id|i
op_assign
l_int|0
suffix:semicolon
...
multiline_comment|/*&n; * Call fn for each reference in dir that has index in the range&n; * offset &lt;= index &lt; dir-&gt;nr.  This function does not sort the dir;&n; * sorting should be done by the caller.&n; */
DECL|function|do_for_each_ref_in_dir
r_static
r_int
id|do_for_each_ref_in_dir
c_func
(paren
r_struct
id|ref_dir
...
multiline_comment|/*&n; * Call fn for each reference in the union of dir1 and dir2, in order&n; * by refname.  If an entry appears in both dir1 and dir2, then only&n; * process the version that is in dir2.  The input dirs must already&n; * be sorted.&n; */
DECL|function|do_for_each_ref_in_dirs
r_static
r_int
id|do_for_each_ref_in_dirs
c_func
(paren
r_struct
id|ref_dir
...
multiline_comment|/*&n; * Return true iff a reference named refname could be created without&n; * conflicting with the name of an existing reference in array.  If&n; * oldrefname is non-NULL, ignore potential conflicts with oldrefname&n; * (e.g., because oldrefname is scheduled for deletion in the same&n; * operation).&n; */
