...
(brace
multiline_comment|/* If this condition is true, then we may use the attribute&n;&t; * machinery in grep_buffer_1. The attribute code is not&n;&t; * thread safe, so we disable the use of threads.&n;&t; */
r_if
c_cond
(paren
id|opt-&gt;funcname
...
l_int|0
suffix:semicolon
multiline_comment|/* If we are showing hunk marks, we should not do it for the&n;&t; * first match. The synchronization problem we get for this&n;&t; * constraint is not yet solved, so we disable threading in&n;&t; * this case.&n;&t; */
r_if
c_cond
(paren
id|opt-&gt;pre_context
