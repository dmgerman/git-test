...
id|grep_defaults
suffix:semicolon
multiline_comment|/*&n; * Initialize the grep_defaults template with hardcoded defaults.&n; * We could let the compiler do this, but without C99 initializers&n; * the code gets unwieldy and unreadable, so...&n; */
...
l_int|1
suffix:semicolon
id|opt-&gt;pattern_type_option
op_assign
id|GREP_PATTERN_TYPE_UNSPECIFIED
suffix:semicolon
id|opt-&gt;extended_regexp_option
op_assign
l_int|0
suffix:semicolon
id|strcpy
...
multiline_comment|/*&n; * Read the configuration file once and store it in&n; * the grep_defaults template.&n; */
...
id|var
comma
l_string|&quot;color.grep.matchcontext&quot;
)paren
)paren
id|color
op_assign
...
id|var
comma
l_string|&quot;color.grep.matchselected&quot;
)paren
)paren
id|color
op_assign
...
multiline_comment|/*&n; * Initialize one instance of grep_opt and copy the&n; * default values from the template we read the configuration&n; * information in an earlier call to git_config(grep_config).&n; */
...
id|opt-&gt;extended_regexp_option
op_assign
id|def-&gt;extended_regexp_option
suffix:semicolon
id|opt-&gt;pattern_type_option
op_assign
id|def-&gt;pattern_type_option
suffix:semicolon
id|opt-&gt;linenum
op_assign
id|def-&gt;linenum
...
suffix:semicolon
multiline_comment|/*&n;&t; * The textconv interface is intimately tied to diff_filespecs, so we&n;&t; * have to pretend to be one. If we could unify the grep_source&n;&t; * and diff_filespec structs, this mess could just go away.&n;&t; */
id|df
op_assign
id|alloc_filespec
c_func
...
c_func
(paren
l_string|&quot;BUG: attempt to textconv something without a path?&quot;
)paren
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * fill_textconv is not remotely thread-safe; it may load objects&n;&t; * behind the scenes, and it modifies the global diff tempfile&n;&t; * structure.&n;&t; */
id|grep_read_lock
c_func
(paren
)paren
...
suffix:semicolon
multiline_comment|/*&n;&t; * The normal fill_textconv usage by the diff machinery would just keep&n;&t; * the textconv&squot;d buf separate from the diff_filespec. But much of the&n;&t; * grep code passes around a grep_source and assumes that its &quot;buf&quot;&n;&t; * pointer is the beginning of the thing we are searching. So let&squot;s&n;&t; * install our textconv&squot;d version into the grep_source, taking care not&n;&t; * to leak any existing buffer.&n;&t; */
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * We might set up the shared textconv cache data here, which&n;&t;&t; * is not thread-safe.&n;&t;&t; */
id|grep_attr_lock
c_func
(paren
)paren
...
multiline_comment|/*&n;&t; * We know the result of a textconv is text, so we only have to care&n;&t; * about binary handling if we are not using it.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
id|GREP_SOURCE_BUF
suffix:colon
multiline_comment|/* leave user-provided buf intact */
r_break
suffix:semicolon
)brace
)brace
...
c_func
(paren
l_string|&quot;BUG: invalid grep_source type&quot;
)paren
suffix:semicolon
)brace
