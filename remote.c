...
suffix:semicolon
multiline_comment|/* no previous entry overwritten */
r_return
id|ret
suffix:semicolon
)brace
...
id|REMOTE_BRANCHES
suffix:semicolon
multiline_comment|/*&n;&t; * The branches file would have URL and optionally&n;&t; * #branch specified.  The &quot;master&quot; (or specified) branch is&n;&t; * fetched and stored in the local branch matching the&n;&t; * remote name.&n;&t; */
id|frag
op_assign
id|strchr
c_func
...
c_func
(paren
l_string|&quot;refs/heads/%s:refs/heads/%s&quot;
comma
id|frag
comma
id|remote-&gt;name
...
c_cond
(paren
id|ref1-&gt;fetch_head_status
op_ne
id|FETCH_HEAD_IGNORE
op_logical_and
id|ref2-&gt;fetch_head_status
op_ne
id|FETCH_HEAD_IGNORE
)paren
(brace
...
c_func
(paren
l_string|&quot;Cannot fetch both %s and %s to %s&quot;
)paren
comma
id|ref1-&gt;name
comma
...
c_func
(paren
l_string|&quot;%s usually tracks %s, not %s&quot;
)paren
comma
id|ref2-&gt;peer_ref-&gt;name
comma
...
r_else
(brace
multiline_comment|/*&n;&t;&t;&t; * This last possibility doesn&squot;t occur because&n;&t;&t;&t; * FETCH_HEAD_IGNORE entries always appear at&n;&t;&t;&t; * the end of the list.&n;&t;&t;&t; */
id|die
c_func
(paren
id|_
...
c_func
(paren
l_string|&quot;query_refspecs_multiple: need either src or dst&quot;
)paren
suffix:semicolon
r_for
c_loop
...
id|sent_tips
suffix:semicolon
multiline_comment|/*&n;&t; * Collect everything we know they would have at the end of&n;&t; * this push, and collect all tags they have.&n;&t; */
id|memset
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* Collect tags they do not have. */
r_for
c_loop
(paren
id|ref
...
r_continue
suffix:semicolon
multiline_comment|/* they already have it */
r_if
c_cond
(paren
id|sha1_object_info
...
suffix:semicolon
multiline_comment|/*&n;&t; * At this point, src_tag lists tags that are missing from&n;&t; * dst, and sent_tips lists the tips we are pushing or those&n;&t; * that we know they already have. An element in the src_tag&n;&t; * that is an ancestor of any of the sent_tips needs to be&n;&t; * sent to the other side.&n;&t; */
r_if
c_cond
(paren
id|sent_tips.nr
...
multiline_comment|/* not pushing a commit, which is not an error */
r_continue
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * Is this tag, which they do not have, reachable from&n;&t;&t;&t; * any of the commits we are sending?&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*&n; * Given only the set of local refs, sanity-check the set of push&n; * refspecs. We can&squot;t catch all errors that match_push_refs would,&n; * but we can catch some errors early before even talking to the&n; * remote side.&n; */
...
(brace
multiline_comment|/*&n;&t;&t; * no merge config; let&squot;s make sure we don&squot;t confuse callers&n;&t;&t; * with a non-zero merge_nr but a NULL merge&n;&t;&t; */
id|ret-&gt;merge_nr
op_assign
l_int|0
suffix:semicolon
...
c_func
(paren
l_string|&quot;push destination &squot;%s&squot; on remote &squot;%s&squot; has no local tracking branch&quot;
)paren
comma
id|refname
comma
...
c_func
(paren
l_string|&quot;branch &squot;%s&squot; has no remote for pushing&quot;
)paren
comma
id|branch-&gt;name
)paren
...
c_func
(paren
l_string|&quot;push refspecs for &squot;%s&squot; do not include &squot;%s&squot;&quot;
)paren
comma
id|remote-&gt;name
comma
...
c_func
(paren
l_string|&quot;push has no destination (push.default is &squot;nothing&squot;)&quot;
)paren
)paren
suffix:semicolon
r_case
...
c_func
(paren
l_string|&quot;cannot resolve &squot;simple&squot; push to a single destination&quot;
)paren
)paren
suffix:semicolon
r_return
...
c_func
(paren
l_string|&quot;BUG: unhandled push situation&quot;
)paren
suffix:semicolon
)brace
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * Both new and old must be commit-ish and new is descendant of&n;&t; * old.  Otherwise we require --force.&n;&t; */
id|o
op_assign
id|deref_tag
c_func
...
multiline_comment|/*&n;&t; * If we did find a suitable refspec and it&squot;s not a symref and&n;&t; * it&squot;s not in the list of refs that currently exist in that&n;&t; * remote, we consider it to be stale. In order to deal with&n;&t; * overlapping refspecs, we need to go over all of the&n;&t; * matching refs.&n;&t; */
r_if
c_cond
(paren
id|flags
...
(brace
multiline_comment|/* just &quot;--&lt;option&gt;&quot; */
id|cas-&gt;use_tracking_for_rest
op_assign
l_int|1
suffix:semicolon
r_return
...
multiline_comment|/* &quot;--&lt;option&gt;=refname&quot; or &quot;--&lt;option&gt;=refname:value&quot; */
id|colon
op_assign
id|strchrnul
c_func
...
c_func
(paren
l_string|&quot;cannot parse expected object name &squot;%s&squot;&quot;
comma
id|colon
op_plus
l_int|1
...
multiline_comment|/*&n; * Look at remote.fetch refspec and see if we have a remote&n; * tracking branch for the refname there.  Fill its current&n; * value in sha1[].&n; * If we cannot do so, return negative to signal an error.&n; */
...
l_int|1
suffix:semicolon
multiline_comment|/* no tracking ref for refname at remote */
r_if
c_cond
(paren
id|read_ref
...
l_int|1
suffix:semicolon
multiline_comment|/* we know what the tracking ref is but we cannot read it */
r_return
l_int|0
suffix:semicolon
)brace
...
id|i
suffix:semicolon
multiline_comment|/* Find an explicit --&lt;option&gt;=&lt;name&gt;[:&lt;value&gt;] entry */
r_for
c_loop
(paren
id|i
...
multiline_comment|/* Are we using &quot;--&lt;option&gt;&quot; to cover all? */
r_if
c_cond
(paren
op_logical_neg
