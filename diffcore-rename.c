...
multiline_comment|/*&n; * Find exact renames first.&n; *&n; * The first round matches up the up-to-date entries,&n; * and then during the second round we try to match&n; * cache-dirty entries as well.&n; *&n; * Note: the rest of the rename logic depends on this&n; * phase also populating all the filespecs for any&n; * entry that isn&squot;t matched up with an exact rename,&n; * see &quot;is_exact_match()&quot;.&n; */
...
r_continue
suffix:semicolon
multiline_comment|/* see if there is a basename match, too */
r_for
c_loop
(paren
id|k
