...
r_char
op_star
id|git_getpass_with_description
c_func
(paren
r_const
r_char
...
suffix:semicolon
multiline_comment|/*&n;&t; * NEEDSWORK: for usernames, we should do something less magical that&n;&t; * actually echoes the characters. However, we need to read from&n;&t; * /dev/tty and not stdio, which is not portable (but getpass will do&n;&t; * it for us). http.c uses the same workaround.&n;&t; */
id|r
op_assign
id|git_getpass
c_func
...
(brace
multiline_comment|/* No credentials, but we may have to ask for some later */
id|host
op_assign
id|cp
suffix:semicolon
