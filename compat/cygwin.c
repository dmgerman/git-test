...
multiline_comment|/*&n; * At start up, we are trying to determine whether Win32 API or cygwin stat&n; * functions should be used. The choice is determined by core.ignorecygwinfstricks.&n; * Reading this option is not always possible immediately as git_dir may be&n; * not be set yet. So until it is set, use cygwin lstat/stat functions.&n; * However, if core.filemode is set, we must use the Cygwin posix&n; * stat/lstat as the Windows stat fuctions do not determine posix filemode.&n; *&n; * Note that git_cygwin_config() does NOT call git_default_config() and this&n; * is deliberate.  Many commands read from config to establish initial&n; * values in variables and later tweak them from elsewhere (e.g. command line).&n; * init_stat() is called lazily on demand, typically much late in the program,&n; * and calling git_default_config() from here would break such variables.&n; */
