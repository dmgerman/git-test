...
multiline_comment|/*&n; * At start up, we are trying to determine whether Win32 API or cygwin stat&n; * functions should be used. The choice is determined by core.ignorecygwinfstricks.&n; * Reading this option is not always possible immediately as git_dir may be&n; * not be set yet. So until it is set, use cygwin lstat/stat functions.&n; */
