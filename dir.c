...
multiline_comment|/*&n; * For each subdirectory in name, starting with the top-most, checks&n; * to see if that subdirectory is excluded, and if so, returns the&n; * corresponding exclude structure.  Otherwise, checks whether name&n; * itself (which is presumably a file) is excluded.&n; *&n; * A path to a directory known to be excluded is left in check-&gt;path to&n; * optimize for repeated checks for files in the same excluded directory.&n; */
DECL|function|last_exclude_matching_path
r_struct
id|exclude
op_star
id|last_exclude_matching_path
c_func
(paren
r_struct
id|path_exclude_check
...
suffix:semicolon
multiline_comment|/*&n;&t; * If path is non-empty, and name is equal to path or a&n;&t; * subdirectory of path, name should be excluded, because&n;&t; * it&squot;s inside a directory which is already known to be&n;&t; * excluded and was previously left in check-&gt;path.&n;&t; */
r_if
c_cond
(paren
id|path-&gt;len
...
multiline_comment|/* An entry in the index; cannot be a directory with subentries */
id|strbuf_setlen
c_func
(paren
id|path
...
multiline_comment|/*&n; * Is this name excluded?  This is for a caller like show_files() that&n; * do not honor directory hierarchy and iterate through paths that are&n; * possibly in an ignored directory.&n; */
...
multiline_comment|/* might be a sub directory in an excluded directory */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*&n; * Decide what to do when we find a file while traversing the&n; * filesystem. Mostly two cases:&n; *&n; *  1. We are looking for ignored files&n; *   (a) File is ignored, include it&n; *   (b) File is in ignored path, include it&n; *   (c) File is not ignored, exclude it&n; *&n; *  2. Other scenarios, include the file if not excluded&n; *&n; * Return 1 for exclude, 0 for include.&n; */
