...
suffix:semicolon
multiline_comment|/*&n; * Support data structure for our opendir/readdir/closedir wrappers&n; */
...
multiline_comment|/*&n; * Given a subdirectory name and &quot;dir&quot; of the current directory,&n; * search the subdir in &quot;dir&quot; and return it, or create a new one if it&n; * does not exist in &quot;dir&quot;.&n; *&n; * If &quot;name&quot; has the trailing slash, it&squot;ll be excluded in the search.&n; */
...
multiline_comment|/*&n; * Given a file with name &quot;fname&quot;, read it (either from disk, or from&n; * the index if &quot;check_index&quot; is non-zero), parse it and store the&n; * exclude rules in &quot;el&quot;.&n; *&n; * If &quot;ss&quot; is not NULL, compute SHA-1 of the exclude file and fill&n; * stat data from disk (only valid if add_excludes returns zero). If&n; * ss_valid is non-zero, &quot;ss&quot; must contain good value as input.&n; */
...
suffix:semicolon
multiline_comment|/* no content change, ss-&gt;sha1 still good */
r_else
r_if
c_cond
(paren
...
id|el
suffix:semicolon
multiline_comment|/*&n;&t; * catch setup_standard_excludes() that&squot;s called before&n;&t; * dir-&gt;untracked is assigned. That function behaves&n;&t; * differently when dir-&gt;untracked is non-NULL.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
id|dir-&gt;unmanaged_exclude_files
op_increment
suffix:semicolon
id|el
op_assign
...
op_increment
suffix:semicolon
multiline_comment|/* see validate_untracked_cache() */
id|add_excludes_from_file_1
c_func
(paren
id|dir
...
l_int|1
suffix:semicolon
multiline_comment|/*&n;&t;&t; * This can happen when we ignore some exclude rules&n;&t;&t; * on directories in other to see if negative rules&n;&t;&t; * may match. E.g.&n;&t;&t; *&n;&t;&t; * /abc&n;&t;&t; * !/abc/def/ghi&n;&t;&t; *&n;&t;&t; * The pattern of interest is &quot;/abc&quot;. On the first&n;&t;&t; * try, we should match path &quot;abc&quot; with this pattern&n;&t;&t; * in the &quot;if&quot; statement right above, but the caller&n;&t;&t; * ignores it.&n;&t;&t; *&n;&t;&t; * On the second try with paths within &quot;abc&quot;,&n;&t;&t; * e.g. &quot;abc/xyz&quot;, we come here and try to match it&n;&t;&t; * with &quot;/abc&quot;.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*&n; * Return non-zero if pathname is a directory and an ancestor of the&n; * literal path in a (negative) pattern. This is used to keep&n; * descending in &quot;foo&quot; and &quot;foo/bar&quot; when the pattern is&n; * &quot;!foo/bar/.gitignore&quot;. &quot;foo/notbar&quot; will not be descended however.&n; */
DECL|function|match_neg_path
r_static
r_int
id|match_neg_path
c_func
...
suffix:semicolon
multiline_comment|/* see strbuf_detach() below */
id|clear_exclude_list
c_func
(paren
id|el
...
r_return
suffix:semicolon
multiline_comment|/*&n;&t; * Lazy initialization. All call sites currently just&n;&t; * memset(dir, 0, sizeof(*dir)) before use. Changing all of&n;&t; * them seems lots of work for little benefit.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* Try to read per-directory file */
id|hashclr
c_func
(paren
id|sha1_stat.sha1
...
multiline_comment|/*&n;&t;&t; * NEEDSWORK: when untracked cache is enabled, prep_exclude()&n;&t;&t; * will first be called in valid_cached_dir() then maybe many&n;&t;&t; * times more in last_exclude_matching(). When the cache is&n;&t;&t; * used, last_exclude_matching() will not be called and&n;&t;&t; * reading .gitignore content will be a waste.&n;&t;&t; *&n;&t;&t; * So when it&squot;s called by valid_cached_dir() and we can get&n;&t;&t; * .gitignore SHA-1 from the index (i.e. .gitignore is not&n;&t;&t; * modified on work tree), we could delay reading the&n;&t;&t; * .gitignore content until we absolutely need it in&n;&t;&t; * last_exclude_matching(). Be careful about ignore rule&n;&t;&t; * order, though, if you do that.&n;&t;&t; */
r_if
c_cond
(paren
id|untracked
...
suffix:semicolon
multiline_comment|/* treat_one_path() does this before it calls treat_directory() */
id|strbuf_complete
c_func
(paren
id|path
...
c_cond
(paren
id|cdir-&gt;ucd-&gt;check_only
)paren
multiline_comment|/*&n;&t;&t; * check_only is set as a result of treat_directory() getting&n;&t;&t; * to its bottom. Verify again the same set of directories&n;&t;&t; * with check_only set.&n;&t;&t; */
r_return
id|read_directory_recursive
c_func
(paren
...
suffix:semicolon
multiline_comment|/*&n;&t; * We get path_recurse in the first run when&n;&t; * directory_exists_in_index() returns index_nonexistent. We&n;&t; * are sure that new changes in the index does not impact the&n;&t; * outcome. Return now.&n;&t; */
r_return
id|path_recurse
suffix:semicolon
)brace
...
multiline_comment|/*&n;&t; * prep_exclude will be called eventually on this directory,&n;&t; * but it&squot;s called much later in last_exclude_matching(). We&n;&t; * need it now to determine the validity of the cache for this&n;&t; * path. The next calls will be nearly no-op, the way&n;&t; * prep_exclude() is designed.&n;&t; */
r_if
c_cond
(paren
id|path-&gt;len
...
suffix:semicolon
multiline_comment|/* hopefully prep_exclude() haven&squot;t invalidated this entry... */
r_return
id|untracked-&gt;valid
suffix:semicolon
)brace
...
id|cdir-&gt;nr_files
OL
id|cdir-&gt;untracked-&gt;untracked_nr
)paren
(brace
r_struct
id|untracked_cache_dir
...
suffix:semicolon
multiline_comment|/*&n;&t; * We have gone through this directory and found no untracked&n;&t; * entries. Mark it valid.&n;&t; */
r_if
c_cond
(paren
id|cdir-&gt;untracked
...
(brace
id|cdir-&gt;untracked-&gt;valid
op_assign
l_int|1
suffix:semicolon
id|cdir-&gt;untracked-&gt;recurse
op_assign
l_int|1
suffix:semicolon
)brace
...
c_func
(paren
l_string|&quot;failed to get kernel name and information&quot;
)paren
)paren
suffix:semicolon
id|strbuf_addf
...
id|sb
comma
l_string|&quot;Location %s, system %s %s %s&quot;
comma
...
suffix:semicolon
multiline_comment|/* this strbuf contains a list of strings, save NUL too */
id|strbuf_addch
c_func
(paren
op_amp
...
l_int|NULL
suffix:semicolon
multiline_comment|/*&n;&t; * We only support $GIT_DIR/info/exclude and core.excludesfile&n;&t; * as the global ignore rule files. Any other additions&n;&t; * (e.g. from command line) invalidate the cache. This&n;&t; * condition also catches running setup_standard_excludes()&n;&t; * before setting dir-&gt;untracked!&n;&t; */
r_if
c_cond
(paren
id|dir-&gt;unmanaged_exclude_files
...
l_int|NULL
suffix:semicolon
multiline_comment|/*&n;&t; * Optimize for the main use case only: whole-tree git&n;&t; * status. More work involved in treat_leading_path() if we&n;&t; * use cache on just a subset of the worktree. pathspec&n;&t; * support could make the matter even worse.&n;&t; */
r_if
c_cond
(paren
id|base_len
...
l_int|NULL
suffix:semicolon
multiline_comment|/* Different set of flags may produce different results */
r_if
c_cond
(paren
id|dir-&gt;flags
op_ne
id|dir-&gt;untracked-&gt;dir_flags
op_logical_or
multiline_comment|/*&n;&t;     * See treat_directory(), case index_nonexistent. Without&n;&t;     * this flag, we may need to also cache .git file content&n;&t;     * for the resolve_gitlink_ref() call, which we don&squot;t.&n;&t;     */
op_logical_neg
(paren
id|dir-&gt;flags
op_amp
...
op_logical_or
multiline_comment|/* We don&squot;t support collecting ignore files */
(paren
id|dir-&gt;flags
op_amp
(paren
...
l_int|NULL
suffix:semicolon
multiline_comment|/*&n;&t; * If we use .gitignore in the cache and now you change it to&n;&t; * .gitexclude, everything will go wrong.&n;&t; */
r_if
c_cond
(paren
id|dir-&gt;exclude_per_dir
op_ne
id|dir-&gt;untracked-&gt;exclude_per_dir
op_logical_and
id|strcmp
c_func
(paren
...
l_int|NULL
suffix:semicolon
multiline_comment|/*&n;&t; * EXC_CMDL is not considered in the cache. If people set it,&n;&t; * skip the cache.&n;&t; */
r_if
c_cond
(paren
id|dir-&gt;exclude_list_group
...
c_func
(paren
l_string|&quot;Untracked cache is disabled on this system.&quot;
)paren
)paren
suffix:semicolon
r_return
...
multiline_comment|/* Validate $GIT_DIR/info/exclude and core.excludesfile */
id|root
op_assign
id|dir-&gt;untracked-&gt;root
suffix:semicolon
...
c_func
(paren
id|dir-&gt;ss_info_exclude.sha1
comma
id|dir-&gt;untracked-&gt;ss_info_exclude.sha1
)paren
)paren
(brace
id|invalidate_gitignore
...
suffix:semicolon
id|dir-&gt;untracked-&gt;ss_info_exclude
op_assign
id|dir-&gt;ss_info_exclude
suffix:semicolon
)brace
r_if
c_cond
...
c_func
(paren
id|dir-&gt;ss_excludes_file.sha1
comma
id|dir-&gt;untracked-&gt;ss_excludes_file.sha1
)paren
)paren
(brace
id|invalidate_gitignore
...
suffix:semicolon
id|dir-&gt;untracked-&gt;ss_excludes_file
op_assign
id|dir-&gt;ss_excludes_file
suffix:semicolon
)brace
multiline_comment|/* Make sure this directory is not dropped out at saving phase */
id|root-&gt;recurse
op_assign
l_int|1
suffix:semicolon
...
multiline_comment|/*&n;&t;&t; * make sure untracked cache code path is disabled,&n;&t;&t; * e.g. prep_exclude()&n;&t;&t; */
id|dir-&gt;untracked
op_assign
l_int|NULL
suffix:semicolon
...
l_string|&quot;.gitignore&quot;
suffix:semicolon
multiline_comment|/* core.excludefile defaulting to $XDG_HOME/git/ignore */
r_if
c_cond
(paren
op_logical_neg
...
r_struct
id|ondisk_untracked_cache
(brace
DECL|member|info_exclude_stat
r_struct
id|stat_data
id|info_exclude_stat
suffix:semicolon
DECL|member|excludes_file_stat
r_struct
id|stat_data
id|excludes_file_stat
suffix:semicolon
...
suffix:semicolon
DECL|macro|ouc_size
mdefine_line|#define ouc_size(len) (offsetof(struct ondisk_untracked_cache, exclude_per_dir) + len + 1)
...
id|index
suffix:semicolon
multiline_comment|/* number of written untracked_cache_dir */
...
id|check_only
suffix:semicolon
multiline_comment|/* from untracked_cache_dir */
DECL|member|valid
r_struct
id|ewah_bitmap
op_star
...
id|sha1_valid
suffix:semicolon
multiline_comment|/* set if exclude_sha1 is not null */
DECL|member|out
r_struct
id|strbuf
id|out
...
op_increment
suffix:semicolon
multiline_comment|/*&n;&t; * untracked_nr should be reset whenever valid is clear, but&n;&t; * for safety..&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* skip non-recurse directories */
r_for
c_loop
(paren
id|i
...
(paren
op_amp
id|ouc-&gt;info_exclude_stat
comma
op_amp
id|untracked-&gt;ss_info_exclude.stat
)paren
suffix:semicolon
id|stat_data_to_disk
c_func
(paren
op_amp
id|ouc-&gt;excludes_file_stat
comma
op_amp
id|untracked-&gt;ss_excludes_file.stat
)paren
suffix:semicolon
id|hashcpy
c_func
(paren
id|ouc-&gt;info_exclude_sha1
comma
id|untracked-&gt;ss_info_exclude.sha1
)paren
suffix:semicolon
id|hashcpy
c_func
(paren
id|ouc-&gt;excludes_file_sha1
comma
id|untracked-&gt;ss_excludes_file.sha1
)paren
suffix:semicolon
id|ouc-&gt;dir_flags
op_assign
...
c_func
(paren
id|untracked-&gt;dir_flags
)paren
suffix:semicolon
id|memcpy
c_func
(paren
id|ouc-&gt;exclude_per_dir
comma
id|untracked-&gt;exclude_per_dir
comma
id|len
...
suffix:semicolon
multiline_comment|/* safe guard for string lists */
id|ewah_free
c_func
(paren
id|wd.valid
...
suffix:semicolon
multiline_comment|/* NUL after exclude_per_dir is covered by sizeof(*ouc) */
id|next
op_add_assign
id|ouc_size
c_func
...
multiline_comment|/*&n; * Normally when an entry is added or removed from a directory,&n; * invalidating that directory is enough. No need to touch its&n; * ancestors. When a directory is shown as &quot;foo/bar/&quot; in git-status&n; * however, deleting or adding an entry may have cascading effect.&n; *&n; * Say the &quot;foo/bar/file&quot; has become untracked, we need to tell the&n; * untracked_cache_dir of &quot;foo&quot; that &quot;bar/&quot; is not an untracked&n; * directory any more (because &quot;bar&quot; is managed by foo as an untracked&n; * &quot;file&quot;).&n; *&n; * Similarly, if &quot;foo/bar/file&quot; moves from untracked to tracked and it&n; * was the last untracked entry in the entire &quot;foo&quot;, we should show&n; * &quot;foo/&quot; instead. Which means we have to invalidate past &quot;bar&quot; up to&n; * &quot;foo&quot;.&n; *&n; * This function traverses all directories from root to leaf. If there&n; * is a chance of one of the above cases happening, we invalidate back&n; * to root. Otherwise we just invalidate the leaf. There may be a more&n; * sophisticated way than checking for SHOW_OTHER_DIRECTORIES to&n; * detect these cases and avoid unnecessary invalidation, for example,&n; * checking for the untracked entry named &quot;bar/&quot; in &quot;foo&quot;, but for now&n; * stick to something safe and simple.&n; */
DECL|function|invalidate_one_component
r_static
r_int
id|invalidate_one_component
c_func
(paren
r_struct
id|untracked_cache
